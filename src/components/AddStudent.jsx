        ...
        try {
            ...
            if (err.response?.data?.status === false && err.response?.data?.data) {
                ...
                Swal.fire({
                    ...
                    html: msg, // ✅ use html instead of text
                    confirmButtonColor: "#2563eb",
                    background: "#111827",
                    color: "#f9fafb",
                    width: 600,
                ...
                // ✅ Handle standard Laravel format { errors: {...} }
            } else if (err.response?.data?.errors) {
                const errors = err.response.data.errors;
                const formatted = Object.entries(errors)
                    .map(([key, value]) => `• ${value.join(", ")}`)
                    .join("<br>");
                msg = `<div style="text-align:left;">${formatted}</div>`;
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-black text-gray-100 pt-24 p-6">
            <motion.div
                ...
                className="max-w-5xl mx-auto bg-gray-900/80 border border-gray-800 rounded-3xl shadow-xl p-8"
            >
                ...
                >
                    <Input label="Registration No" name="registration_number" value={form.registration_number} onChange={handleChange} required />
                    <Input label="Student Name" name="student_name" value={form.student_name} onChange={handleChange} required />
                    <Input label="Nickname" name="nickname" value={form.nickname} onChange={handleChange} required />
                    ...
                    <Input label="Guardian Name" name="guardian_name" value={form.guardian_name} onChange={handleChange} />
                    ...
                    <Input label="Guardian Phone" name="guardian_phone" value={form.guardian_phone} onChange={handleChange} />

                    {/* Contact */}
                    <Input label="Phone 1" name="phone1" value={form.phone1} onChange={handleChange} />
                    ...
                    <Input label="PIN Code" name="pin" value={form.pin} onChange={handleChange} />
                </form>
                ...
                {/* 🧑‍💻 Developer Tools Section (visible only in development mode) */}
                {import.meta.env.MODE === "development" && (
                    <div className="mt-10 p-4 rounded-2xl border border-gray-800 bg-gray-800/60 text-gray-300">
                        ...
                        <p className="text-sm mb-3 text-gray-400">
                            Visible only in <span className="text-sky-400 font-semibold">development</span> mode.
                        </p>
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            <button
                                ...
                                onClick={() =>
                                    setForm({
                                        ...form,
                                        registration_number: "DEV" + Math.floor(Math.random() * 1000),
