  ...
  useEffect(() => {
    const loadImagesAsDataUrls = async () => {
      try {
        ...
        const qrResponse = await fetch(CNATQR);
        const qrBlob = await qrResponse.blob();
        const qrDataUrlReader = new FileReader();
        qrDataUrlReader.onloadend = () => {
          setQrCodeDataUrl(qrDataUrlReader.result);
        };
        qrDataUrlReader.readAsDataURL(qrBlob);
      } catch (error) {
...
export default StudentFeeReceiptPart4;