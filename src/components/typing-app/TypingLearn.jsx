...
export default class TypingLearn extends Component {
  ...
  updateBestTimeForLesson = (lessonId, currentTime) => {
    ...
    let bestTime = null,
      isNewRecord = false;
    if (!currentTime || currentTime &le; 0) return { bestTime: null, isNewRecord: false };
    try {
      const stored = localStorage.getItem(key);
      if (stored !== null) {
  ...
  calculateXPEarned = (accuracy, textLength, timeSeconds) => {
    if (!timeSeconds || timeSeconds &le; 0) return 0;
  ...
  updateGlobalStats = (lessonId, xpEarned, sessionTime, sessionMistakes) => {
    ...
    if (!lastPracticeDate) {
      ...
      if (diffDays &ge; 1 && diffDays < 2) streak += 1;
