...
export default class PythonRoadmap extends Component {
  constructor(props) {
    super(props);

    const stored = JSON.parse(localStorage.getItem("python_progress") || "{}");
  ...
  /* ------------------------------------------------------------
      LIFECYCLE – SCROLL LISTENER (for Top button + mini-progress)
  ------------------------------------------------------------- */
  componentDidMount() {
  ...
  handleScroll = () => {
    const showTop = window.scrollY > 350;
    const showMini = window.scrollY > 120;

    if (showTop !== this.state.showTopButton)
      this.setState({ showTopButton: showTop });
  ...
  toggleComplete = (moduleId) => {
    this.setState((prev) => {
      ...
      localStorage.setItem("python_progress", JSON.stringify(updated));
