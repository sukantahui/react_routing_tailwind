...
export default function Topic6() {
  ...
  const [selectedScenarioId, setSelectedScenarioId] = useState("dep_bank");
  ...
  const scenarios = [{"id":"dep_bank","titleEn":"1. Deposited ₹30,000 Cash into SBI Bank Account","detailEn":"Debit Side: \"To Cash A/c\" (Bank Column ₹30,000) | Credit Side: \"By Bank A/c\" (Cash Column ₹30,000). L.F. = C.","detailBn":"Dr পাশে: \"To Cash A/c\" (Bank কলাম ৩০,০০০) | Cr পাশে: \"By Bank A/c\" (Cash কলাম ৩০,০০০)। L.F. = C।"},{"id":"with_bank","titleEn":"2. Withdrew ₹15,000 Cash from SBI Bank for Office Vault","detailEn":"Debit Side: \"To Bank A/c\" (Cash Column ₹15,000) | Credit Side: \"By Cash A/c\" (Bank Column ₹15,000). L.F. = C.","detailBn":"Dr পাশে: \"To Bank A/c\" (Cash কলাম ১৫,০০০) | Cr পাশে: \"By Cash A/c\" (Bank কলাম ১৫,০০০)। L.F. = C।"}];
  const currentScenario = scenarios.find(s => s.id === selectedScenarioId) || scenarios[0];
  const tallySteps = ["In TallyPrime, press F4 (Contra Voucher).","Select Account: SBI Bank, Particulars: Cash ₹30,000 to deposit cash into bank.","Press Ctrl+A to save the Contra voucher."];
  const tallyStepsBn = ["TallyPrime-এ F4 (Contra Voucher) কি চাপুন।","Account: SBI Bank নির্বাচন করুন, Particulars: Cash ৩০,০০০ টাকা লিখুন।","Ctrl+A চেপে কনট্রা ভাউচার সেভ করুন।"];
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-emerald-500/30 selection:text-emerald-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-950/80 border border-emerald-500/40 text-emerald-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>📊</span>
            <span>TallyPrime Master Series · Module 1.3 · Topic 6</span>
          </div>
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-teal-300 to-sky-300 tracking-tight leading-tight">
            {isBengali ? "দুই-ঘরা নগদ বইয়ে কনট্রা এন্ট্রি (ব্যাংকে নগদ জমা ও উত্তোলন) মাস্টারক্লাস" : "Contra Entries in Two-Column Cash Book Masterclass"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            {isBengali ? "নগদ ও ব্যাংকের অভ্যন্তরীণ আদান-প্রদানে কনট্রা এন্ট্রি (সংকেত \"C\") রেকর্ড করার পদ্ধতি।" : "Executing Contra entries (symbol \"C\") for dual Cash-Bank movements without posting to General Ledger."}
          </p>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
            <div>
              <h2 className="text-xl md:text-2xl font-bold text-white">
                {isBengali ? "দুই-ঘরা নগদ বইয়ে কনট্রা এন্ট্রি (ব্যাংকে নগদ জমা ও উত্তোলন) মাস্টারক্লাস - মূল কাঠামো" : "Contra Entries in Two-Column Cash Book Masterclass - Core Framework"}
              </h2>
            ...
            <div className="flex items-center gap-2 bg-slate-950 p-1.5 rounded-xl border border-slate-800">
              ...
              >
                {isBengali ? "১. ব্যাংকে নগদ জমা" : "1. Cash Deposited into Bank"}
              </button>
              <button
              ...
              >
                {isBengali ? "২. অফিসের জন্য নগদ উত্তোলন" : "2. Cash Withdrawn for Office"}
              </button>
            </div>
          </div>
          ...
          {activeTab === "tab1" ? (
            <div className="p-6 rounded-xl bg-slate-950 border border-emerald-500/40 space-y-4 text-xs sm:text-sm">
              ...
              <p className="text-slate-300 leading-relaxed">
                {isBengali ? "ব্যাংকে নগদ জমা দিলে ব্যাংক ব্যালেন্স বাড়ে (Debit Bank) এবং ক্যাশ কমে (Credit Cash)। L.F. কলামে \"C\" বসে।" : "Depositing Cash into Bank increases Bank balance (Debit Bank Column) and decreases Cash balance (Credit Cash Column). L.F. = \"C\"."}
              </p>
            </div>
          ) : (
            <div className="p-6 rounded-xl bg-slate-950 border border-sky-500/40 space-y-4 text-xs sm:text-sm">
              ...
              <p className="text-slate-300 leading-relaxed">
                {isBengali ? "অফিসের প্রয়োজনে ব্যাংক থেকে নগদ তুললে ক্যাশ বাড়ে (Debit Cash) এবং ব্যাংক কমে (Credit Bank)। L.F. কলামে \"C\" বসে।" : "Withdrawing Cash from Bank for office use increases Cash balance (Debit Cash Column) and decreases Bank balance (Credit Bank Column). L.F. = \"C\"."}
              </p>
            </div>
          )}
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-16 space-y-6">
          <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-2">
            ...
            <span>{isBengali ? "কনট্রা এন্ট্রি সিমুলেটর" : "Contra Entry Simulator"}</span>
          </h2>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-16 space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            {[{"id":1,"titleEn":"Contra Case 1: Deposited Cash ₹40,000 into HDFC Bank Account","titleBn":"কনট্রা ক্ষেত্র ১: এইচডিএফসি ব্যাংকে নগদ ৪০,০০০ টাকা জমা","drEn":"Debit Side: \"To Cash A/c\" (Bank Column ₹40,000) | L.F. = C","drBn":"Dr পাশ: \"To Cash A/c\" (Bank কলাম ৪০,০০০ টাকা) | L.F. = C","crEn":"Credit Side: \"By Bank A/c\" (Cash Column ₹40,000) | L.F. = C","crBn":"Cr পাশ: \"By Bank A/c\" (Cash কলাম ৪০,০০০ টাকা) | L.F. = C"},{"id":2,"titleEn":"Contra Case 2: ATM Withdrawal ₹10,000 for Office Expenses","titleBn":"কনট্রা ক্ষেত্র ২: অফিসের খরচের জন্য এটিএম থেকে ১০,০০০ টাকা উত্তোলন","drEn":"Debit Side: \"To Bank A/c\" (Cash Column ₹10,000) | L.F. = C","drBn":"Dr পাশ: \"To Bank A/c\" (Cash কলাম ১০,০০০ টাকা) | L.F. = C","crEn":"Credit Side: \"By Cash A/c\" (Bank Column ₹10,000) | L.F. = C","crBn":"Cr পাশ: \"By Cash A/c\" (Bank কলাম ১০,০০০ টাকা) | L.F. = C"}].map((ex) => (
          ...
          <div className="bg-gradient-to-br from-slate-900 via-slate-900/90 to-emerald-950/30 border border-emerald-500/30 rounded-2xl p-6 md:p-8 shadow-xl relative overflow-hidden">
            ...
            <div className="space-y-6 text-slate-300 leading-relaxed text-sm sm:text-base">
              <div className="bg-slate-950/70 border border-slate-800 rounded-xl p-5 space-y-3">
                ...
                <p>
                  {isBengali ? "কনট্রা এন্ট্রি একই সাথে Cash ও Bank কলামকে প্রভাবিত করে এবং L.F. কলামে \"C\" সংকেত বসে!" : "Contra entries affect Cash and Bank columns simultaneously with letter \"C\" in L.F. column!"}
                </p>
              </div>
              ...
              <div className="bg-slate-950/70 border border-slate-800 rounded-xl p-5 space-y-3">
                ...
                <div className="space-y-3 text-xs sm:text-sm font-sans border-l-2 border-emerald-500/40 pl-4 py-1">
                  <div>
                    <p><strong className="text-emerald-400">Swadeep (Lab Student):</strong> <em>{isBengali ? "\"স্যার, কনট্রা এন্ট্রির জন্য কেন জেনারেল লেজারে আলাদা পোস্টিং করতে হয় না?\"" : "\"Sir, why is no posting required in General Ledger for Contra entries?\""}</em></p>
                  </div>
                  <div>
                    <p><strong className="text-sky-300">CNAT Sir:</strong> <em>{isBengali ? "\"কারণ ক্যাশ বুকে নগদ ও ব্যাংক উভয় লেজার পাশাপাশি রয়েছে! জেনারেল লেজারে পাঠালে একই হিসাব দুবার রেকর্ড হয়ে যাবে!\"" : "\"Because both Cash and Bank ledger columns exist side-by-side in the Cash Book itself! Posting to general ledger would cause double recording!\""}</em></p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={isBengali ? "Topic 6 মূল্যায়ন ও অনুশীলন প্রশ্নাবলী" : "Topic 6 Assessment & Diagnostic Practice"}
            questions={questions}
          />
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto">
          <Teacher
            note={
              isBengali
                ? "কনট্রা এন্ট্রি একই সাথে Cash ও Bank কলামকে প্রভাবিত করে এবং L.F. কলামে \"C\" সংকেত বসে!"
                : "Contra entries affect Cash and Bank columns simultaneously with letter \"C\" in L.F. column!"
            }
          />
        </section>
