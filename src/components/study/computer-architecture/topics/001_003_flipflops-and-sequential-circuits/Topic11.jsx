...
const Topic11 = () => {
  const [tInput, setTInput] = useState(true);
  const [clkState, setClkState] = useState(false);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-teal-500/30 selection:text-teal-200">
      <header ref={addRef} className="max-w-5xl mx-auto mb-12 text-center">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-teal-950/70 border border-teal-700/60 text-teal-300 text-xs font-semibold uppercase tracking-wider mb-4 shadow-lg">
          <span>⚡</span>
          <span>Computer Architecture Masterclass · Module 001_003 · Topic 11</span>
        </div>
        <h1 className="text-2xl sm:text-xl sm:text-2xl md:text-3xl font-bold text-white tracking-tight leading-tight mb-4">
          T Flip-Flop (NAND Implementation, Toggle Operation &amp; Frequency Division)
        </h1>
        <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
          Master the Toggle (T) Flip-Flop. Discover how linking steering inputs creates deterministic divide-by-2 frequency prescalers and binary ripple counters.
        </p>
      </header>
      ...
      <section ref={addRef} className="max-w-5xl mx-auto mb-16">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-2">
            <span className="text-cyan-400">📐</span> Hardware Schematics &amp; Frequency Divider Waveforms
          </h2>
          ...
          {activeDiagramTab === "nand-schematic" && (
            <div className="space-y-4">
              <span className="text-xs font-mono font-bold uppercase tracking-wider text-teal-400 block">
                Internal 4-NAND Gate T Flip-Flop Schematic
              </span>
              <div className="rounded-xl border border-slate-800 bg-slate-950 p-4 overflow-x-auto">
                <svg viewBox="0 0 940 340" className="w-full h-auto text-xs font-mono select-none">
                  {/* Pin T */}
                  <text x="25" y="65" fill="#14b8a6" fontWeight="bold" fontSize="14">T (Toggle)</text>
                  <circle cx="100" cy="60" r="4" fill="#14b8a6" />
                  <line x1="100" y1="60" x2="280" y2="60" stroke={tInput ? "#14b8a6" : "#475569"} strokeWidth="2.5" />
                  <circle cx="170" cy="60" r="3.5" fill="#14b8a6" />
                  <line x1="170" y1="60" x2="170" y2="255" stroke={tInput ? "#14b8a6" : "#475569"} strokeWidth="2.5" />
                  <line x1="170" y1="255" x2="280" y2="255" stroke={tInput ? "#14b8a6" : "#475569"} strokeWidth="2.5" />

                  {/* Pin CLK */}
                  <text x="20" y="160" fill="#38bdf8" fontWeight="bold" fontSize="14">CLK (Clock)</text>
                  <circle cx="100" cy="155" r="4" fill="#38bdf8" />
                  <line x1="100" y1="155" x2="130" y2="155" stroke={clkState ? "#38bdf8" : "#475569"} strokeWidth="2.5" />
                  <circle cx="130" cy="155" r="3.5" fill="#38bdf8" />
                  <line x1="130" y1="155" x2="130" y2="80" stroke={clkState ? "#38bdf8" : "#475569"} strokeWidth="2.5" />
                  <line x1="130" y1="80" x2="280" y2="80" stroke={clkState ? "#38bdf8" : "#475569"} strokeWidth="2.5" />
                  <line x1="130" y1="155" x2="130" y2="230" stroke={clkState ? "#38bdf8" : "#475569"} strokeWidth="2.5" />
                  <line x1="130" y1="230" x2="280" y2="230" stroke={clkState ? "#38bdf8" : "#475569"} strokeWidth="2.5" />
          ...
          {activeDiagramTab === "block-diagram" && (
            <div className="space-y-4">
              <span className="text-xs font-mono font-bold uppercase tracking-wider text-cyan-400 block">
                Standard T Flip-Flop Block Symbol
              </span>
              <div className="rounded-xl border border-slate-800 bg-slate-950 p-6 flex justify-center">
                <svg viewBox="0 0 500 220" className="w-full max-w-lg h-auto font-mono select-none">
                  ...
                  <text x="250" y="65" fill="#14b8a6" textAnchor="middle" fontWeight="bold" fontSize="16">T FLIP-FLOP</text>
                  <line x1="80" y1="80" x2="150" y2="80" stroke="#14b8a6" strokeWidth="2.5" />
                  <text x="60" y="85" fill="#14b8a6" fontWeight="bold">T</text>
                  <line x1="80" y1="140" x2="150" y2="140" stroke="#38bdf8" strokeWidth="2.5" />
                  <polygon points="150,132 162,140 150,148" fill="#38bdf8" />
                  <text x="40" y="145" fill="#38bdf8" fontWeight="bold">&gt;CLK</text>
          ...
          {activeDiagramTab === "timing-diagram" && (
            <div className="space-y-4">
              <span className="text-xs font-mono font-bold uppercase tracking-wider text-amber-400 block">
                Divide-By-2 Frequency Divider Waveform
              </span>
              <div className="rounded-xl border border-slate-800 bg-slate-950 p-4 overflow-x-auto">
                <svg viewBox="0 0 940 160" className="w-full h-auto text-xs font-mono select-none">
                  <text x="40" y="45" fill="#38bdf8" fontWeight="bold">Input CLK (f_in)</text>
                  ...
                  <text x="40" y="105" fill="#22c55e" fontWeight="bold">Output Q (f_in / 2)</text>
      ...
      <section ref={addRef} className="max-w-5xl mx-auto mb-16">
        <h2 className="text-xl sm:text-2xl font-bold text-white mb-6 flex items-center gap-2">
          <span className="text-emerald-400">⚡</span> Live Interactive T Flip-Flop Simulator
        </h2>
        <div className="rounded-2xl bg-slate-900/90 border border-slate-800 p-6 md:p-8 space-y-6 shadow-2xl">
          <div className="flex flex-wrap items-center justify-between gap-4 pb-6 border-b border-slate-800">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Toggle Input:</span>
            <div className="flex gap-3">
              <button
                onClick={() => setTInput(!tInput)}
                className={clsx(
              ...
              <button
                onClick={pulseClock}
                className="px-5 py-2 rounded-xl text-xs font-mono font-bold bg-teal-600 hover:bg-teal-500 text-white shadow-lg shadow-teal-900/50 transition-all flex items-center gap-2"
              >
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="p-4 rounded-xl bg-slate-950 border border-slate-800">
              <span className="text-xs text-slate-500 block mb-1">Toggle Mode</span>
              <div className="text-lg font-mono font-bold text-teal-300">{tInput ? "Active Toggle (f/2)" : "Memory Hold"}</div>
            </div>
            <div className="p-4 rounded-xl bg-slate-950 border border-slate-800">
              <span className="text-xs text-slate-500 block mb-1">Clock State</span>
              <div className="text-lg font-mono font-bold text-cyan-300">{clkState ? "EDGE ACTIVE (↑)" : "LOW (Idle)"}</div>
            </div>
      ...
      <section ref={addRef} className="max-w-5xl mx-auto mb-16">
        <FAQTemplate title="T Flip-Flop FAQs" questions={questions} />
      </section>
      ...
      <section ref={addRef} className="max-w-5xl mx-auto mb-16">
        <Teacher note="The T Flip-Flop is the fundamental building block of all digital binary counters and digital clocks. Setting T=1 continuously turns it into a perfect divide-by-2 frequency divider!" />
      </section>
    </div>
  );
};
