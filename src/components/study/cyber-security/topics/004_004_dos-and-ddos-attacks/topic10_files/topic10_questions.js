...
if ($ssl_early_data) {
  ...
  {
    question: "Synthesize the mathematical formulation of Cloud Scrubbing Throughput (T_clean), Volumetric Ingress Load (V_total), Layer 3/4 Hardware Drop Rate (DR_L3L4), Layer 7 WAF Drop Rate (DR_L7), and Total Added Scrubbing Latency (\\Delta L_scrub).",
    shortAnswer: "Clean traffic throughput is T_clean = V_total * (1 - DR_L3L4) * (1 - DR_L7); total added latency is \\Delta L_scrub = RTT_Anycast + T_FPGA_inspect + T_WAF_eval + RTT_GRE_tunnel; with Anycast routing and hardware FPGA pipelines, \\Delta L_scrub is bounded under 8.5 milliseconds.",
    explanation: "Let $V_{\\text{total}}$ represent the total incoming dirty flood volume (e.g. 1,000 Gbps). If the Layer 3/4 FPGA hardware filters drop $99.5\\%$ of volumetric UDP/SYN packets ($DR_{\\text{L3L4}} = 0.995$), the remaining traffic entering Layer 7 inspection is $1,000 \\times (1 - 0.995) = 5.0$ Gbps. If the Cloud WAF filters $60\\%$ of malicious HTTP/bot requests ($DR_{\\text{L7}} = 0.60$), the clean traffic delivered to the origin is: $T_{\\text{clean}} = 5.0 \\times (1 - 0.60) = 2.0$ Gbps. Total added scrubbing latency is $\\Delta L = \\text{RTT}_{\\text{Anycast}} (2.5\\text{ms}) + T_{\\text{FPGA}} (0.005\\text{ms}) + T_{\\text{WAF}} (1.8\\text{ms}) + \\text{RTT}_{\\text{GRE}} (3.5\\text{ms}) = 7.805$ milliseconds, providing seamless high-speed user browsing.",
