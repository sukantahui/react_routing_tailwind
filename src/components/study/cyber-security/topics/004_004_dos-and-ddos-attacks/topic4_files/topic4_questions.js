...
# Verification: Full Window Scaling & SACK preserved under SYN Flood attacks!`
  },
  {
    ...
    explanation: "The top 5 bits store t = (time() >> 6) % 32. The counter increments every 64 seconds and wraps around every 32 minutes (32 * 64 = 2048 seconds). When the final ACK arrives, the server checks if the extracted timestamp is within [t, t-1]. If an ACK arrives with a timestamp older than 128 seconds, the server rejects it as an expired replay attack.",
...
// Penalty: Imprisonment for a term up to 2 Years, or with Fine, or with both`
  },
  {
    ...
    explanation: "The HMAC hash incorporates all five parameters: HMAC(SrcIP || SrcPort || DstIP || DstPort || t, K). Because each client connection has a distinct ephemeral source port or source IP, the resulting 24-bit hashes are statistically uniform, preventing cross-connection collisions.",
...
# Result: 2 (Terminates dead embryonic sessions in 7 seconds!)`
  },
  {
    ...
    explanation: "The 32-bit Initial Sequence Number (ISN) is divided into three distinct bitfields: 1. Bits 31-27 (5 bits): Timestamp epoch t = (time() >> 6) % 32. 2. Bits 26-24 (3 bits): Index m in [0, 7] mapping to standard MSS table values. 3. Bits 23-0 (24 bits): Cryptographic signature S = HMAC-SHA1(SrcIP || SrcPort || DstIP || DstPort || t, K) % 2^24. When the final ACK arrives with Ack = ISN + 1, the server computes ISN = Ack - 1, extracts t and m, verifies |t_now - t| <= 1, and recomputes the HMAC. If the lower 24 bits match, connection is authenticated with zero state memory allocation during the SYN phase.",
