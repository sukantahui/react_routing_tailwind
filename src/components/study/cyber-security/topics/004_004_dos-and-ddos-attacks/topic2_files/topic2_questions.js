...
// Penalty: Imprisonment for a term up to 2 Years, or with Fine, or with both`
  },
  {
    ...
    explanation: "IPv4 requires every router and host to verify the header checksum: ∑ Word_i = 0xFFFF. When an attacker floods 20 Million PPS, the server CPU must compute 20,000,000 checksum calculations every second. Modern NICs offload checksum validation to hardware (Checksum Offloading / Rx-Checksum), freeing the CPU to process legitimate application data.",
...
iptables -A INPUT -p icmp --icmp-type 8 -j DROP`
  },
  {
    ...
    explanation: "In volumetric reflection attacks, attackers spoof the victim's IP and send tiny requests to misconfigured third-party servers (DNS, NTP, Memcached). A Memcached request of 15 bytes generates a 750,000-byte reply (50,000x amplification). A single 10 Mbps home internet connection can generate 500 Gbps of volumetric attack traffic directed at the victim in Kolkata.",
...
// Memcached UDP  : 10,000x to 51,000x Amplification (Historic 1.3 Tbps GitHub Flood!)`
  ...
  {
    question: "Synthesize the mathematical relationship between Ingress Attack Bandwidth (V_ingress), Network Uplink Capacity (C_pipe), Buffer Jitter Variance (σ_jitter), and Packet Loss Probability (P_loss).",
    shortAnswer: "Packet loss probability is modeled as P_loss = 1 - e^(- max(0, V_ingress - C_pipe) / σ_jitter); when ingress flood volume V_ingress exceeds physical link capacity C_pipe, packet drop rate approaches 100%.",
    explanation: "Let V_ingress = ∑ (N_j × P_size × R_pps) represent the total incoming traffic volume generated by M attack sources (e.g. 450 Gbps), C_pipe represent the physical uplink bandwidth capacity (e.g. 10 Gbps), and σ_jitter represent traffic burstiness. When V_ingress > C_pipe, link utilization exceeds 100%, queue buffers overflow, and packet drop probability is: P_loss = 1 - e^(-(V_ingress - C_pipe) / σ_jitter). Upstream cloud scrubbing ensures that V_ingress <= C_pipe, maintaining P_loss = 0.0%.",
