...
// P2P / DHT   : [Bot 1] ⇄ [Bot 2] ⇄ [Bot 3] (Decentralized ➔ No single point of failure!)`
  },
  {
    ...
    explanation: "Instead of hardcoding a single C2 domain like `evil-c2.com` (which defenders block in DNS), DGA uses a seed (such as the current date): $DGA(\\text{Date}) \\to 500$ random domain names. The malware attempts to connect to all 500 domains daily. The attacker registers just 1 domain (`k9z3x7w1.in`) for that specific date. Defenders cannot pre-blacklist all possible algorithmic domains.",
...
// Economic Ratio  : 1 : 23,333 Disparity!`
  ...
  {
    question: "Synthesize the mathematical relationship between Aggregated Botnet Flood Bandwidth (B_total = \\sum B_i), Cloud Scrubbing Capacity (C_scrubbing), Jitter / Noise Variance (\\sigma_jitter), and Link Saturation / Service Failure Probability (P_saturation).",
    shortAnswer: "Link saturation probability is modeled as P_saturation = 1 - e^(- max(0, B_total - C_scrubbing) / \\sigma_jitter); deploying multi-terabit Anycast cloud scrubbing networks (C_scrubbing > B_total) drives service failure probability to zero.",
    explanation: "Let $B_{\\text{total}} = \\sum_{i=1}^{N} B_i$ represent the aggregate bandwidth generated by $N$ botnet nodes (e.g. 500,000 nodes generating 1.2 Tbps), $C_{\\text{scrubbing}}$ represent the cloud scrubbing capacity (e.g. 10 Tbps Anycast global network), and $\\sigma_{\\text{jitter}}$ represent traffic burstiness. When attack bandwidth exceeds scrubbing capacity ($B_{\\text{total}} > C_{\\text{scrubbing}}$), saturation probability is: $P_{\\text{saturation}} = 1 - e^{-\\frac{B_{\\text{total}} - C_{\\text{scrubbing}}}{\\sigma_{\\text{jitter}}}}$. When organizations deploy global Anycast scrubbing where $C_{\\text{scrubbing}} \\gg B_{\\text{total}}$, saturation probability collapses to zero ($P_{\\text{saturation}} \\to 0$).",
