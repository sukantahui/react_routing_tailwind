...
-- Automatically terminates any query executing longer than 3 seconds!`
  },
  {
    ...
    explanation: "Let W_workers represent the server's thread pool capacity (e.g. 256 threads in Apache prefork), R_attack represent the attack request rate (e.g. 200 req/s), and T_process represent average processing time (e.g. 3.0 seconds for Slowloris or heavy queries). Total active threads required is W_req = R_attack * T_process = 200 * 3.0 = 600 threads. Since 600 > 256, utilization is U = 600 / 256 = 2.34 > 1.0. Starvation probability is: P_starvation = 1 - e^(-(2.34 - 1.0) * 10) = 1 - e^(-13.4) = 99.99%. Deploying asynchronous Nginx event loops (W_workers = 65,535) reduces U to 0.009, keeping P_starvation = 0.0%.",
