...
// Protection   : National Tier-1 Terabit BGP Scrubbing & Diversion via NIXI (National Internet Exchange of India)`
  },
  {
    ...
    shortAnswer: "Cascading failure is P_cascade = 1 - \\prod_{i=1}^{K} (1 - p_i); EDoS cost accumulation is Cost_EDoS = C_base + \\int_0^T \\lambda_attack(t) * Cost_instance dt; infrastructure resilience index is R_infra = 1 - P_cascade; enforcing dedicated OOB channels and auto-scaling hard caps ensures R_infra = 1.0.",
    explanation: "Let $K$ represent the number of interdependent critical infrastructure sectors (e.g. Telecom, Power, BFSI, Healthcare). If the independent failure probabilities under a multi-vector DDoS attack are $p_1 = 0.40$ (Telecom), $p_2 = 0.35$ (Power), $p_3 = 0.50$ (BFSI), the probability of at least one sector collapsing is $P_{\\text{cascade}} = 1 - (1 - 0.40)(1 - 0.35)(1 - 0.50) = 1 - (0.60 \\times 0.65 \\times 0.50) = 1 - 0.195 = 80.5\\%$. In cloud environments, EDoS cost accumulation is $\\text{Cost}_{\\text{EDoS}} = C_{\\text{base}} + \\int_0^T \\lambda_{\\text{attack}}(t) \\times \\text{Cost}_{\\text{instance}} dt$. Implementing Out-of-Band (OOB) SCADA dark fiber and strict auto-scaling hard caps ($MaxSize = 20$) forces $p_i \\to 0$, driving Infrastructure Resiliency to $R_{\\text{infra}} = 1 - 0 = 100.0\\%$.",
