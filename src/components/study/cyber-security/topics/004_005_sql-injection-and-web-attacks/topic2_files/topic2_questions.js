...
'; DECLARE @data varchar(100); SELECT @data = password FROM users WHERE id=1;
  ...
  },
  {
    ...
    explanation: "Let the relational table be represented as a set of tuples T = {t_1, t_2, ..., t_N}. In a legitimate query, the row filter is: R = {t in T | P_orig(t) = True}. Under string concatenation, an attacker injects a tautology P_inject ≡ True. The evaluated filter predicate mutates to: P_eval(t) = P_orig(t) OR True ≡ True for all t in T. Therefore, the returned relation is R = {t in T | True} = T (the entire database table is returned). In Prepared Statements, the AST parser treats P_inject as a literal value parameter v in Σ*, keeping the AST node structure P_eval(t) = (t.user = v) fixed, guaranteeing P_bypass = 0.00%.",
