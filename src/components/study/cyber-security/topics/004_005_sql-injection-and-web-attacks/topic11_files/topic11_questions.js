...
pgaudit.log_level = 'notice'`
  },
  {
    ...
    explanation: "Let the domain of application object models be $\\mathcal{O}$. An ORM maps object queries $Q_{\\mathcal{O}}$ to relational algebra expressions: $\\mathcal{R}: \\mathcal{O} \\to \\mathcal{A}$. In standard ORM compilation (e.g. Prisma/Hibernate), $\\mathcal{R}$ pre-compiles the algebraic tree into a static Abstract Syntax Tree template $T_0 = Q_{\\text{AST}}$ with positional parameter slots $\\{$1, $2, \\dots, $n\\}$. Entity attribute values are mapped directly to binary wire parameter buffers: $f_{\\text{bind}}: \\mathcal{O}_{\\text{attrs}} \\to V_T$. Because no user-supplied data passes through the grammar production rules of the relational compiler, $\\Delta \\text{AST} = \\text{AST}(Q_{\\text{executed}}) - T_0 = \\emptyset$. This establishes the mathematical proof that standard ORM query builders provide absolute injection immunity ($P_{\\text{sqli}} = 0.00\\%$) as long as raw escape hatches (`$queryRawUnsafe`, `Sequelize.literal`) are avoided.",
