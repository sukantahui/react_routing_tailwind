...
await User.findAll({
  ...
  },
  {
    ...
    explanation: "Let the formal grammar of the database query language be G = (V_N, V_T, P, S) where V_N is non-terminal grammar symbols, V_T is terminal literal tokens, P is production rules, and S is the start symbol. In an un-parameterized query, untrusted input ω passes through P, adding new non-terminal subtrees (e.g. S → WHERE → OR → TRUE), resulting in ΔAST ≠ ∅. In a Parameterized Prepared Statement, the query template Q_template is compiled in Phase 1, generating a fixed tree T_0 = AST(Q_template). In Phase 2, parameter binding function f_param: Ω → V_T assigns inputs strictly to leaf nodes in V_T without invoking any production rule in P. Therefore, AST(Q_executed) = T_0, proving ΔAST = ∅ and mathematically establishing that the probability of query logic mutation is P_sqli = 0.000% (absolute mathematical immunity).",
