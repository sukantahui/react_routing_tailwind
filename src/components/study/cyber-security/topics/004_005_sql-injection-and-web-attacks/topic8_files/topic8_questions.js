...
// 2. Autorize replays with: Cookie: session=low_priv_user (HTTP 200 - VULNERABILITY FLAGGED!)`
  },
  {
    ...
    explanation: "Let the universe of users be $U$, operations be $R$, and objects be $O$. An access control function $\\text{Access}: U \\times R \\times O \\to \\{0, 1\\}$ evaluates: $\\text{Access}(u, r, o) = \\mathbf{1}_{\\{\\text{Role}(u) \\ge \\text{ReqRole}(r)\\}} \\times \\mathbf{1}_{\\{\\text{Owner}(o) = u \\lor \\text{IsAdmin}(u)\\}}$. When ownership is omitted, $\\text{Access}$ evaluates strictly to 1 for all $o \\in O$, causing IDOR. For CSRF, let $\\text{SameSite}_{\\text{strict}} \\in \\{0, 1\\}$ and $\\text{Token}_{\\text{valid}} \\in \\{0, 1\\}$. The CSRF exploitability probability is: $P_{\\text{csrf}} = (1 - \\text{SameSite}_{\\text{strict}}) \\times (1 - \\text{Token}_{\\text{valid}})$. When both controls are enforced ($\\{1, 1\\}$), $P_{\\text{csrf}} = (1 - 1) \\times (1 - 1) = 0.00\\%$, providing mathematical proof of complete CSRF immunity.",
