...
dalfox url "https://kolkata-fintech.in/search?q=1" --deep-domxss --waf-bypass --report-json report.json`
  },
  {
    ...
    explanation: "Let the browser HTML parser be a Finite State Machine M = (Q, Σ, δ, q_0, F) where state q in Q represents parsing states (e.g. q_data, q_tag_open, q_script_data). An XSS payload aims to force a state transition δ(q_data, '<') = q_tag_open or δ(q_attribute, '\"') = q_tag_open. Context-aware output encoding maps input characters σ in Σ to safe entity representations f(σ), ensuring δ(q, f(σ)) = q (state invariance). Therefore, sanitization efficiency is E_context in [0.0, 1.0]. With strict nonce CSP enforcement CSP_enforce in [0.0, 1.0], overall exploitability is: P_xss = (1 - E_context) × (1 - CSP_enforce). When both layers are enforced (E_context = 1.0, CSP_enforce = 1.0), P_xss = 0.00%, providing mathematical proof of complete XSS immunity.",
