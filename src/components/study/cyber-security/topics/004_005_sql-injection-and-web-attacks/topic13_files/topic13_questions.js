...
// 4. Archive temporary AWS WAF custom rule`
  },
  {
    ...
    explanation: "Let the formal grammar of the database query language be $G = (V_N, V_T, P, S)$. In vulnerable string concatenation, untrusted input $x \\in \\Sigma^*$ is parsed by the SQL lexer, introducing new non-terminal grammar productions $P_{\\text{injected}} \\subset P$, such that $\\text{AST}(Q_{\\text{vuln}}) = T_0 \\cup \\Delta T$, where $\\Delta T \\ne \\emptyset$ (e.g. injected OR nodes, subqueries). In parameterized remediation, the compilation function $C: \\Sigma^* \\to \\mathcal{T}$ pre-compiles the query template $Q_0$ into a fixed tree structure $T_0 = C(Q_0)$ with placeholder slots $S_1, \\dots, S_n \\in V_T$. In Phase 2, parameters $x_1, \\dots, x_n$ are bound directly to leaf slots without passing through the production rules $P$. Therefore, $\\Delta \\text{AST} = \\text{AST}(Q_{\\text{rem}}) - T_0 = \\emptyset$. Because $\\Delta \\text{AST} = \\emptyset$ for all $x \\in \\Sigma^*$, the probability of query logic alteration is $P_{\\text{sqli}} = 0.000\\%$, establishing the formal mathematical proof of complete remediation.",
