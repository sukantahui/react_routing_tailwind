...
def generate_explanations(project, code):
    ...
    # Common logic patterns – can be extended as needed
    if "Quick Sort" in title:
        logic = (
            "Quicksort is a divide‑and‑conquer algorithm. It picks the last element as pivot, "
            "partitions the array so that elements less than the pivot come before it and greater ones after, "
            "then recursively sorts the subarrays."
        )
        code_exp = (
            "The code defines `swap()` to exchange two integers, `partition()` which uses the last element as pivot "
            "and returns the correct pivot index, and `quickSort()` which calls itself on the left and right parts. "
            "In `main()`, the array is read, `quickSort()` is called, and the sorted array is printed."
        )
    elif "Merge Sort" in title:
        logic = (
            "Merge sort is a divide‑and‑conquer algorithm that recursively splits the array into halves, "
            "sorts each half, and then merges the two sorted halves using an auxiliary array."
        )
        code_exp = (
            "The code includes `merge()` which combines two sorted subarrays using temporary arrays, and "
            "`mergeSort()` which recursively divides the array. In `main()`, input is read, sorted, and printed."
        )
    elif "Heap Sort" in title:
        logic = (
            "Heap sort first builds a max‑heap from the array, then repeatedly extracts the maximum element "
            "and places it at the end, reducing the heap size each time."
        )
        code_exp = (
            "The code provides `heapify()` to maintain the heap property, `heapSort()` that builds the heap "
            "and then extracts elements. `main()` handles I/O and displays the sorted array."
        )
    elif "Counting Sort" in title:
        logic = (
            "Counting sort is a non‑comparison sort that works by counting the occurrences of each distinct value, "
            "then using those counts to compute the positions of each element in the sorted output."
        )
        code_exp = (
            "The function `countingSort()` first finds the maximum element, creates a count array, computes cumulative "
            "counts, and then places elements in the correct order in an output array, finally copying back. "
            "`main()` reads input, calls the sort, and prints the result."
        )
    elif "Radix Sort" in title:
        logic = (
            "Radix sort processes each digit of the numbers, sorting the array digit by digit from least significant "
            "to most significant using a stable sort (counting sort) as a subroutine."
        )
        code_exp = (
            "The code includes `countingSortRadix()` which sorts based on a specific digit, and `radixSort()` that "
            "iterates over each digit (powers of 10) and calls the counting sort. `main()` reads data and prints the "
            "sorted array."
        )
    elif "Median of Two Sorted Arrays" in title:
        logic = (
            "The algorithm uses binary search on the smaller array to find a partition such that all elements on the left "
            "are ≤ all elements on the right, allowing the median to be computed in O(log(min(m,n))) time."
        )
        code_exp = (
            "The function `findMedianSortedArrays()` applies binary search on the smaller array, calculates the "
            "corresponding partition in the larger array, and checks the left‑max and right‑min conditions. "
            "`main()` reads two sorted arrays and prints the median."
        )
    elif "Kth Smallest Element (Quickselect)" in title:
        logic = (
            "Quickselect uses the same partitioning as quicksort but only recurses into the part that contains the "
            "k‑th smallest element, giving average O(n) time."
        )
        code_exp = (
            "The code provides `partition()` (same as quicksort) and `quickSelect()` which recurses into the appropriate "
            "half based on the pivot index. `main()` reads the array and k, calls quickselect, and prints the result."
        )
    elif "Next Permutation" in title:
        logic = (
            "The algorithm finds the longest decreasing suffix, then swaps the pivot with the next larger element, "
            "and finally reverses the suffix to get the next lexicographic permutation."
        )
        code_exp = (
            "`nextPermutation()` implements the three steps: find the first decreasing element from the right, "
            "find the element just larger than it, swap, and reverse the suffix. `main()` reads an array and outputs "
            "the next permutation."
        )
    elif "Longest Increasing Subsequence" in title:
        logic = (
            "The O(n log n) solution uses a `tails` array where `tails[i]` stores the smallest possible tail value for "
            "an increasing subsequence of length i+1. Binary search is used to update tails."
        )
        code_exp = (
            "`lengthOfLIS()` initializes `tails` and for each number performs binary search to find its position. "
            "If it extends the longest sequence, the size increases. `main()` reads the array and prints the LIS length."
        )
    elif "Maximum Product Subarray" in title:
        logic = (
            "The algorithm tracks both the maximum and minimum product ending at each position because a negative number "
            "can turn a small product into a large one. It updates the result accordingly."
        )
        code_exp = (
            "`maxProduct()` iterates through the array, swapping `maxProd` and `minProd` when encountering a negative "
            "number, then updates both using the current element. The global maximum is stored in `result`. `main()` "
            "reads input and prints the answer."
        )
    elif "Find All Triplets with Zero Sum" in title:
        logic = (
            "After sorting the array, the algorithm fixes one element and uses two pointers (left and right) to find "
            "pairs that sum to the negative of the fixed element, skipping duplicates to ensure unique triplets."
        )
        code_exp = (
            "`findTriplets()` sorts the array, then for each i (avoiding duplicates) it uses two pointers to find "
            "triplets. When a triplet is found, it prints it and moves the pointers past duplicates. `main()` handles "
            "input and calls the function."
        )
    elif "Trapping Rain Water" in title:
        logic = (
            "The two‑pointer technique maintains left and right maxima. Water trapped at a position is determined by "
            "the smaller of the two maxima minus the current height."
        )
        code_exp = (
            "`trap()` initializes two pointers and two maximums. At each step, it moves the pointer with the smaller "
            "height, updates the corresponding maximum, and adds trapped water. `main()` reads heights and prints "
            "the total water."
        )
    elif "Container With Most Water" in title:
        logic = (
            "Start with two pointers at the ends and move the pointer pointing to the shorter line inward, "
            "calculating the area at each step and keeping the maximum."
        )
        code_exp = (
            "`maxArea()` uses two pointers and a while loop. It computes the area using the shorter height and the "
            "distance, updates the maximum, and moves the pointer of the shorter line. `main()` reads heights and "
            "outputs the maximum area."
        )
    elif "Sliding Window Maximum" in title:
        logic = (
            "A deque is used to store indices of array elements in decreasing order. Before adding a new element, "
            "indices out of the window are removed from the front, and smaller elements are removed from the back. "
            "The front of the deque always holds the maximum of the current window."
        )
        code_exp = (
            "The code implements a simple deque structure. `maxSlidingWindow()` iterates through the array, maintaining "
            "the deque as described, and prints the maximum when the window is full. `main()` reads input and calls "
            "the function."
        )
    elif "Minimum Size Subarray Sum" in title:
        logic = (
            "A sliding window (two pointers) is used. Expand the right pointer to increase the sum, and when the sum "
            "meets or exceeds the target, shrink the left pointer to find the minimal length."
        )
        code_exp = (
            "`minSubArrayLen()` uses two pointers and a running sum. For each right, it adds the element, then while "
            "sum ≥ target, it updates the minimal length and subtracts the left element. `main()` reads input and prints "
            "the result."
        )
    elif "First Missing Positive" in title:
        logic = (
            "The algorithm places each positive integer at its correct index (value i should be at index i-1) by swapping. "
            "Then the first index where the value is not i+1 gives the smallest missing positive."
        )
        code_exp = (
            "`firstMissingPositive()` loops through the array, swapping elements to their correct positions. Then it "
            "scans for the first mismatch. `main()` reads input and prints the missing number."
        )
    elif "Rotate Image (2D Matrix) 90° Clockwise In‑Place" in title:
        logic = (
            "First transpose the matrix (swap rows and columns), then reverse each row to achieve a 90° clockwise rotation."
        )
        code_exp = (
            "`rotate()` first performs an in‑place transpose by swapping elements across the diagonal, then reverses each "
            "row. `main()` reads the matrix, calls rotate, and prints the rotated matrix."
        )
    elif "Set Matrix Zeroes" in title:
        logic = (
            "Use the first row and first column as markers. First determine whether the first row/column should be zeroed, "
            "then mark rows and columns to be zeroed using the markers, finally apply the zeroing."
        )
        code_exp = (
            "`setZeroes()` checks first row/column for zeros, then uses those rows/cols as markers. After marking, it "
            "zeroes the appropriate rows and columns, and finally handles the first row/column if needed. `main()` reads "
            "the matrix and prints the result."
        )
    elif "Pascal's Triangle" in title:
        logic = (
            "Each row is built using the previous row: the first and last elements are 1, and the inner elements are the "
            "sum of the two elements above from the previous row."
        )
        code_exp = (
            "`generate()` allocates memory for the triangle, sets the first and last elements to 1, and computes the inner "
            "values. `main()` reads the number of rows, calls generate, and prints the triangle."
        )
    elif "Find Duplicate Number (Floyd's Cycle Detection)" in title:
        logic = (
            "Treat the array as a linked list where the value at index i points to index nums[i]. The duplicate creates a "
            "cycle. Floyd's algorithm finds the cycle entry point, which is the duplicate."
        )
        code_exp = (
            "`findDuplicate()` uses two pointers (slow and fast) to detect a cycle, then finds the cycle start. "
            "`main()` reads the array and prints the duplicate."
        )
    elif "Missing and Repeating Number" in title:
        logic = (
            "Using XOR, find the XOR of all array elements and numbers 1..n. The result is XOR of the missing and repeating "
            "numbers. Then separate based on a set bit and identify which is missing and which is repeating."
        )
        code_exp = (
            "`findMissingRepeating()` computes XOR, finds a set bit, separates elements into two groups, and XORs again. "
            "It then checks which value appears in the array to decide the repeating number. `main()` reads input and prints "
            "both numbers."
        )
    elif "Merge Overlapping Intervals" in title:
        logic = (
            "Sort intervals by start time. Then iterate, merging if the next interval's start is ≤ current end, otherwise "
            "adding the current interval to the result."
        )
        code_exp = (
            "`merge()` sorts intervals using `qsort`, then builds a result list by merging overlapping intervals. "
            "`main()` reads intervals, calls merge, and prints the merged intervals."
        )
    elif "Maximum Gap (Bucket Sort)" in title:
        logic = (
            "Bucket sort: place numbers into buckets based on their value, then the maximum gap will be between the max of "
            "one bucket and the min of the next non‑empty bucket."
        )
        code_exp = (
            "`maximumGap()` finds min and max, determines bucket size and count, places each element into its bucket, then "
            "scans buckets to compute the maximum gap. `main()` reads array and prints the gap."
        )
    elif "Find All Pairs with Given Sum (Unsorted)" in title:
        logic = (
            "Use a hash table to store the count of each element. For each element, check if the complement exists in the "
            "hash table and handle duplicates appropriately."
        )
        code_exp = (
            "The code implements a simple hash table with chaining. `main()` inserts all elements, then for each element "
            "checks the complement's count and prints pairs. A simple duplicate avoidance is included."
        )
    elif "3Sum Closest" in title:
        logic = (
            "Sort the array. For each element, use two pointers to find a sum closest to the target, updating the closest "
            "sum as you go."
        )
        code_exp = (
            "`threeSumClosest()` sorts the array, then for each i it uses two pointers to adjust the sum and track the "
            "closest. `main()` reads input and prints the closest sum."
        )
    elif "4Sum" in title:
        logic = (
            "Similar to 3Sum but with an extra nested loop. Sort the array, fix two numbers, and use two pointers to find "
            "the remaining two, skipping duplicates."
        )
        code_exp = (
            "`fourSum()` sorts the array, then uses two outer loops (with duplicate skipping) and two pointers inside. "
            "When a quadruplet is found, it prints it and moves pointers past duplicates. `main()` reads input and calls "
            "the function."
        )
    elif "Subarray Sum Equals K" in title:
        logic = (
            "Use a hash map to store the frequency of prefix sums. For each prefix sum, the number of subarrays ending at "
            "the current index with sum k is the frequency of (prefix - k)."
        )
        code_exp = (
            "`subarraySum()` maintains a hash table of prefix sums, initializing with 0. For each element, it updates the "
            "prefix, adds the count of (prefix - k) to the answer, and inserts the current prefix. `main()` reads input "
            "and prints the count."
        )
    elif "Subarray Sum Divisible by K" in title:
        logic = (
            "Similar to subarray sum equals k, but using modulo of the prefix sum. The remainder is adjusted to be positive, "
            "and the count of subarrays with same remainder is accumulated."
        )
        code_exp = (
            "`subarraysDivByK()` uses a hash table for remainders. It tracks the prefix sum modulo k, increments the count "
            "of that remainder, and adds the previous count of the same remainder to the answer. `main()` reads input and "
            "prints the number."
        )
    elif "Maximum Circular Subarray Sum" in title:
        logic = (
            "Compute the maximum subarray sum using Kadane, and also the minimum subarray sum. The circular maximum is the "
            "total sum minus the minimum subarray sum (if that yields a larger value)."
        )
        code_exp = (
            "`maxSubarraySumCircular()` uses Kadane for max and min, then returns the larger of the ordinary max and the "
            "circular max (total - min). `main()` reads array and prints the result."
        )
    elif "Find Peak Element (O(log n))" in title:
        logic = (
            "Binary search on the array: compare mid with its right neighbor. If mid < right, the peak lies to the right; "
            "otherwise, to the left (including mid)."
        )
        code_exp = (
            "`findPeakElement()` performs binary search narrowing down to a peak. `main()` reads array and prints the index "
            "and value of a peak."
        )
    elif "Majority Element II (n/3)" in title:
        logic = (
            "Extended Boyer‑Moore majority vote: maintain two candidates and their counts. After one pass, verify the counts "
            "to ensure they exceed n/3."
        )
        code_exp = (
            "`majorityElement()` runs the voting algorithm, then counts actual occurrences to determine which candidates "
            "are valid. `main()` reads input and prints the majority elements (if any)."
        )
    elif "Sort Colors (Dutch National Flag)" in title:
        logic = (
            "Three‑pointer approach: low for 0, mid for 1, high for 2. Move mid through the array, swapping with low or high "
            "as needed."
        )
        code_exp = (
            "`sortColors()` uses three pointers and swaps to place 0s at the beginning and 2s at the end. `main()` reads "
            "the array and prints the sorted result."
        )
    elif "Remove Duplicates from Sorted Array (II)" in title:
        logic = (
            "Two pointers: one to iterate, one to place the next allowed element. Allow at most two duplicates by comparing "
            "with the element two positions behind."
        )
        code_exp = (
            "`removeDuplicates()` uses index j to place elements. For each i, if nums[i] != nums[j-2], it copies the element. "
            "`main()` reads a sorted array and prints the new length and array."
        )
    elif "Find the Smallest Missing Positive (Version 2)" in title:
        logic = "Same as First Missing Positive but works with negative numbers by ignoring them."
        code_exp = "Same as E016."
    elif "Longest Consecutive Sequence" in title:
        logic = (
            "Sort the array and then scan, counting consecutive sequences. Alternatively, use a hash set for O(n) average, "
            "but here sorting is used for simplicity in C."
        )
        code_exp = (
            "`longestConsecutive()` sorts the array, then scans counting consecutive runs, updating the maximum length. "
            "`main()` reads input and prints the result."
        )
    elif "Gas Station (Circular Tour)" in title:
        logic = (
            "If total gas is less than total cost, no solution. Otherwise, find the starting station where the running total "
            "never drops below zero."
        )
        code_exp = (
            "`canCompleteCircuit()` accumulates total and current surplus. If current surplus becomes negative, it resets "
            "the start to the next station. `main()` reads gas and cost arrays and prints the starting index."
        )
    elif "Candy Distribution" in title:
        logic = (
            "Give each child 1 candy initially. Then two passes: left to right (if rating higher than left, give more), and "
            "right to left (if rating higher than right and current candy is not enough, update). Sum the candies."
        )
        code_exp = (
            "`candy()` allocates an array of candies, fills with 1, then does two passes as described, finally summing. "
            "`main()` reads ratings and prints the minimum candies."
        )
    elif "Jump Game (Minimum Jumps)" in title:
        logic = "Track the maximum reachable index. If at any point the current index exceeds the max reach, it's impossible."
        code_exp = (
            "`canJump()` iterates, updating maxReach. If i exceeds maxReach, return false. `main()` reads jumps and prints "
            "whether the last index is reachable."
        )
    elif "Jump Game II (Minimum Number of Jumps)" in title:
        logic = (
            "Use a BFS‑like approach: at each step, find the farthest we can reach within the current jump, and when we reach "
            "the end of the current range, increment jump count."
        )
        code_exp = (
            "`jump()` uses variables `curEnd` and `curFarthest`. For each position, update farthest, and when i reaches "
            "curEnd, increment jumps and set curEnd to curFarthest. `main()` reads array and prints minimum jumps."
        )
    elif "Best Time to Buy and Sell Stock with Cooldown" in title:
        logic = (
            "Dynamic programming with states: `sold` (profit after selling), `held` (profit after buying/holding), "
            "`cooled` (profit after cooldown). Transition based on prices."
        )
        code_exp = (
            "`maxProfit()` maintains three variables, updating them each day according to the recurrence. `main()` reads "
            "prices and prints the maximum profit."
        )
    elif "Word Search in 2D Grid" in title:
        logic = "DFS backtracking: try each cell as start, explore four directions, marking visited cells."
        code_exp = (
            "`exist()` loops over all cells and calls `dfs()` which recursively searches, marking cells with '#' to avoid "
            "reuse. `main()` reads board and word, then prints whether the word exists."
        )
    elif "Number of Islands" in title:
        logic = "DFS/BFS to traverse each connected component of '1's, marking them visited."
        code_exp = (
            "`numIslands()` loops over grid, when it finds a '1', increments count and calls `dfs()` to sink the island. "
            "`main()` reads grid and prints island count."
        )
    elif "Rotting Oranges" in title:
        logic = "Multi‑source BFS: enqueue all rotten oranges initially, then BFS level by level, rotting adjacent fresh oranges."
        code_exp = (
            "`orangesRotting()` counts fresh oranges, initializes queue with all rotten ones, then BFS while tracking minutes. "
            "`main()` reads grid and prints the minutes or -1 if impossible."
        )
    elif "Median of Row‑wise Sorted Matrix" in title:
        logic = (
            "Binary search on value range. For a candidate mid, count how many elements are ≤ mid. Adjust range until the "
            "count equals the desired median position."
        )
        code_exp = (
            "`findMedian()` finds global min and max, then binary searches, using `countLessEqual()` which counts elements ≤ mid "
            "in each row via binary search. `main()` reads matrix and prints median."
        )
    elif "Search in a Row‑wise and Column‑wise Sorted Matrix" in title:
        logic = (
            "Start from top‑right corner. If target is greater, move down; if smaller, move left. Continue until found or out "
            "of bounds."
        )
        code_exp = (
            "`searchMatrix()` implements the staircase search. `main()` reads matrix and target, then prints the coordinates "
            "if found."
        )
    elif "Kth Smallest Element in a Sorted Matrix" in title:
        logic = "Binary search on value range, similar to median search but looking for the k‑th smallest."
        code_exp = (
            "`kthSmallest()` performs binary search on value, using `countLessEqual()` to count elements ≤ mid, narrowing the "
            "range until it converges. `main()` reads matrix and k, prints the k‑th smallest."
        )
    elif "Spiral Matrix II" in title:
        logic = "Simulate filling the matrix in spiral order: maintain four boundaries and fill layer by layer."
        code_exp = (
            "`generateMatrix()` allocates matrix and fills using while loops that traverse right, down, left, up, shrinking "
            "boundaries after each direction. `main()` reads n and prints the generated spiral matrix."
        )
    elif "Matrix Chain Multiplication" in title:
        logic = (
            "Dynamic programming: `dp[i][j]` = minimum multiplications to multiply matrices from i to j. Try all possible "
            "split points and choose the minimum."
        )
        code_exp = (
            "`matrixChainOrder()` initializes dp, then for increasing chain lengths, it computes dp[i][j] by trying all k. "
            "`main()` reads dimensions and prints the minimum cost."
        )
    elif "Coin Change (Minimum Coins)" in title:
        logic = "Dynamic programming: `dp[amount]` = minimum coins needed. For each amount, try all coins and take the minimum."
        code_exp = (
            "`coinChange()` initializes dp array with large values, sets dp[0]=0, then iterates amounts and coins, updating. "
            "`main()` reads denominations and amount, prints the minimum coins or -1 if impossible."
        )
    elif "Implement Min‑Heap (Priority Queue)" in title:
        logic = (
            "A min‑heap is a complete binary tree where each node is smaller than its children. Insert: add at end and bubble up. "
            "ExtractMin: remove root, replace with last element, and heapify down."
        )
        code_exp = (
            "The code defines a `MinHeap` structure with array, size, capacity. `insert()` adds a value and bubbles up. "
            "`extractMin()` removes the root, places the last element at root, and heapifies down. `main()` demonstrates "
...
print(f"✅ Successfully created 'answers' folder with {len(c_files_content)} C files and 'expert-projects.json' (with explanations)")