...
import cCode from "./topic4_files/UnionsDemo.c?raw";
    ...
    <div className="space-y-12 bg-slate-900 text-slate-200 p-4 md:p-8 rounded-2xl border border-slate-800">
      ...
      <header className="space-y-3 border-b border-slate-800 pb-6">
        ...
        <h1 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight">
          Unions: Shared Memory Overlays & Type Punning
        </h1>
        <p className="text-slate-400 text-base max-w-4xl leading-relaxed">
          Master C unions where all member variables share the exact same starting memory address. Learn memory savings, variant data structures, and hardware type punning.
        </p>
      </header>
      ...
      <section className="space-y-4 bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
        <h2 className="text-2xl font-bold text-sky-300 flex items-center gap-2">
          <span>📖</span> Topic Description: Unions: Shared Memory Overlays & Type Punning
        </h2>
        <div className="space-y-3 text-slate-300 text-sm md:text-base leading-relaxed">
          <p>
            A Union allocates memory equal to its single largest member. All members start at offset 0, holding one active field at a time.
          </p>
          <div className="bg-slate-900/60 p-4 rounded-xl border-l-4 border-amber-500 text-xs md:text-sm text-slate-300 space-y-2">
            ...
            <p>
              Swadeep created a sensor payload union holding either an integer temperature or a float pressure. Sukanta Hui showed how unions reduce memory footprint to the largest member size.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800 overflow-x-auto">
          <svg viewBox="0 0 900 200" className="w-full min-w-[700px] font-sans">
            ...
            <text x="450" y="90" textAnchor="middle" fill="#38bdf8" className="font-bold text-base">
              Memory Layout Architecture: Unions: Shared Memory Overlays & Type Punning
            </text>
            <text x="450" y="120" textAnchor="middle" fill="#94a3b8" className="text-xs">
              Contiguous byte alignment in C memory space (Module 003_008 • Topic 4)
            </text>
          </svg>
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
          <div className="bg-slate-800/40 p-4 rounded-xl border border-slate-750 space-y-2">
            ...
            <p className="text-slate-300">A Union allocates memory equal to its single largest member. All members start at offset 0, holding one active field at a time.</p>
          </div>
          <div className="bg-slate-800/40 p-4 rounded-xl border border-slate-750 space-y-2">
      ...
      <section className="space-y-5 bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
        <h2 className="text-2xl font-bold text-emerald-400 flex items-center gap-2">
          <span>💻</span> Example Section: Unions: Shared Memory Overlays & Type Punning Demonstration
        </h2>
        <p className="text-slate-300 text-sm leading-relaxed">
          The program below (<code>UnionsDemo.c</code>) demonstrates a complete standalone C console application for Unions: Shared Memory Overlays & Type Punning.
        </p>

        <CFileLoader fileModule={cCode} title="UnionsDemo.c" editable={false} />
...
{`====================================================
  CODER & ACCOTAX - C UNIONS & MEMORY OVERLAY DEMO
  Center: Barrackpore | Educator: Sukanta Hui
====================================================

Union Size: 4 bytes (Shares memory across int, float, and char[4])

Setting Integer Value:  1065353216
Active Float View (Type Punning): 1.000000

Setting Float Value:    3.141593
Raw Hex Representation: 0x40490fdb`}
          </pre>
        </div>
      </section>
      ...
      <section>
        <FAQTemplate title="Module 003_008 Topic 4 FAQs" questions={questions} />
      </section>
      ...
      <section>
        <PlainTextPrint
          content={noteText}
          title="Module 003_008 Topic 4 Note"
          stampEnabled={true}
          showDownload={true}
          downloadButtonText="Download Printable Note"
          downloadFileName="module_003_008_topic4_note.txt"
        />
      </section>

      {/* 10. Teacher's Note Section */}
      <section>
        <Teacher note="Swadeep created a sensor payload union holding either an integer temperature or a float pressure. Sukanta Hui showed how unions reduce memory footprint to the largest member size. — Sukanta Hui" />
      </section>
    </div>
  );
}
