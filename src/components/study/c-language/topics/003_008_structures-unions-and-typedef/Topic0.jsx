...
import cCode from "./topic0_files/StructBasicsDemo.c?raw";
    ...
    <div className="space-y-12 bg-slate-900 text-slate-200 p-4 md:p-8 rounded-2xl border border-slate-800">
      ...
      <header className="space-y-3 border-b border-slate-800 pb-6">
        ...
        <h1 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight">
          Structure Declaration, Member Variables & Dot Operator
        </h1>
        <p className="text-slate-400 text-base max-w-4xl leading-relaxed">
          Understand user-defined heterogeneous data types in C. Learn structure declaration syntax, Designated Initializers, memory layouts, and dot (.) member access operator mechanics.
        </p>
      </header>
      ...
      <section className="space-y-4 bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
        <h2 className="text-2xl font-bold text-sky-300 flex items-center gap-2">
          <span>📖</span> Topic Description: Structure Declaration, Member Variables & Dot Operator
        </h2>
        <div className="space-y-3 text-slate-300 text-sm md:text-base leading-relaxed">
          <p>
            A C Structure (struct) groups variables of different data types (int, char array, float) into a contiguous block of memory.
          </p>
          <div className="bg-slate-900/60 p-4 rounded-xl border-l-4 border-amber-500 text-xs md:text-sm text-slate-300 space-y-2">
            ...
            <p>
              When Swadeep asked how to represent a complex student record with roll number, name, and marks without creating 3 separate arrays, Sukanta Hui introduced C structures as a single unified memory package.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800 overflow-x-auto">
          <svg viewBox="0 0 900 200" className="w-full min-w-[700px] font-sans">
            ...
            <text x="450" y="90" textAnchor="middle" fill="#38bdf8" className="font-bold text-base">
              Memory Layout Architecture: Structure Declaration, Member Variables & Dot Operator
            </text>
            <text x="450" y="120" textAnchor="middle" fill="#94a3b8" className="text-xs">
              Contiguous byte alignment in C memory space (Module 003_008 • Topic 0)
            </text>
          </svg>
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
          <div className="bg-slate-800/40 p-4 rounded-xl border border-slate-750 space-y-2">
            ...
            <p className="text-slate-300">A C Structure (struct) groups variables of different data types (int, char array, float) into a contiguous block of memory.</p>
          </div>
          <div className="bg-slate-800/40 p-4 rounded-xl border border-slate-750 space-y-2">
      ...
      <section className="space-y-5 bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
        <h2 className="text-2xl font-bold text-emerald-400 flex items-center gap-2">
          <span>💻</span> Example Section: Structure Declaration, Member Variables & Dot Operator Demonstration
        </h2>
        <p className="text-slate-300 text-sm leading-relaxed">
          The program below (<code>StructBasicsDemo.c</code>) demonstrates a complete standalone C console application for Structure Declaration, Member Variables & Dot Operator.
        </p>

        <CFileLoader fileModule={cCode} title="StructBasicsDemo.c" editable={false} />
...
Total Memory Size of struct Student: 64 bytes`}
          </pre>
        </div>
      </section>
      ...
      <section>
        <Teacher note="When Swadeep asked how to represent a complex student record with roll number, name, and marks without creating 3 separate arrays, Sukanta Hui introduced C structures as a single unified memory package. — Sukanta Hui" />
      </section>
    </div>
  );
}
