...
public class Segment1ComprehensiveMCQExamDemo {
    ...
    // Builds the Segment 1 Exam Bank using primitive arrays
    public static ExamQuestion[] buildSegment1QuestionBank() {
        return new ExamQuestion[] {
            new ExamQuestion(1, "001_001 JVM", "Where are local primitive variables stored in JVM memory?",
                new String[]{"Heap Memory", "Stack Frame", "Method Area", "Native Stack"}, 1,
                "Local variables and primitive values reside inside the thread's Stack Frame."),

            new ExamQuestion(2, "001_002 Datatypes", "What is the size of a primitive 'int' in Java?",
                new String[]{"16 bits", "32 bits", "64 bits", "Platform dependent"}, 1,
                "In Java, 'int' is guaranteed to be 32 bits signed two's complement across all platforms."),

            new ExamQuestion(3, "001_003 Casting", "What is the result of (int) 7.9 in Java?",
                new String[]{"8", "7", "7.9", "Compilation error"}, 1,
                "Explicit narrowing cast truncates the decimal portion, yielding 7."),

            new ExamQuestion(4, "001_004 Switch", "What avoids fall-through in Java 14+ switch expressions?",
                new String[]{"break keyword", "arrow syntax (->)", "continue keyword", "yield statement"}, 1,
                "The arrow syntax (case ->) executes only the targeted branch with zero fall-through."),

            new ExamQuestion(5, "001_005 Loops", "Which loop is guaranteed to execute at least ONCE?",
                new String[]{"for loop", "while loop", "do-while loop", "enhanced for-each"}, 2,
                "do-while evaluates its condition at the bottom, guaranteeing at least one execution."),

            new ExamQuestion(6, "001_006 Arrays", "What is the default value of elements in new boolean[5]?",
                new String[]{"true", "false", "null", "0"}, 1,
                "In Java, boolean arrays are initialized with false by default."),

            new ExamQuestion(7, "001_007 Methods", "How are object references passed to methods in Java?",
                new String[]{"Pass-by-reference", "Pass-by-value (copy of address reference)", "Pass-by-pointer", "Pass-by-name"}, 1,
                "Java is strictly Pass-by-Value; for objects, a copy of the reference address is passed.")
        };
    }

    // Evaluates student submissions
    public static StudentExamResult evaluateExam(String studentName, int[] studentAnswers, ExamQuestion[] questions) {
        int correctCount = 0;
        for (int i = 0; i < questions.length; i++) {
            if (i < studentAnswers.length && studentAnswers[i] == questions[i].correctOptionIndex()) {
                correctCount++;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        ExamQuestion[] examBank = buildSegment1QuestionBank();
        System.out.printf("Segment 1 Foundations Question Bank Loaded: %d Sample Synthesized Questions%n%n", examBank.length);
        ...
        System.out.println("4. Built purely with primitive arrays (String[], ExamQuestion[]) - no Collections needed!");
        System.out.println("5. In Topic 12, we conclude Segment 1 with the Timed Coding Assessment!");
        System.out.println("================================================================================");
    }
}
