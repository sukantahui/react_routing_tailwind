// src/components/study/JavaXModuleView.jsx
...
import roadmapData from "./icse-class-10-roadmap.json";

export default function JavaXModuleView() {
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-10">
        ...
        <section className="border border-slate-800 bg-slate-900/70 p-6 rounded-3xl shadow-lg">
          ...
          {lastTopicIndex !== null && (
            <Link
              ...
              className="inline-flex items-center gap-2 mt-4 text-sky-400 text-sm hover:underline"
            >
              Resume last topic <ChevronRight size={14} />
            </Link>
          )}
        </section>
        ...
        <section className="border border-slate-800 bg-slate-900/80 p-6 rounded-3xl shadow-lg">
          ...
          <p className="text-[11px] text-slate-500 mb-4">
            Progress: {completedCount}/{totalTopics} topics ({progressPercent}%)
          </p>
          ...
          <div className="space-y-3">
            {topics
              ...
              .map(({ topic, idx }) => {
                ...
                const isLast = idx === lastTopicIndex;
                return (
                  <div
                    key={idx}
                    ref={(el) => (topicRefs.current[idx] = el)}
                    className={`flex items-center gap-3 px-4 py-3 rounded-xl border text-sm ${done
