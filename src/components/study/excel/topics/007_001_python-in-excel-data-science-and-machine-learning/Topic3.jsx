    ...
    <div className="dark bg-slate-950 text-slate-100 min-h-screen py-8 px-4 sm:px-6 lg:px-8 font-sans selection:bg-sky-500/30 selection:text-sky-200">
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🐼 Data Wrangling with pandas in Excel"} · Topic 3
            </span>
          ...
          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Performing advanced enterprise data wrangling with pandas inside Excel: boolean filtering, multi-column grouping (groupby), relational joins (merge), reshaping (pivot_table, melt), and datetime conversions"}. Master native Python in Excel execution, pandas DataFrame wrangling, seaborn cell visualizations, statistical hypothesis testing, and scikit-learn predictive AI models.
          </p>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. Vectorized Boolean Filtering"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`df[(df['Zone'] == 'East') & (df['Margin'] > 0.15)]`. Executes conditional filtering across 500,000 rows in milliseconds."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Multi-Aggregation GroupBy Engine"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`df.groupby(['Category', 'Year']).agg({'Sales': 'sum', 'Cost': 'mean', 'ID': 'count'}).reset_index()`."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Relational Joins & Reshaping"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`pd.merge(dfSales, dfCustomers, on='CustID', how='left')` executes high-speed relational joins replacing complex multi-criteria XLOOKUPs."}</p>
            </div>
        ...
        >
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod19_3" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod19_3" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod19_3)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Data Wrangling & Manipulation"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod19_3)" fill="none" />
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Multi-Subsidiary Sales Consolidation & Grouping"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Groups 250,000 transaction rows by subsidiary, product category, and financial quarter, calculating weighted average margins in 3 lines."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Automated Relational Tax Ledger Join"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Executes inner join between purchase register and 2B tax ledger on invoice number and HSN code."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Sensor Telemetry Reshaping with Melt"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Unpivots 50 machine sensor columns into normalized key-value time-series format using `pd.melt()`."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Carrier Performance Pivot Table Engine"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Builds dynamic multi-index logistics pivot tables summarizing transit delays."}</p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Forgetting .reset_index() After GroupBy (Missing Column Names)"}</td>
                  <td className="py-3 px-4">{"Grouping without `.reset_index()` leaves grouping keys in the DataFrame index, causing missing headers when spilled."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always append `.reset_index()` to GroupBy operations: `df.groupby(...).sum().reset_index()`."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Merge Key Type Mismatch (String vs Integer)"}</td>
                  <td className="py-3 px-4">{"Joining tables where ID in Table A is integer `101` and in Table B is string `\"101\"` returns empty DataFrame."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Cast join keys: `df1['ID'] = df1['ID'].astype(str)`."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Chained Indexing Assignment Warning"}</td>
                  <td className="py-3 px-4">{"Modifying DataFrame slices with `df[df['A']>5]['B'] = 10`."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use `.loc`: `df.loc[df['A'] > 5, 'B'] = 10`."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Always Reset Index"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Guarantees clean column headers on output."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"df.groupby('Col').sum().reset_index()"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"High-Speed Join"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Replaces 20 complex VLOOKUP formulas in 1 line."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"pd.merge(df1, df2, on='Key', how='left')"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Sort Values"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Sorts DataFrame by column."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"df.sort_values(by='Revenue', ascending=False)"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why is appending `.reset_index()` essential after performing a `groupby()` operation in pandas?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"How does `pd.merge()` replace complex multi-condition XLOOKUP formulas in Excel?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on Python data structures, cloud execution mechanics, and statistical inference.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"Why must data types of join keys match exactly (e.g. string vs integer) before merging DataFrames?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on Python data structures, cloud execution mechanics, and statistical inference.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Data wrangling with pandas in Excel: Filtering, grouping, merging, and reshaping DataFrames - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"pandas is the king of data manipulation. In 3 lines of Python code, you can filter 100,000 rows, group by Region, calculate average margin, and merge with your customer database. Always add `.reset_index()` at the end!"}
          />
        </div>
      </div>
    </div>
