      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"📉 Linear & Multiple Regression with statsmodels"} · Topic 8
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Linear and multiple regression modeling with statsmodels in Excel"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Building econometric regression models and predictive coefficients in Excel using statsmodels.OLS: Ordinary Least Squares regression, R-squared, coefficient p-values, and multicollinearity diagnostics"}. Master native Python in Excel execution, pandas DataFrame wrangling, seaborn cell visualizations, statistical hypothesis testing, and scikit-learn predictive AI models.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Linear Regression: import statsmodels.api as sm; X = sm.add_constant(df[['AdSpend', 'Discount']]); y = df['Revenue']; model = sm.OLS(y, X).fit(); model.summary()"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The `sm.add_constant()` Requirement"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"statsmodels does NOT include an intercept by default; `sm.add_constant(X)` is mandatory to calculate the constant regression intercept (Beta 0)."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Comprehensive Econometric Diagnostics (`model.summary()`)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Computes R-squared, Adjusted R-squared, F-statistic, coefficient values, standard errors, t-statistics, p-values, and 95% confidence intervals."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Predictive Forecasting (`model.predict(new_X)`)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Generates projected future revenue scenarios based on multivariate input feature assumptions."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Multiple Regression Modeling Pipeline: Feature Matrix X -> add_constant -> sm.OLS(y, X).fit() -> R-squared & Coefficients Deck"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod19_8" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod19_8" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod19_8)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Econometric & Regression Modeling"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod19_8)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic8_Linear_and_multipl"
            title={"Linear and multiple regression modeling with statsmodels in Excel - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Multivariate Revenue Driver Econometric Model"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Builds multiple OLS regression model predicting monthly revenue from digital marketing spend, sales headcount, and economic indicators with R² = 0.89."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-amber-400">{"Case 2 · Lead FP&A Analyst"}</span>
                <span className="text-xs font-mono text-slate-400">{"Shyamnagar Plant"}</span>
              </div>
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Factory Fixed vs Variable Cost Separation"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses linear regression to separate fixed overhead costs (intercept) from variable per-unit manufacturing costs (slope)."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Furnace Energy Consumption Model"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Predicts electrical kilowatt consumption from ambient temperature and steel throughput tonnage."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Freight Route Cost Prediction Model"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Regresses shipment delivery cost against distance, fuel price, and consignment weight."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Ingest ranges: <code className="text-cyan-300 font-mono font-bold">{"Linear Regression: import statsmodels.api as sm; X = sm.add_constant(df[['AdSpend', 'Discount']]); y = df['Revenue']; model = sm.OLS(y, X).fit(); model.summary()"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Omitting sm.add_constant(X) (Zero Intercept Disaster)"}</td>
                  <td className="py-3 px-4">{"Forgetting `sm.add_constant()` forces the regression line through the origin (0,0), generating completely invalid R-squared and biased coefficients."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always wrap feature matrices: `X = sm.add_constant(X)`."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Multicollinearity Between Features (VIF > 10)"}</td>
                  <td className="py-3 px-4">{"Including two highly correlated independent variables (e.g. Total Revenue and Gross Sales) distorting coefficient p-values."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Check correlation matrix and remove redundant collinear features."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Passing Strings or Missing Values into OLS"}</td>
                  <td className="py-3 px-4">{"statsmodels requires clean numeric floats with zero NaN values."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Drop NaNs: `df.dropna(subset=['y', 'x1', 'x2'])` and encode categorical variables."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Add Constant Intercept"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Mandatory for accurate OLS regression modeling."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"X = sm.add_constant(X)"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Inspect model.summary()"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Generates full econometric report with R² and p-values."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"model.summary()"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Extract Coefficients"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Extracts individual slope and intercept weights."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"model.params"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why is calling `sm.add_constant(X)` mandatory before fitting an OLS regression model in statsmodels?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"What is the business interpretation of the R-squared metric in a sales forecasting model?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on Python data structures, cloud execution mechanics, and statistical inference.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"How do coefficient p-values indicate whether an advertising channel has a statistically significant impact on revenue?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on Python data structures, cloud execution mechanics, and statistical inference.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Linear and multiple regression modeling with statsmodels in Excel - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Linear regression is the ultimate tool for corporate forecasting. Always write `X = sm.add_constant(X)` before fitting your model! In 4 lines of Python, you can find out exactly how much every ₹1 spent on marketing increases your bottom-line revenue."}
          />
        </div>
      </div>
    </div>
