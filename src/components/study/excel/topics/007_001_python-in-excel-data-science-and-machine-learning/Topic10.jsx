      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🧹 Missing Data Imputation & Outlier Detection"} · Topic 10
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Handling missing data, imputation, and outlier detection with Python algorithms"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Engineering production-grade data hygiene pipelines in Python: missing value profiling (isnull), median/KNN imputation, IQR boxplot outlier filtering, and z-score anomaly detection"}. Master native Python in Excel execution, pandas DataFrame wrangling, seaborn cell visualizations, statistical hypothesis testing, and scikit-learn predictive AI models.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Data Cleaning: df.isnull().sum() | Imputation: df['Salary'].fillna(df['Salary'].median(), inplace=True) | Outliers: q1, q3 = df['Val'].quantile([0.25, 0.75]); iqr = q3 - q1"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. Comprehensive Missing Value Profiling"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`df.isnull().sum()` / `df.isnull().mean() * 100` identifies the exact missing data percentage per column across massive datasets."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Robust Imputation Strategies"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Replaces missing numerical values with median (resistant to extreme outliers) or mode for categorical features: `df['Income'].fillna(df['Income'].median())`."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Tukey's IQR Outlier Boundary Detection"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Calculates Interquartile Range (`IQR = Q3 - Q1`); points below `Q1 - 1.5*IQR` or above `Q3 + 1.5*IQR` are flagged as statistical outliers."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Data Hygiene & Outlier Cleaning Pipeline: Dirty Input Data -> Missing Value Imputation -> IQR / Z-Score Outlier Isolation -> Pristine Dataset"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod19_10" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod19_10" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod19_10)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Data Hygiene & Preprocessing"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod19_10)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic10_Handling_missing_d"
            title={"Handling missing data, imputation, and outlier detection with Python algorithms - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Enterprise Customer Ledger Cleansing Pipeline"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Cleanses 500,000 raw customer records, imputing missing postal codes and removing erroneous negative revenue outliers."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Automated Tax Ledger Data Sanitization"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Profiles and imputes missing vendor PAN numbers and flags anomalous tax deduction spikes using IQR."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Sensor Glitch & Drop-out Imputation"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Interpolates missing temperature readings caused by brief sensor network dropouts using `df.interpolate(method='time')`."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Freight Weight Outlier Detection"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Isolates impossible truck weight entries (e.g. 500,000 kg on a 10-ton vehicle) using Z-score filters."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Ingest ranges: <code className="text-cyan-300 font-mono font-bold">{"Data Cleaning: df.isnull().sum() | Imputation: df['Salary'].fillna(df['Salary'].median(), inplace=True) | Outliers: q1, q3 = df['Val'].quantile([0.25, 0.75]); iqr = q3 - q1"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Mean Imputation on Skewed Datasets (Distortion Trap)"}</td>
                  <td className="py-3 px-4">{"Using mean imputation when data contains extreme billionaire outliers shifts the central tendency artificially high."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always use median imputation (`.median()`) for skewed financial data."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Deleting All Rows with Any NaN (Massive Data Loss)"}</td>
                  <td className="py-3 px-4">{"Calling `df.dropna()` indiscriminately deleting 80% of valuable transaction records."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Impute missing features selectively or drop rows only when target label `y` is missing."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Overwriting Raw Data in Place Without Backup"}</td>
                  <td className="py-3 px-4">{"Mutating original DataFrame without preserving a clean raw copy."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Create a clean copy: `df_clean = df.copy()`."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Median Imputation"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Robust against extreme outlier distortion."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"df['Col'].fillna(df['Col'].median(), inplace=True)"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"IQR Outlier Filter"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"The standard Tukey boxplot outlier filter."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"df[(df['Val'] >= q1 - 1.5*iqr) & (df['Val'] <= q3 + 1.5*iqr)]"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Missing Percent Summary"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Instant percentage of missing values per column."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"df.isnull().mean() * 100"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why is median imputation vastly superior to mean imputation for skewed corporate financial data?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"How does Tukey's Interquartile Range (IQR) rule define statistical outliers?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on Python data structures, cloud execution mechanics, and statistical inference.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"Why is calling `df.dropna()` without checking missing percentages dangerous for enterprise datasets?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on Python data structures, cloud execution mechanics, and statistical inference.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Handling missing data, imputation, and outlier detection with Python algorithms - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Clean data equals accurate answers. If your data has blanks, do not delete the whole row! Fill numeric blanks with the median (`df['Col'].fillna(df['Col'].median())`). It protects your model from distortion."}
          />
        </div>
      </div>
    </div>
