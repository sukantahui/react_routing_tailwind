      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🛡️ Parameterized SQL & SQL Injection Defense"} · Topic 4
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Parameterized SQL queries in VBA to prevent SQL injection and handle dynamic user inputs"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Defending enterprise databases against SQL Injection attacks using ADODB Parameterized Commands: prepared statements, type-safe parameter binding, and dynamic user inputs"}. Master ADODB relational database connectivity, high-speed CopyFromRecordset pipelines, closed Excel SQL querying, REST APIs, and ACID transactions.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Parameterized Query: Set cmd = CreateObject(\"ADODB.Command\") | cmd.CommandText = \"SELECT * FROM Sales WHERE Region = ? AND Year = ?\" | cmd.Parameters.Append cmd.CreateParameter(..., adVarChar, adParamInput, 50, \"East\")"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The SQL Injection Vulnerability"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Concatenating user text directly into SQL strings (`\"WHERE Customer = '\" & txtName.Text & \"'\"`) allows attackers to inject malicious SQL (`' OR '1'='1'; DROP TABLE Customers;--`)."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. The `ADODB.Command` Prepared Statement"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Separates SQL code logic from data literals. SQL statements use `?` placeholders; parameters are passed as strongly-typed binary data objects."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Type-Safe Parameter Binding"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`cmd.CreateParameter(Name, Type, Direction, Size, Value)`. Enforces strict type boundaries (e.g. integer parameters cannot contain rogue SQL text)."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"SQL Injection Defense Architecture: Raw Concatenation (Vulnerable) vs Parameterized Pre-Compiled Query (Secure)"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod17_4" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod17_4" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod17_4)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Database Security & Defense"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod17_4)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic4_Parameterized_SQL_"
            title={"Parameterized SQL queries in VBA to prevent SQL injection and handle dynamic user inputs - Interactive Practice Grid"}
            rowsPerPage={10}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-amber-400">{"Case 1 · Chief Security & Risk Officer"}</span>
                <span className="text-xs font-mono text-slate-400">{"Barrackpore HQ"}</span>
              </div>
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Enterprise Commercial Data Room Query Hardening"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Hardens 30 financial search portals against SQL Injection by refactoring string concatenation into `ADODB.Command` parameterized queries."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Secure Multi-Criteria Tax Voucher Filter"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Passes user-selected date windows and GSTIN numbers securely via typed ADO parameters."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Shopfloor Machine Log Search Tool"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Secures machine telemetry lookup queries against rogue operator inputs."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Waybill Search Parameterization"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Parameterizes driver license lookups across central transport databases."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Open database socket or send HTTP request with custom headers: <code className="text-cyan-300 font-mono font-bold">{"Parameterized Query: Set cmd = CreateObject(\"ADODB.Command\") | cmd.CommandText = \"SELECT * FROM Sales WHERE Region = ? AND Year = ?\" | cmd.Parameters.Append cmd.CreateParameter(..., adVarChar, adParamInput, 50, \"East\")"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Concatenating User Input Directly into SQL (Fatal Security Vulnerability)"}</td>
                  <td className="py-3 px-4">{"Writing `sql = \"SELECT * FROM Users WHERE User='\" & txtUser.Text & \"'\"` leaves systems 100% exposed to catastrophic SQL injection data deletion."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always use `ADODB.Command` with `?` placeholders and parameters."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Parameter Data Type Mismatch (adInteger vs adVarChar)"}</td>
                  <td className="py-3 px-4">{"Passing string text into an integer parameter causes ADO to reject the execution."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Match ADO parameter data types (`adVarChar`, `adInteger`, `adDate`, `adDouble`) to database column schema."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Missing Parameter Size on Variable Strings"}</td>
                  <td className="py-3 px-4">{"Omitting the size parameter on `adVarChar` throws runtime error."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Specify maximum string length: `cmd.CreateParameter(..., adVarChar, adParamInput, 100, strVal)`."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Use ? Placeholders"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Separates SQL logic from dynamic user inputs."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"CommandText = \"SELECT * FROM Table WHERE ID = ?\""}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"CreateParameter Syntax"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Type-safe parameter constructor."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"cmd.CreateParameter(\"p1\", adVarChar, adParamInput, 50, val)"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Execute to Recordset"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Executes prepared statement and returns secure recordset."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Set rs = cmd.Execute()"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why is concatenating user TextBox strings directly into SQL statements a major security vulnerability?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"How do parameterized queries with `?` placeholders prevent SQL Injection attacks?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the database connection architecture and cloud integration principles.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"What are the benefits of type-safe parameter binding in enterprise VBA systems?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the database connection architecture and cloud integration principles.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Parameterized SQL queries in VBA to prevent SQL injection and handle dynamic user inputs - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Never concatenate user input directly into SQL strings! If a user types `' OR '1'='1`, your macro could expose your entire database. Use `ADODB.Command` with `?` placeholders for bulletproof enterprise security."}
          />
        </div>
      </div>
    </div>
