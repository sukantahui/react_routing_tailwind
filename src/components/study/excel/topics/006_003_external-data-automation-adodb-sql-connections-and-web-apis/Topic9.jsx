      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"⚖️ Database Transactions: BeginTrans & RollbackTrans"} · Topic 9
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"ACID Database Transaction Governance"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Managing database transactions in VBA (`BeginTrans`, `CommitTrans`, `RollbackTrans`)"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Implementing ACID database transactions in Excel VBA using BeginTrans, CommitTrans, and RollbackTrans: ensuring multi-step updates succeed atomically or roll back completely on failure"}. Master ADODB relational database connectivity, high-speed CopyFromRecordset pipelines, closed Excel SQL querying, REST APIs, and ACID transactions.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Database Transactions: conn.BeginTrans | On Error GoTo TransError | conn.Execute sql1 | conn.Execute sql2 | conn.CommitTrans | TransError: conn.RollbackTrans"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The ACID Principle in Spreadsheets"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Atomicity (All operations succeed or none do); Consistency (Database remains in a valid state); Isolation (Concurrent operations don't interfere); Durability (Committed data survives crashes)."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. The `conn.BeginTrans` Lock"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Marks the beginning of an atomic transaction. All subsequent INSERT/UPDATE statements are held in temporary staging by the database engine."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. The `conn.RollbackTrans` Safeguard"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"If any error occurs during a 10-step billing update, `RollbackTrans` completely undoes all previous steps, leaving the database 100% pristine and uncorrupted."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Database Transaction ACID Pipeline: BeginTrans -> Multiple SQL Operations -> Commit (Success) / Rollback (Error)"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod17_9" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod17_9" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod17_9)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"ACID Database Transaction Governance"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod17_9)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic9_Managing_database_"
            title={"Managing database transactions in VBA (`BeginTrans`, `CommitTrans`, `RollbackTrans`) - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Enterprise Two-Sided Bank Journal Posting"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Wraps debit and credit journal postings in `conn.BeginTrans`: if debit succeeds but credit fails, `RollbackTrans` reverses the debit instantly, preventing financial imbalance."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Multi-Table Tax Invoice & Ledger Commit"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Ensures invoice header and 50 tax item detail rows commit atomically to MySQL."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Finished Goods & Raw Material Deduction Transaction"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Decrements scrap steel inventory while adding finished billet counts atomically."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Waybill Dispatch & Fuel Allocation Transaction"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Commits truck dispatch orders and fuel voucher deductions as a single atomic transaction."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Open database socket or send HTTP request with custom headers: <code className="text-cyan-300 font-mono font-bold">{"Database Transactions: conn.BeginTrans | On Error GoTo TransError | conn.Execute sql1 | conn.Execute sql2 | conn.CommitTrans | TransError: conn.RollbackTrans"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Omitting RollbackTrans on Error (Database Lockup)"}</td>
                  <td className="py-3 px-4">{"Error occurs inside a transaction without calling `RollbackTrans`, leaving table rows locked on the database server."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always place `conn.RollbackTrans` inside the structured error handler block."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Long-Running Transactions Holding Locks"}</td>
                  <td className="py-3 px-4">{"Pausing a transaction with a `MsgBox` while holding active database table locks blocks other corporate users."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Perform all user confirmations BEFORE calling `conn.BeginTrans`; execute SQL in milliseconds and commit immediately."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Calling CommitTrans When BeginTrans Failed"}</td>
                  <td className="py-3 px-4">{"Calling `CommitTrans` on an uninitialized transaction throws runtime error."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Track transaction state with a Boolean flag (`inTransaction = True`)."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Transaction Pattern"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"The universal ACID transaction template."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"conn.BeginTrans -> SQLs -> conn.CommitTrans (ErrorHandler: conn.RollbackTrans)"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"No UI Inside Transactions"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Keeps transaction duration under 50 milliseconds to prevent lock contention."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Never put MsgBox inside BeginTrans...CommitTrans"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Atomicity Guarantee"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Eliminates half-completed corrupted database records."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"All succeed or all fail together"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why are database transactions (`BeginTrans` / `CommitTrans` / `RollbackTrans`) essential when posting two-sided accounting entries?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"What happens if an error occurs mid-way through a 5-step update without calling `RollbackTrans`?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the database connection architecture and cloud integration principles.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"Why must you never display a `MsgBox` prompt while inside an active database transaction?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the database connection architecture and cloud integration principles.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Managing database transactions in VBA (`BeginTrans`, `CommitTrans`, `RollbackTrans`) - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Transactions are the mark of a true software engineer. When you update a database, always wrap it: `conn.BeginTrans`. If everything works, `conn.CommitTrans`. If anything fails, `conn.RollbackTrans`. Your data will NEVER be corrupted."}
          />
        </div>
      </div>
    </div>
