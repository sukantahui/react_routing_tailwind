...
import sampleWorkbookUrl from "./excel_files/external_data_automation_adodb_sql_connections_and_web_apis_master.xlsx?url";
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🏆 Two-Way MySQL Database Synchronization Project"} · Topic 11
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Two-Way Database Sync Capstone"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Real-world project: Building an automated two-way synchronization tool between Excel and a MySQL Server"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Comprehensive enterprise capstone project: building an automated Two-Way Synchronization Tool between Excel and a remote MySQL server with dirty-row tracking, transactional batch updates, and conflict resolution"}. Master ADODB relational database connectivity, high-speed CopyFromRecordset pipelines, closed Excel SQL querying, REST APIs, and ACID transactions.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Two-Way MySQL Sync Engine: Read MySQL -> CopyFromRecordset Staging -> Highlight Modified Rows -> Batch UPDATE MySQL via Transactions"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. Two-Way Bidirectional Synchronization"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Step 1: Pulls latest records from MySQL into an interactive Excel grid; Step 2: Tracks user edits using `Worksheet_Change` (flags 'dirty' rows in yellow); Step 3: Writes modified rows back to MySQL in an atomic transaction."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Optimistic Concurrency & Conflict Detection"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Compares record version timestamps before write-back to ensure another user did not modify the record concurrently on the server."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. High-Speed Batch Sync"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Processes 5,000 modified records in under 1 second with full rollback protection."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Two-Way Database Synchronization Architecture: MySQL Server <-> ADODB Stream <-> Excel Interactive Grid <-> Transactional Write-Back"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod17_11" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod17_11" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod17_11)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Two-Way Database Sync Capstone"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod17_11)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            title={"Real-world project: Building an automated two-way synchronization tool between Excel and a MySQL Server - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Enterprise Pricing Master Two-Way Sync Tool"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Builds a two-way synchronization spreadsheet allowing regional sales directors to update product prices in Excel and sync changes back to the production MySQL database in 1 click."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Vendor Compliance Two-Way Management Portal"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Synchronizes vendor GST audit statuses between Excel audit workbooks and central MySQL ERP databases."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Machine Maintenance Scheduling Two-Way Sync"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Allows plant engineers to update machine servicing schedules in Excel and sync to factory database."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Carrier Freight Rate Two-Way Sync Portal"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Synchronizes negotiated transport rate tariffs between spreadsheet grids and logistics databases."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Open database socket or send HTTP request with custom headers: <code className="text-cyan-300 font-mono font-bold">{"Two-Way MySQL Sync Engine: Read MySQL -> CopyFromRecordset Staging -> Highlight Modified Rows -> Batch UPDATE MySQL via Transactions"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Updating All 10,000 Rows Instead of Only Modified Rows"}</td>
                  <td className="py-3 px-4">{"Executing SQL UPDATE statements for every single row in the table, even unchanged rows, causing massive network lag."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use a 'Dirty Flag' column (or `Worksheet_Change` event) to track and update ONLY modified rows."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Syncing Without Primary Key Matching"}</td>
                  <td className="py-3 px-4">{"Attempting to update MySQL without passing the unique primary key `ID` in the `WHERE` clause."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always store and match the immutable database primary key in a hidden column."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Missing Rollback on Partial Sync Failure"}</td>
                  <td className="py-3 px-4">{"Writing 50 rows but crashing on row 51 without rollback, leaving database partially updated."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Wrap the entire sync loop inside `conn.BeginTrans ... conn.CommitTrans`."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Dirty Row Flag"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Slashes sync time by 98% by updating only changed records."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Flag modified rows with color / boolean"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Transactional Write-Back"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Guarantees 100% database integrity on network hiccups."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Wrap sync in BeginTrans...CommitTrans"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"1-Click Sync Button"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Delivers an intuitive software experience for business users."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Assign Macro to 'Sync with MySQL' button"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"How does a 'Dirty Row Flag' allow this two-way sync tool to update 50 changed records out of 10,000 in milliseconds?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"Why is optimistic concurrency conflict detection essential when multiple team members edit database records simultaneously?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the database connection architecture and cloud integration principles.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"How does wrapping the sync loop in `BeginTrans` protect the database if the internet disconnects mid-way through updating?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the database connection architecture and cloud integration principles.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Real-world project: Building an automated two-way synchronization tool between Excel and a MySQL Server - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Two-way database sync is the Holy Grail of spreadsheet development. Users get the familiar Excel interface they love, while IT gets the centralized MySQL database they require. You have built a true enterprise application."}
          />
        </div>
      </div>
    </div>
