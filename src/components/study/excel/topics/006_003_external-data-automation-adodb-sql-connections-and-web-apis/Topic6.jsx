      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🌐 Consuming REST APIs with MSXML2.ServerXMLHTTP"} · Topic 6
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Cloud Integration & REST APIs"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Consuming external REST APIs and JSON endpoints using `MSXML2.ServerXMLHTTP` in VBA"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Connecting Excel VBA to modern Cloud Web Services and REST APIs using MSXML2.ServerXMLHTTP: HTTP GET/POST methods, custom request headers (API Keys, Bearer Tokens), and capturing JSON payloads"}. Master ADODB relational database connectivity, high-speed CopyFromRecordset pipelines, closed Excel SQL querying, REST APIs, and ACID transactions.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"REST API Call: Set http = CreateObject(\"MSXML2.ServerXMLHTTP.6.0\") | http.Open \"GET\", url, False | http.setRequestHeader \"Authorization\", \"Bearer key\" | http.Send | jsonResponse = http.responseText"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The ServerXMLHTTP Communication Engine"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Native Windows HTTP client executing high-performance, non-cached synchronous and asynchronous HTTPS requests directly to web endpoints."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. HTTP Request Headers & Authentication"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Passes authentication tokens: `http.setRequestHeader \"Authorization\", \"Bearer \" & API_KEY` and `http.setRequestHeader \"Content-Type\", \"application/json\"`."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Status Code Validation"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Checks `http.Status = 200` (Success / OK), gracefully intercepting HTTP 401 (Unauthorized), 404 (Not Found), or 500 (Server Error)."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"REST API Integration Architecture: Excel VBA Client -> HTTPS Request (Headers/Payload) -> Web Server -> JSON Response"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod17_6" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod17_6" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod17_6)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Cloud Integration & REST APIs"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod17_6)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic6_Consuming_external"
            title={"Consuming external REST APIs and JSON endpoints using `MSXML2.ServerXMLHTTP` in VBA - Interactive Practice Grid"}
            rowsPerPage={10}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-amber-400">{"Case 1 · Chief Commercial Strategist"}</span>
                <span className="text-xs font-mono text-slate-400">{"Barrackpore HQ"}</span>
              </div>
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Automated Live Currency & Forex Rate Ingestion"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Fetches live daily USD/INR, EUR/INR, and GBP/INR exchange rates from open currency REST APIs using `ServerXMLHTTP` in 0.3 seconds."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Automated Government GST Portal API Validation"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Queries government tax APIs to verify active GSTIN registration statuses directly from Excel."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Cloud Weather & Humidity Telemetry Fetcher"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Fetches regional barometric weather data from REST APIs to calibrate foundry furnace drying times."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Live GPS Fleet Vehicle Location Streamer"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Fetches real-time truck GPS coordinates from telematics APIs."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Open database socket or send HTTP request with custom headers: <code className="text-cyan-300 font-mono font-bold">{"REST API Call: Set http = CreateObject(\"MSXML2.ServerXMLHTTP.6.0\") | http.Open \"GET\", url, False | http.setRequestHeader \"Authorization\", \"Bearer key\" | http.Send | jsonResponse = http.responseText"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Using MSXML2.XMLHTTP Instead of ServerXMLHTTP (Caching Bug)"}</td>
                  <td className="py-3 px-4">{"`XMLHTTP` caches responses in Internet Explorer cache, returning stale exchange rates rather than fresh data."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always use `MSXML2.ServerXMLHTTP.6.0` to guarantee fresh, non-cached server responses."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Freezing Excel on Slow / Timed-Out Web Servers"}</td>
                  <td className="py-3 px-4">{"Synchronous HTTP requests hanging indefinitely if external API server is down."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Configure explicit timeouts: `http.setTimeouts 5000, 5000, 10000, 10000` (Resolve, Connect, Send, Receive in milliseconds)."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Exposing Production API Keys in Unlocked Workbooks"}</td>
                  <td className="py-3 px-4">{"Hardcoding proprietary API keys in exposed VBA modules."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Lock VBA Project with password protection."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Use ServerXMLHTTP.6.0"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Enterprise-grade HTTP client with zero caching issues."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Set http = CreateObject(\"MSXML2.ServerXMLHTTP.6.0\")"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Set Timeouts"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Prevents Excel from freezing on broken web servers."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"http.setTimeouts 5000, 5000, 5000, 5000"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Check Status 200"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Verifies HTTP success before attempting to parse response."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"If http.Status = 200 Then ..."}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why is `MSXML2.ServerXMLHTTP.6.0` preferred over `MSXML2.XMLHTTP` for calling financial web APIs?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"How do you set request headers to pass API authentication keys in VBA?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the database connection architecture and cloud integration principles.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"Why must you configure explicit timeouts on HTTP requests in enterprise spreadsheet models?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the database connection architecture and cloud integration principles.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Consuming external REST APIs and JSON endpoints using `MSXML2.ServerXMLHTTP` in VBA - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Excel can talk to the internet! Use `MSXML2.ServerXMLHTTP.6.0` to fetch live stock prices, weather data, or currency rates from any REST API in the world. It takes 5 lines of code."}
          />
        </div>
      </div>
    </div>
