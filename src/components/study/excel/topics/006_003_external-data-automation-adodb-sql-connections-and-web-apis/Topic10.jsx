      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🔐 Database Credential Security & Best Practices"} · Topic 10
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Security best practices: Managing database credentials securely in VBA"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Managing database credentials securely in Excel VBA: eliminating hardcoded plaintext passwords, leveraging Windows Integrated Authentication, DPAPI credential storage, and password vaulting"}. Master ADODB relational database connectivity, high-speed CopyFromRecordset pipelines, closed Excel SQL querying, REST APIs, and ACID transactions.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Security Architecture: Windows Integrated Auth | Secure Vault API | Storing Encrypted Credentials in Registry / Hidden Worksheets"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The Plaintext Hardcoding Danger"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Hardcoding database passwords (`Uid=admin;Pwd=Secret123;`) inside VBA modules is a severe security vulnerability that violates SOC-2 and ISO-27001 compliance standards."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Windows Integrated Authentication (SSPI / Trusted)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Leverages the active Windows user login credentials (`Trusted_Connection=Yes;` or `Integrated Security=SSPI;`) to authenticate directly with SQL Server without passwords."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Encrypted Credential Storage (DPAPI / Registry)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Stores encrypted connection credentials in secure Windows Credential Manager or protected user registry keys."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Database Security & Credential Management: Plaintext Exposure (High Risk) vs Windows Integrated Auth / DPAPI Vault (Secure)"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod17_10" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod17_10" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod17_10)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Enterprise Credential Security"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod17_10)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic10_Security_best_prac"
            title={"Security best practices: Managing database credentials securely in VBA - Interactive Practice Grid"}
            rowsPerPage={10}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-amber-400">{"Case 1 · Chief Information Security Officer"}</span>
                <span className="text-xs font-mono text-slate-400">{"Barrackpore HQ"}</span>
              </div>
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Enterprise Database Security Hardening"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Audits and purges all plaintext passwords from 120 corporate financial workbooks, converting them to Windows Active Directory Trusted Authentication."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Encrypted User Role Authentication"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Prompts tax auditors for encrypted database credentials on launch without saving passwords to disk."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Machine Operator Role-Based SQL Access"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Restricts shopfloor database write permissions using dedicated read-only database service accounts."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Secure Fleet DB API Token Vault"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Retrieves API keys dynamically from secure environment variables (`Environ(\"CORP_API_KEY\")`)."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Open database socket or send HTTP request with custom headers: <code className="text-cyan-300 font-mono font-bold">{"Security Architecture: Windows Integrated Auth | Secure Vault API | Storing Encrypted Credentials in Registry / Hidden Worksheets"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Committing Plaintext Passwords to Shared Network Drives"}</td>
                  <td className="py-3 px-4">{"Saving an `.xlsm` file with master database administrative credentials in a shared public folder."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Never store plaintext credentials in code; use Windows Authentication or prompt dynamically."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Using 'sa' or Root Administrative Accounts in Spreadsheets"}</td>
                  <td className="py-3 px-4">{"Connecting Excel using the database 'root' superuser account."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Create dedicated restricted database users with `SELECT` and `INSERT` permissions only on specific tables."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Leaving VBA Project Unprotected with Stored Strings"}</td>
                  <td className="py-3 px-4">{"Leaving connection strings readable in unlocked VBE code windows."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Lock the VBA Project with a strong password (Tools > VBAProject Properties > Protection)."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Trusted Connection Standard"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Zero hardcoded passwords; uses Windows Active Directory login."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Trusted_Connection=Yes; (SQL Server)"}</kbd>
            </div>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              ...
              <p className="text-slate-300 leading-relaxed">{"Limits blast radius if workbook is compromised."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Grant SELECT only to spreadsheet user"}</kbd>
            </div>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              ...
              <p className="text-slate-300 leading-relaxed">{"Stores secrets outside the workbook file."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"apiKey = Environ(\"CORP_API_KEY\")"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why is hardcoding database passwords inside VBA code considered a critical corporate security violation?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"How does Windows Integrated Authentication (`Trusted_Connection=Yes`) authenticate users without storing passwords?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the database connection architecture and cloud integration principles.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"What is the Principle of Least Privilege when creating database accounts for spreadsheet automation tools?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the database connection architecture and cloud integration principles.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Security best practices: Managing database credentials securely in VBA - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Never write passwords in your VBA code! If you are connecting to SQL Server, use `Trusted_Connection=Yes;` so it uses your Windows login. If you must use passwords, prompt the user or load it from an environment variable."}
          />
        </div>
      </div>
    </div>
