      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"📜 Parsing JSON Responses in VBA"} · Topic 7
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Modern Web Data Parsing"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Parsing JSON responses in VBA using lightweight parser classes"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Parsing complex hierarchical JSON payloads returned by REST APIs in VBA: converting JSON objects to Dictionaries, JSON arrays to Collections, and extracting nested attributes"}. Master ADODB relational database connectivity, high-speed CopyFromRecordset pipelines, closed Excel SQL querying, REST APIs, and ACID transactions.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"JSON Parsing: Set json = JsonConverter.ParseJson(http.responseText) | Val = json(\"rates\")(\"INR\") | Loop: For Each item In json(\"data\"): ... : Next item"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The JSON-to-VBA Object Mapping"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"JSON Objects (`{ \"key\": \"value\" }`) map to `Scripting.Dictionary`; JSON Arrays (`[ 1, 2, 3 ]`) map to VBA `Collection` objects."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Accessing Nested JSON Attributes"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Traverses nested hierarchies intuitively: `dblRate = json(\"data\")(\"rates\")(\"INR\")`."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Iterating JSON Array Collections"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Loops through lists of objects: `For Each item In json(\"items\"): ws.Cells(r, 1).Value = item(\"name\"): r = r + 1: Next item`."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"JSON Parsing Architecture: Raw JSON String -> Recursive Lexer / Parser -> VBA Dictionary & Collection Tree"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod17_7" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod17_7" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod17_7)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Modern Web Data Parsing"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod17_7)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic7_Parsing_JSON_respo"
            title={"Parsing JSON responses in VBA using lightweight parser classes - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Live Currency JSON Payload Extraction"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Parses currency JSON payload `{\"base\":\"USD\",\"rates\":{\"INR\":86.42,\"EUR\":0.92}}` and populates treasury exchange rate tables."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: GST Portal Response JSON Parser"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Parses taxpayer profile JSON responses extracting legal business name, status, and registration date."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Telemetry JSON Array Extraction"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Iterates through an array of 500 sensor reading objects in a single JSON API response."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Fleet Telematics JSON Parsing"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Extracts vehicle latitude, longitude, and speed from telematics API streams."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Open database socket or send HTTP request with custom headers: <code className="text-cyan-300 font-mono font-bold">{"JSON Parsing: Set json = JsonConverter.ParseJson(http.responseText) | Val = json(\"rates\")(\"INR\") | Loop: For Each item In json(\"data\"): ... : Next item"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Dictionary vs Collection Indexing Confusion"}</td>
                  <td className="py-3 px-4">{"Attempting to access a Dictionary with numeric index `json(1)` or a Collection with a string key `json(\"name\")` throws runtime error."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use string keys `dict(\"key\")` for Objects `{}`; use numeric index or `For Each` for Arrays `[]`."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Missing Scripting Runtime Reference for Dictionaries"}</td>
                  <td className="py-3 px-4">{"JSON parsers relying on `Scripting.Dictionary` failing if reference is missing."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Enable `Microsoft Scripting Runtime` or use late-bound Dictionary objects."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Crashing on Null or Missing JSON Keys"}</td>
                  <td className="py-3 px-4">{"Accessing a key that does not exist in the API response."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Check if key exists: `If json.Exists(\"key\") Then ...`."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"VBA-JSON Library Standard"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"The open-source gold standard for JSON parsing in VBA."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"JsonConverter.ParseJson(responseText)"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Nested Key Access"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Traverses nested JSON trees seamlessly."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"json(\"level1\")(\"level2\")(\"field\")"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Array Loop Pattern"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Dumps JSON arrays directly into worksheet rows."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"For Each rowItem In json(\"records\") ... Next"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"How do JSON Objects `{}` and JSON Arrays `[]` map into VBA Dictionaries and Collections?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"How do you extract a nested value like `{\"user\": {\"profile\": {\"email\": \"a@b.com\"}}}` in VBA?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the database connection architecture and cloud integration principles.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"Why is structured JSON parsing vastly superior to crude string manipulation with `InStr` and `Mid`?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the database connection architecture and cloud integration principles.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Parsing JSON responses in VBA using lightweight parser classes - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Modern APIs return data in JSON format. With a lightweight JSON parser class, you can parse any API response into a VBA Dictionary in 1 line: `Set json = ParseJson(http.responseText)`. Then just write `ws.Range(\"A1\").Value = json(\"rates\")(\"INR\")`."}
          />
        </div>
      </div>
    </div>
