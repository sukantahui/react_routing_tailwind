...
import sampleWorkbookUrl from "./excel_files/vba_and_macro_automation_master.xlsx?url";
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🔁 Looping Constructs: For, For Each & Do While"} · Topic 8
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Iteration & Repetitive Automation"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Looping constructs: For...Next, For Each...Next (Looping over sheets, cells, shapes), and Do While/Until"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Mastering iteration and looping algorithms in VBA: For...Next counter loops, For Each...Next collection loops (sheets, cells, shapes), Do While/Until, and loop early-exit strategies (Exit For)"}. Master the complete VBA language architecture, object model hierarchy, dynamic range operations, loops, performance switches, and defensive error handling.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Looping Constructs: For i = 1 To lastRow ... Next i | For Each ws In ThisWorkbook.Worksheets ... Next ws | Do While Condition ... Loop"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. For...Next (Indexed Counter Loops)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Loops from a start index to an end index with optional `Step` (`For i = 2 To lastRow Step 1`). Perfect for processing rows in a table."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. For Each...Next (Collection Iteration)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Iterates through every object in an Excel collection (e.g. `For Each ws In ThisWorkbook.Worksheets`, `For Each c In Range(\"A1:A100\")`). Cleanest and fastest way to loop through sheets."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Do While / Do Until (Condition-Driven)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Continues looping until a specific business condition is met (e.g. reading lines from an external file until End-of-File `EOF`)."}</p>
            </div>
            
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"4. Looping Backwards for Row Deletion"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"When deleting rows based on a condition, you MUST loop backwards: `For i = lastRow To 2 Step -1` to prevent row index skipping."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"VBA Looping Architecture: Counter Loops vs Collection Loops (Worksheets/Ranges)"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod15_8" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod15_8" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod15_8)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Iteration & Repetitive Automation"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod15_8)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic8_Looping_constructs"
            title={"Looping constructs: For...Next, For Each...Next (Looping over sheets, cells, shapes), and Do While/Until - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Batch Processing All Worksheets in Model"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses `For Each ws In ThisWorkbook.Worksheets: ws.Range(\"A1\").Value = \"CONFIDENTIAL\": Next ws` to watermark 30 tabs in 0.1 seconds."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Deleting Blank & Zero Tax Rows Backwards"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses `For i = lastRow To 2 Step -1: If ws.Cells(i, 5).Value = 0 Then ws.Rows(i).Delete: Next i` to clean 50,000 ledger rows."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Sensor Telemetry Matrix Row Scanner"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Scans 100,000 temperature rows with a For-Next loop, logging exceedance timestamps."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Waybill Batch Number Processor"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Iterates through shipment manifest collections updating transit status codes."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Place <code className="text-cyan-300 font-mono font-bold">Option Explicit</code> at the top and declare variables with explicit types <code className="text-cyan-300 font-mono font-bold">{"Looping Constructs: For i = 1 To lastRow ... Next i | For Each ws In ThisWorkbook.Worksheets ... Next ws | Do While Condition ... Loop"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Looping Forward When Deleting Rows (The Row Skipping Bug)"}</td>
                  <td className="py-3 px-4">{"Looping `For i = 2 To lastRow` and deleting row 5 shifts row 6 into row 5 position; the next loop evaluates row 6 (skipping the new row 5!)."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always loop BACKWARDS when deleting rows: `For i = lastRow To 2 Step -1`."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Unbounded Do While Loop Freezing Excel"}</td>
                  <td className="py-3 px-4">{"Writing `Do While x < 10` and forgetting to increment `x = x + 1` creates an infinite calculation loop."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Include increment counters and test loops with F8 stepping."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Looping Individual Cells on 1-Million Row Grids (Slow)"}</td>
                  <td className="py-3 px-4">{"Iterating cell-by-cell `For Each c In Range(\"A1:A1000000\")` is slow because it crosses the COM boundary 1 million times."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Read the entire range into a Variant Array in 1 shot, process in RAM, and write back in 1 shot."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Loop Backwards for Deletion"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"The only safe way to delete rows conditionally without skipping."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"For i = lastRow To 2 Step -1"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Loop All Sheets"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Fastest way to perform operations across all workbook tabs."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"For Each ws In ThisWorkbook.Worksheets"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Exit For Early"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Terminates loop immediately once search target is found."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"If found Then Exit For"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why MUST you loop backwards (`Step -1`) when deleting rows from a worksheet in VBA?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"How does `For Each ws In ThisWorkbook.Worksheets` allow you to apply formatting across 50 tabs in 2 lines of code?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the procedural execution mechanics and object model architecture.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"Why is reading a range into a VBA memory array 100x faster than looping through cells one by one on the worksheet?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the procedural execution mechanics and object model architecture.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Looping constructs: For...Next, For Each...Next (Looping over sheets, cells, shapes), and Do While/Until - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Remember this golden rule: If you are DELETING rows, always loop backwards! `For i = lastRow To 2 Step -1`. If you loop forward, deleting a row shifts everything up, and your macro will skip the next row."}
          />
        </div>
      </div>
    </div>
