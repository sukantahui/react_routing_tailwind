      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🧱 Variables, Data Types & Option Explicit"} · Topic 4
            </span>
          ...
          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Mastering VBA language fundamentals: mandatory Option Explicit declaration, primitive and object data types (Long vs Integer, Double, String, Boolean, Range), and variable scope (Dim, Public, Private)"}. Master the complete VBA language architecture, object model hierarchy, dynamic range operations, loops, performance switches, and defensive error handling.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Option Explicit | Dim VarName As DataType (Long, Double, String, Boolean, Date, Range, Object, Variant) | Scope: Dim / Public / Private"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. Mandatory Option Explicit"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Forces explicit variable declaration. Prevents catastrophic bugs caused by simple variable name typos (e.g. `TotalRevenue` vs `TotallRevenue`)."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. The Long vs Integer Rule (VBA Best Practice)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Never use `Integer` (capped at 32,767). In modern 32/64-bit systems, ALWAYS use `Long` for whole numbers and row counters (up to 2.14 billion)."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Object Variables Require `Set` Keyword"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Scalar variables (numbers/strings) use simple assignment `x = 10`. Object variables (Ranges, Worksheets, Workbooks) MUST use `Set`: `Set rng = Range(\"A1:B10\")`."}</p>
            </div>
        ...
        >
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod15_4" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod15_4" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod15_4)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"VBA Language Fundamentals"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod15_4)" fill="none" />
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Enterprise Variable Standardization"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Enforces `Option Explicit` across all corporate financial modules, standardizing Hungarian naming prefixes (`strName`, `lngRow`, `dblAmount`, `rngTarget`)."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Eliminating Row Overflow Errors with Long"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Converts legacy integer row loop counters to `Dim lastRow As Long` to process 150,000 purchase ledger records."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Range Object Reference Management"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses `Dim wsSource As Worksheet: Set wsSource = ThisWorkbook.Sheets(\"RawData\")` for clean object referencing."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Public Global Configuration Variables"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Defines `Public Const DIESEL_RATE As Double = 94.50` across multi-module transportation models."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Place <code className="text-cyan-300 font-mono font-bold">Option Explicit</code> at the top and declare variables with explicit types <code className="text-cyan-300 font-mono font-bold">{"Option Explicit | Dim VarName As DataType (Long, Double, String, Boolean, Date, Range, Object, Variant) | Scope: Dim / Public / Private"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Omitting Option Explicit (The Silent Typo Trap)"}</td>
                  <td className="py-3 px-4">{"Misspelling `TotalTax = TotlTax + 100` creates a new blank Variant variable with value 0, silently corrupting financial calculations."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always place `Option Explicit` at the very top of every single module (Tools > Options > Check 'Require Variable Declaration')."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Overflow Error (Error 6) with Integer Data Type"}</td>
                  <td className="py-3 px-4">{"Using `Dim i As Integer` on a loop exceeding 32,767 rows throws runtime error 'Overflow'."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always declare row counters as `Long`."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Omitting 'Set' on Object Assignment"}</td>
                  <td className="py-3 px-4">{"Writing `rng = Sheets(1).Range(\"A1\")` throws runtime error 'Object variable not set'."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use `Set`: `Set rng = Sheets(1).Range(\"A1\")`."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Require Variable Declaration"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Automatically inserts 'Option Explicit' at the top of all new modules."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Tools > Options > Check 'Require Variable Declaration'"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Always Use Long, Never Integer"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Supports up to 2.14 billion rows with zero overflow errors."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Dim lastRow As Long"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Use Set for Objects"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Mandatory syntax for Range, Worksheet, and Workbook objects."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Set ws = ThisWorkbook.Sheets(1)"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why is `Option Explicit` considered the single most important safety command in all of VBA programming?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"Why does looping through 40,000 rows with `Dim i As Integer` crash with runtime error 'Overflow'?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the procedural execution mechanics and object model architecture.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"What is the difference between assigning a scalar variable (`x = 5`) versus an object variable (`Set rng = Range(\"A1\")`)?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the procedural execution mechanics and object model architecture.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"VBA Language fundamentals: Variables, Data types, Option Explicit, and Scope (Dim, Public, Private) - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
