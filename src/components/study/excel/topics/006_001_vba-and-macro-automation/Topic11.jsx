      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🛡️ Error Handling: On Error GoTo & Defensive Code"} · Topic 11
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Defensive Programming & Fault Tolerance"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Basic error handling: `On Error Resume Next`, `On Error GoTo ErrorHandler`"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Building bulletproof, production-grade error handling in VBA: On Error GoTo handlers, On Error Resume Next traps, inspecting Err objects (Number, Description), and clean resource exits"}. Master the complete VBA language architecture, object model hierarchy, dynamic range operations, loops, performance switches, and defensive error handling.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Structured Error Handling: On Error GoTo ErrorHandler | On Error Resume Next | Err.Number, Err.Description, Err.Clear | Exit Sub"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The Unhandled Error Disaster"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"An unhandled error crashes the macro, exposes raw VBA code to end-users with an ugly 'Debug/End' dialog, and leaves Excel in an unstable state."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Structured Error Handler Pattern"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Includes `Exit Sub` before the `ErrorHandler:` label so error handling code only runs when an actual error occurs."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. The `Err` Object Properties"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`Err.Number` (numeric error code e.g. 9, 1004), `Err.Description` (readable message), and `Err.Clear` (resets error state)."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"VBA Error Handling Architecture: Normal Execution Stream -> Error Interceptor -> Recovery & Reset Block"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod15_11" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod15_11" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod15_11)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Defensive Programming & Fault Tolerance"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod15_11)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic11_Basic_error_handli"
            title={"Basic error handling: `On Error Resume Next`, `On Error GoTo ErrorHandler` - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Enterprise Production Macro Fault-Tolerance"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Wraps 40 mission-critical financial automation scripts in structured error handlers, logging errors gracefully to text files without user crashes."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Missing Sheet & Corrupted File Trapping"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Gracefully traps 'File not found' and 'Sheet subscript out of range' errors when importing monthly vendor ledgers."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Sensor File Lock Error Recovery"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Handles locked external file errors by pausing and retrying file access before alerting operators."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Database Timeout Error Interception"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Catches SQL connection timeouts and displays clear recovery guidance."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Place <code className="text-cyan-300 font-mono font-bold">Option Explicit</code> at the top and declare variables with explicit types <code className="text-cyan-300 font-mono font-bold">{"Structured Error Handling: On Error GoTo ErrorHandler | On Error Resume Next | Err.Number, Err.Description, Err.Clear | Exit Sub"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Omitting 'Exit Sub' Before Error Handler Label"}</td>
                  <td className="py-3 px-4">{"Leaving out `Exit Sub` causes the error handler code to run every single time the macro runs, even when there was NO error!"}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always place `Exit Sub` immediately before the `ErrorHandler:` label."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Overusing 'On Error Resume Next' Everywhere"}</td>
                  <td className="py-3 px-4">{"Blinded by `Resume Next`, the macro silently ignores corrupted data, leading to catastrophic incorrect financial statements."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use `On Error Resume Next` strictly for single targeted lines, followed immediately by `On Error GoTo 0`."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Leaving ScreenUpdating Disabled on Crash"}</td>
                  <td className="py-3 px-4">{"Crash occurs before error handler, leaving screen updating off."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Put UI reset code inside the ErrorHandler block."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Standard Error Handler Structure"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"The bulletproof template for production VBA."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"On Error GoTo ErrHandler ... Exit Sub ... ErrHandler: ..."}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Reset Error Trapping"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Disables error bypass and restores default runtime error handling."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"On Error GoTo 0"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Inspect Err.Description"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Displays polite, informative error messages to end-users."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"MsgBox \"Error: \" & Err.Description, vbCritical"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why must you always place `Exit Sub` immediately before your `ErrorHandler:` label?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"Why is overusing `On Error Resume Next` dangerous for financial calculations?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the procedural execution mechanics and object model architecture.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"How does structured error handling prevent end-users from seeing the scary VBA 'Debug/End' dialog?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the procedural execution mechanics and object model architecture.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Basic error handling: `On Error Resume Next`, `On Error GoTo ErrorHandler` - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Never let your users see the yellow 'Debug' error screen! Put `On Error GoTo ErrHandler` at the top of your macro, put `Exit Sub` before the label, and display a polite message like: 'Could not find file. Please check folder path.'"}
          />
        </div>
      </div>
    </div>
