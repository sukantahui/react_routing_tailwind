      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🎯 Range & Cell Manipulation (Offset & CurrentRegion)"} · Topic 6
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Cell Navigation & Grid Mastery"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Working with Ranges and Cells: Value, Text, Formula, Offset, CurrentRegion, and SpecialCells"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Mastering programmatic range and cell operations: Value vs Value2 vs Text, Cells(r,c) coordinate addressing, Offset navigation, finding the last row (End(xlUp)), CurrentRegion, and SpecialCells"}. Master the complete VBA language architecture, object model hierarchy, dynamic range operations, loops, performance switches, and defensive error handling.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Range Mastery: Range(\"A1\").Value | Cells(r, c) | Range(\"A1\").CurrentRegion | Range(\"A1\").Offset(r, c) | SpecialCells(xlCellTypeConstants)"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. Dynamic Last Row Detection (The Gold Standard)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`lastRow = ws.Cells(ws.Rows.Count, \"A\").End(xlUp).Row`. Starts at bottom of column A and shoots upward, finding the true last row regardless of table size."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Cells(Row, Col) Coordinate Indexing"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Ideal for programmatic loops (`Cells(i, 1)` where row `i` increments from 2 to `lastRow`)."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. CurrentRegion & SpecialCells Magic"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`Range(\"A1\").CurrentRegion` selects the entire contiguous table block (like Ctrl+A); `SpecialCells(xlCellTypeFormulas)` selects all formula cells across the sheet."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Range Navigation & Boundary Detection Engine: Offset -> End(xlUp) -> CurrentRegion"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod15_6" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod15_6" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod15_6)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Cell Navigation & Grid Mastery"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod15_6)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic6_Working_with_Range"
            title={"Working with Ranges and Cells: Value, Text, Formula, Offset, CurrentRegion, and SpecialCells - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Variable-Length Monthly Table Ingestion"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses `lastRow = ws.Cells(ws.Rows.Count, \"A\").End(xlUp).Row` to process monthly sales files ranging from 5,000 to 250,000 rows dynamically."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Isolating Hardcoded Numbers with SpecialCells"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses `ws.UsedRange.SpecialCells(xlCellTypeConstants, xlNumbers)` to audit hardcoded tax numbers masquerading in formula sheets."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Table Block Copy via CurrentRegion"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Copies entire machine shift inspection tables in 1 line: `Sheet1.Range(\"A1\").CurrentRegion.Copy Sheet2.Range(\"A1\")`."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Offset Next Available Row Write-Back"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Appends new freight waybill logs to the next empty row: `ws.Cells(lastRow + 1, 1).Value = NewWaybillID`."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Place <code className="text-cyan-300 font-mono font-bold">Option Explicit</code> at the top and declare variables with explicit types <code className="text-cyan-300 font-mono font-bold">{"Range Mastery: Range(\"A1\").Value | Cells(r, c) | Range(\"A1\").CurrentRegion | Range(\"A1\").Offset(r, c) | SpecialCells(xlCellTypeConstants)"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Finding Last Row with UsedRange.Rows.Count (Unreliable)"}</td>
                  <td className="py-3 px-4">{"`UsedRange` does not reset when rows are deleted, returning phantom last row counts."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always use `ws.Cells(ws.Rows.Count, \"A\").End(xlUp).Row`."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Using .Text Instead of .Value / .Value2"}</td>
                  <td className="py-3 px-4">{"Reading `.Text` returns the formatted display string (e.g. \"$ 10.00\" or \"###\" if column is narrow) instead of the true underlying number."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always read and write `.Value` or `.Value2`."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"SpecialCells Crashing on Empty Sets"}</td>
                  <td className="py-3 px-4">{"Calling `SpecialCells(xlCellTypeFormulas)` on a sheet with zero formulas throws a fatal runtime error 1004."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Wrap SpecialCells calls inside `On Error Resume Next` and check `If Not rng Is Nothing Then`."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Dynamic Last Row"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"The universal gold standard formula to find the last row in Excel VBA."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"lastRow = ws.Cells(ws.Rows.Count, \"A\").End(xlUp).Row"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Contiguous Table Copy"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Copies an entire dataset in 1 line of code."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Range(\"A1\").CurrentRegion.Copy wsTarget.Range(\"A1\")"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Offset Navigation"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Moves 1 row down from current position."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"ActiveCell.Offset(1, 0)"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why is `ws.Cells(ws.Rows.Count, \"A\").End(xlUp).Row` the single most reliable way to find the last row in a table?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"What is the dangerous difference between reading `Range(\"A1\").Text` versus `Range(\"A1\").Value`?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the procedural execution mechanics and object model architecture.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"Why does `SpecialCells` throw a fatal runtime error if no matching cells are found, and how do you handle it?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the procedural execution mechanics and object model architecture.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Working with Ranges and Cells: Value, Text, Formula, Offset, CurrentRegion, and SpecialCells - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"If you remember only one line of VBA code for the rest of your life, make it this: `lastRow = ws.Cells(ws.Rows.Count, \"A\").End(xlUp).Row`. It automatically finds the bottom of your data every single time."}
          />
        </div>
      </div>
    </div>
