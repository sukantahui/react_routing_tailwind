      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"⚡ VBA Speed Optimization (100x Execution Speedup)"} · Topic 10
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"High-Speed Execution Optimization"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Speed optimization techniques: `Application.ScreenUpdating = False`, `Application.Calculation = xlCalculationManual`"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Supercharging VBA execution speed by 50x to 100x using the Big 4 optimization switches: disabling ScreenUpdating, pausing automatic calculation, disabling event triggers, and processing data in RAM arrays"}. Master the complete VBA language architecture, object model hierarchy, dynamic range operations, loops, performance switches, and defensive error handling.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"VBA High-Speed Optimization Engine: Application.ScreenUpdating = False | Application.Calculation = xlCalculationManual | Application.EnableEvents = False | Application.DisplayAlerts = False"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The Big 4 Optimization Switches"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"1. `ScreenUpdating = False` (stops screen flicker and redraw overhead); 2. `Calculation = xlCalculationManual` (stops sheet recalculation on every cell write); 3. `EnableEvents = False` (prevents event triggers); 4. `DisplayAlerts = False` (suppresses confirmation prompts)."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. The Mandatory Reset & Error-Proofing"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Every optimization routine MUST restore switches in an error-handling block (`Finally: Application.ScreenUpdating = True ...`), otherwise Excel remains frozen."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Variant Array RAM Processing (The 100x Multiplier)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Transferring an entire 100,000-row range to a 2D Variant array in RAM (`arr = ws.Range(\"A1:E100000\").Value2`), processing in memory, and dumping back in 1 shot."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"VBA Execution Acceleration Pipeline: ScreenUpdating Off + Manual Calc + RAM Array Ingestion"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod15_10" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod15_10" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod15_10)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"High-Speed Execution Optimization"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod15_10)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic10_Speed_optimization"
            title={"Speed optimization techniques: `Application.ScreenUpdating = False`, `Application.Calculation = xlCalculationManual` - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Slashing 15-Minute Macro to 4 Seconds"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Applies the Big 4 optimization switches and Variant RAM arrays to an enterprise consolidation macro, reducing execution time from 15 minutes to 4.2 seconds."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: High-Speed Tax Reconciliation Engine"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Processes 200,000 vendor invoice lines in RAM memory with zero screen flicker."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Telemetry Ingestion Speedup"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Eliminates Excel UI redraw lag during massive multi-sensor log imports."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Nationwide Route Batch Re-Calculation"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Executes 100,000 freight tariff calculations in under 2 seconds."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Place <code className="text-cyan-300 font-mono font-bold">Option Explicit</code> at the top and declare variables with explicit types <code className="text-cyan-300 font-mono font-bold">{"VBA High-Speed Optimization Engine: Application.ScreenUpdating = False | Application.Calculation = xlCalculationManual | Application.EnableEvents = False | Application.DisplayAlerts = False"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Leaving ScreenUpdating / Events Disabled on Macro Error"}</td>
                  <td className="py-3 px-4">{"Macro crashes mid-way before restoring `ScreenUpdating = True`, leaving Excel frozen and unresponsive to user clicks."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always wrap in structured error handling (`On Error GoTo CleanUp`) to guarantee switches are restored."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Leaving Calculation in Manual Mode After Execution"}</td>
                  <td className="py-3 px-4">{"User edits numbers later and wonders why formulas are not updating."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Restore `Application.Calculation = xlCalculationAutomatic` before exiting sub."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Writing Cell-by-Cell in a 100,000 Row Loop"}</td>
                  <td className="py-3 px-4">{"Crossing the COM bridge 100,000 times to write individual cells wastes 99% of CPU time."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use a Variant array to write all 100,000 rows in a single 1-line assignment: `Range(\"A1:E100000\").Value2 = arr`."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Speed Optimization Wrapper"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"The standard professional template for every production macro."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Turn Big 4 OFF at start -> ON at end"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"RAM Variant Array"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"The fastest possible way to manipulate large data in Excel."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"arr = Range.Value2 -> Process -> Range.Value2 = arr"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Restore in Error Block"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Guarantees Excel UI is never left in a frozen state."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"CleanUp: Application.ScreenUpdating = True"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why does setting `Application.ScreenUpdating = False` accelerate macro execution by 10x to 50x?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"Why must you always use an error-handling block (`On Error GoTo CleanUp`) when disabling screen updating and events?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the procedural execution mechanics and object model architecture.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"Why is processing data in a VBA memory array 100x faster than writing to worksheet cells one by one?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the procedural execution mechanics and object model architecture.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Speed optimization techniques: `Application.ScreenUpdating = False`, `Application.Calculation = xlCalculationManual` - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Every serious macro you write MUST start with `Application.ScreenUpdating = False` and end with `Application.ScreenUpdating = True`. It stops the screen from flickering and makes your code run 50x faster instantly."}
          />
        </div>
      </div>
    </div>
