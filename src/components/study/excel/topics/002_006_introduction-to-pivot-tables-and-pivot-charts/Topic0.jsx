    ...
    <div className="dark bg-slate-950 text-slate-100 min-h-screen py-8 px-4 sm:px-6 lg:px-8 font-sans selection:bg-sky-500/30 selection:text-sky-200">
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🧊 Pivot Engine Architecture"} · Topic 0
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"OLAP & In-Memory Summarization"}
            </span>
          ...
          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Understanding the PivotTable in-memory cache architecture, separating dimensional categorical attributes (Rows/Columns) from numeric quantifiable measures (Values)"}. Master the complete architecture, field layouts, calculation settings, and interactive executive reporting standards.
          </p>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The In-Memory Pivot Cache"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"When a PivotTable is created, Excel builds a compact, highly optimized in-memory snapshot of source data (the Pivot Cache), decoupling aggregation from sheet calculations."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Dimensions vs Measures"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Dimensions (e.g. Region, Product, Branch) define grouping axes in Rows and Columns; Measures (e.g. Sales, Units, Cost) define aggregated values in the Values quadrant."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Zero-Formula Dynamic Slicing"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Rearranging fields dynamically rebuilds cross-tabulated reports in milliseconds without typing or maintaining a single nested formula."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"PivotTable In-Memory Cache & Multidimensional Aggregation Pipeline"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod6_0" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod6_0" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod6_0)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"OLAP & In-Memory Summarization"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod6_0)" fill="none" />
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Enterprise Multi-Branch Sales Aggregation"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Consolidates 500,000 transaction rows into an executive cross-tabular summary across 12 regional branches in under 2 seconds."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Vendor Purchase Summary by Tax Slab"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Groups annual procurement invoices by GST tax rate categories to reconcile monthly input tax credits."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-amber-400">{"Case 3 · Plant Inventory Lead"}</span>
                <span className="text-xs font-mono text-slate-400">{"Ichapur Works"}</span>
              </div>
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Raw Material Warehouse Stock Summaries"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Analyzes quarterly steel scrap generation across 4 production shifts with instant row-and-column drill-down."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Carrier Freight Cost Breakdown"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Evaluates freight expenditure across transport corridors and vehicle classes with zero nested formulas."}</p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Pivot Cache Memory Bloat"}</td>
                  <td className="py-3 px-4">{"Creating separate PivotTables from raw ranges rather than sharing a single Pivot Cache inflates workbook file size."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Create subsequent PivotTables from the existing PivotTable to share a unified Pivot Cache."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Treating Pivot Table as Live Formula"}</td>
                  <td className="py-3 px-4">{"Expecting PivotTables to update automatically when source cells change (unlike formulas, PivotTables require a Refresh)."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Right-click > Refresh (Alt + F5) or enable 'Refresh data when opening the file'."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Missing Grouping on Mixed Data Types"}</td>
                  <td className="py-3 px-4">{"Mixing text strings in date columns prevents PivotTable date grouping features."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Ensure source columns contain 100% pure serial dates without text entries."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Instant Pivot Table"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Fastest keyboard accelerator to insert a PivotTable from any data table."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Alt + N + V + T"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Refresh Active Pivot"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Instantly refreshes the active PivotTable cache."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Alt + F5"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Refresh All Workbooks"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Refreshes all PivotTables and external queries across the entire workbook."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Ctrl + Alt + F5"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why does a PivotTable not update automatically when source cells change?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"What is the memory and performance benefit of sharing a single Pivot Cache across 10 pivot reports?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the multidimensional OLAP mechanics and dashboard interactivity principles.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"How do Dimensions differ from Measures in dimensional data modeling?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the multidimensional OLAP mechanics and dashboard interactivity principles.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Understanding Pivot Tables: Dimensions, Measures, and Tabular Summarization - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"The Pivot Cache is Excel's in-memory analytical database. Understand that PivotTables summarize data from this cache, which is why clicking 'Refresh' is essential after updating records."}
          />
        </div>
      </div>
    </div>
