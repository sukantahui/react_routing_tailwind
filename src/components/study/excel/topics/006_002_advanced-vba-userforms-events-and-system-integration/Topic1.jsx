      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🎯 Worksheet Events: Change & SelectionChange"} · Topic 1
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Cell-Level Event Trapping"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Worksheet events: `Worksheet_Change`, `Worksheet_SelectionChange`, `Worksheet_Calculate`"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Trapping granular cell changes in real-time using Worksheet_Change: intercepting specific target columns (Intersect), auto-timestamping edits, audit trails, and cascading menu updates"}. Master event-driven programming, GUI UserForm engineering, FileSystemObject automation, cross-Office integrations, and custom add-in deployment.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Worksheet Events (Sheet Modules): Private Sub Worksheet_Change(ByVal Target As Range) | Worksheet_SelectionChange | Worksheet_Calculate"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The `Target` Range Parameter"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`Target` represents the exact cell or multi-cell range that was modified by the user or formula."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. The Mandatory `Intersect` Guard Rail"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`If Not Intersect(Target, Range(\"B2:B100\")) Is Nothing Then` ensures the macro runs ONLY when the user edits designated monitoring columns."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Event Loop Suppression (`EnableEvents = False`)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"If `Worksheet_Change` modifies another cell on the sheet, it will trigger ITSELF recursively unless wrapped in `Application.EnableEvents = False`."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Worksheet Event Interceptor Pipeline: Cell Edit -> Intersect Check -> Target Processing -> UI Update"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod16_1" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod16_1" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod16_1)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Cell-Level Event Trapping"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod16_1)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic1_Worksheet_events__"
            title={"Worksheet events: `Worksheet_Change`, `Worksheet_SelectionChange`, `Worksheet_Calculate` - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Automated Modification Timestamp & Auditor Trail"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses `Worksheet_Change` to auto-stamp `Target.Offset(0, 1).Value = Now` and record `Target.Offset(0, 2).Value = Environ(\"USERNAME\")` whenever price cells change."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Instant Uppercase HSN Code Formatter"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Auto-converts entered voucher text to uppercase in real time using `Target.Value = UCase(Target.Value)`."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Dynamic Secondary Dropdown Reset"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Clears sub-category selection cells automatically whenever the primary category cell is modified by an operator."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Immediate Visual Highlight on Status Change"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Turns entire row green when driver status column changes to 'Delivered'."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Implement the event procedure <code className="text-cyan-300 font-mono font-bold">{"Worksheet Events (Sheet Modules): Private Sub Worksheet_Change(ByVal Target As Range) | Worksheet_SelectionChange | Worksheet_Calculate"}</code> with appropriate guard rails.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"The Fatal Infinite Event Recurse Crash"}</td>
                  <td className="py-3 px-4">{"Modifying a cell inside `Worksheet_Change` without `Application.EnableEvents = False` causes the macro to trigger itself endlessly until Excel crashes."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always wrap cell writes in `Application.EnableEvents = False ... Target.Offset(0,1).Value = Now ... Application.EnableEvents = True`."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Crashing on Multi-Cell Paste or Delete"}</td>
                  <td className="py-3 px-4">{"Assuming `Target` is a single cell (`Target.Value`) crashes with 'Type Mismatch' when user pastes 50 cells at once."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Check `If Target.CountLarge > 1 Then Exit Sub` or loop `For Each c In Target`."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Placing Code in Standard Module"}</td>
                  <td className="py-3 px-4">{"Sheet events written in Module1 are completely ignored by Excel."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Place code in the specific `Sheet1 (Code)` object."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Intersect Guard Rail"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Restricts execution strictly to target columns."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"If Not Intersect(Target, Range(\"A:A\")) Is Nothing Then"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Mandatory Event Toggle"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Prevents catastrophic recursive event loops."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Application.EnableEvents = False (before writing)"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Multi-Cell Guard"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Prevents runtime errors on large paste operations."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"If Target.CountLarge > 1 Then Exit Sub"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why is `Application.EnableEvents = False` mandatory before writing to a cell inside `Worksheet_Change`?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"How does `Intersect(Target, Range(\"B:B\"))` restrict your macro to running only when column B is edited?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the event lifecycle dynamics and system integration principles.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"Why does writing `Target.Value` crash if the user pastes a range of 10 cells simultaneously?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the event lifecycle dynamics and system integration principles.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Worksheet events: `Worksheet_Change`, `Worksheet_SelectionChange`, `Worksheet_Calculate` - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"The #1 rule of Worksheet_Change: ALWAYS turn off events before modifying a cell! `Application.EnableEvents = False`, write your timestamp, then `Application.EnableEvents = True`. If you forget this, Excel will get stuck in an infinite loop."}
          />
        </div>
      </div>
    </div>
