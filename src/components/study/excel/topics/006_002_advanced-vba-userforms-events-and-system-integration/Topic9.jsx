      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"📋 Enterprise Logging & Call Stack Tracing"} · Topic 9
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Enterprise Auditing & Call Stack Tracing"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Robust enterprise error handling, call stack tracing, and debug logging to text files"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Building institutional logging frameworks and call stack tracing in VBA: writing persistent timestamped execution logs to external text files, catching silent failures, and debug auditing"}. Master event-driven programming, GUI UserForm engineering, FileSystemObject automation, cross-Office integrations, and custom add-in deployment.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Enterprise Debug Logging: Open logPath For Append As #fNum -> Print #fNum, Format(Now, \"yyyy-mm-dd hh:nn:ss\") & \" | \" & ErrMsg -> Close #fNum"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. Persistent Disk Audit Logging"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Appends execution milestones, duration metrics, user identities, and error dumps to a dedicated `automation_audit.log` file on disk."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Call Stack Tracing Paradigm"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Tracks the active subroutine hierarchy (`MainController -> IngestData -> ValidateRow -> ParseDate`), recording the exact call path when a failure occurs."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Non-Intrusive Production Telemetry"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Runs silently in background with zero UI interruption, providing IT and auditors with a complete forensic audit trail."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Enterprise Logging & Call Stack Architecture: Subroutine Invocations -> Call Stack Tracker -> Persistent Disk Audit Log"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod16_9" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod16_9" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod16_9)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Enterprise Auditing & Call Stack Tracing"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod16_9)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic9_Robust_enterprise_"
            title={"Robust enterprise error handling, call stack tracing, and debug logging to text files - Interactive Practice Grid"}
            rowsPerPage={10}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-amber-400">{"Case 1 · Enterprise IT Systems Architect"}</span>
                <span className="text-xs font-mono text-slate-400">{"Barrackpore HQ"}</span>
              </div>
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Mission-Critical Financial Core Audit Logger"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Deploys an enterprise logging module across 50 corporate financial automation workbooks, recording user sessions and error traces."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-amber-400">{"Case 2 · Corporate Audit Lead"}</span>
                <span className="text-xs font-mono text-slate-400">{"Shyamnagar Plant"}</span>
              </div>
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Statutory Tax Reconciliation Audit Trail"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Maintains an immutable text log of all tax adjustment transactions executed via automation scripts."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Shopfloor Automation Performance Telemetry"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Logs batch processing cycle times and machine communication exceptions to network server logs."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Consignment Processing Exception Log"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Logs invalid waybill numbers and supplier rate discrepancies."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Implement the event procedure <code className="text-cyan-300 font-mono font-bold">{"Enterprise Debug Logging: Open logPath For Append As #fNum -> Print #fNum, Format(Now, \"yyyy-mm-dd hh:nn:ss\") & \" | \" & ErrMsg -> Close #fNum"}</code> with appropriate guard rails.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Locking the Log File on Network Share"}</td>
                  <td className="py-3 px-4">{"Multiple users running macros simultaneously trying to write to the same shared log file without error trapping."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use localized user log files (`\"audit_\" & Environ(\"USERNAME\") & \".log\"`) or brief append-and-close locks."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Logging Sensitive Passwords or PII Data in Plaintext"}</td>
                  <td className="py-3 px-4">{"Writing database passwords or sensitive customer credit numbers into unencrypted log files."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Sanitize and mask sensitive credentials before writing to audit logs."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Failing to Handle Log File Path Access Denied"}</td>
                  <td className="py-3 px-4">{"Writing logs to `C:\\Program Files\\` where standard corporate users lack write permissions."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Write logs to `ThisWorkbook.Path` or `Environ(\"LOCALAPPDATA\")`."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Append Mode Syntax"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Standard high-speed file append syntax in VBA."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Open logFile For Append As #1: Print #1, msg: Close #1"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Safe User Folder"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Guarantees full write permissions across all corporate PCs."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Environ(\"TEMP\") or ThisWorkbook.Path"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Log Severity Levels"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Standard software logging taxonomy."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"INFO | WARN | ERROR | FATAL"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why is external text file logging essential for diagnosing macro failures on remote user laptops?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"What is 'Call Stack Tracing', and how does it pinpoint the exact subroutine where an error originated?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the event lifecycle dynamics and system integration principles.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"Why should audit logs never record plaintext database passwords or sensitive personal data?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the event lifecycle dynamics and system integration principles.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Robust enterprise error handling, call stack tracing, and debug logging to text files - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"When you build tools for other people, always include an Error Logger! Write a simple 3-line Sub that appends the error date, user name, and error description to `error_log.txt`. When someone says 'the macro broke', you can check the log and fix it in minutes."}
          />
        </div>
      </div>
    </div>
