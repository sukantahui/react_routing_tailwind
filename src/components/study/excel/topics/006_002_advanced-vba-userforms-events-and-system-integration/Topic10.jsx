      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🪟 Windows API Declarations (64-Bit PtrSafe)"} · Topic 10
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Low-Level Windows Operating System API"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Windows API declarations in VBA (64-bit PtrSafe calls, sleep timers, file dialogs)"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Harnessing the Windows Win32/Win64 Operating System API from VBA: declaring PtrSafe functions, conditional compilation (#If VBA7), high-resolution microsecond timers (GetTickCount / QueryPerformanceCounter), Sleep timers, and native file pickers"}. Master event-driven programming, GUI UserForm engineering, FileSystemObject automation, cross-Office integrations, and custom add-in deployment.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"64-Bit Windows API: #If VBA7 Then | Private Declare PtrSafe Function GetTickCount Lib \"kernel32\" () As LongLong | #Else | Declare Function ... | #EndIf"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The 32-Bit vs 64-Bit Compatibility Bridge"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Modern 64-bit Office requires the `PtrSafe` keyword and `LongPtr` data type for memory pointers, managed seamlessly using conditional compilation (`#If VBA7 Then ...`)."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Microsecond Precision Execution Timers"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`GetTickCount64` / `QueryPerformanceCounter` measures macro execution benchmarks down to sub-millisecond precision."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Non-Freezing Sleep Delays"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`Sleep(1000)` pauses execution precisely without consuming 100% CPU spinning in empty `DoEvents` loops."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Windows API Integration Architecture: VBA Runtime -> PtrSafe Win64 Bridge -> Windows OS Kernel32 / User32"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod16_10" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod16_10" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod16_10)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Low-Level Windows Operating System API"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod16_10)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic10_Windows_API_declar"
            title={"Windows API declarations in VBA (64-bit PtrSafe calls, sleep timers, file dialogs) - Interactive Practice Grid"}
            rowsPerPage={10}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-amber-400">{"Case 1 · Chief Quantitative Architect"}</span>
                <span className="text-xs font-mono text-slate-400">{"Barrackpore HQ"}</span>
              </div>
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Microsecond Financial Algorithm Benchmarking"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses `QueryPerformanceCounter` Win64 API to profile calculation latency across high-frequency financial derivative pricing routines."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-amber-400">{"Case 2 · Senior Systems Developer"}</span>
                <span className="text-xs font-mono text-slate-400">{"Shyamnagar Plant"}</span>
              </div>
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Cross-Bitness Enterprise VBA Compatibility"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Implements `#If VBA7` PtrSafe declarations ensuring payroll macros run seamlessly across both 32-bit and 64-bit Office installations."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Hardware Sleep Delay Integration"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses `kernel32.Sleep` to pause execution while industrial PLC hardware registers settle."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Native Windows Folder Browser Dialog"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Invokes `Shell32.BrowseForFolder` Win32 API for native Windows directory selection."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Implement the event procedure <code className="text-cyan-300 font-mono font-bold">{"64-Bit Windows API: #If VBA7 Then | Private Declare PtrSafe Function GetTickCount Lib \"kernel32\" () As LongLong | #Else | Declare Function ... | #EndIf"}</code> with appropriate guard rails.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Missing PtrSafe on 64-Bit Office (Compile Error)"}</td>
                  <td className="py-3 px-4">{"Declaring `Private Declare Function Sleep Lib \"kernel32\" ...` on 64-bit Office throws 'The code in this project must be updated for use on 64-bit systems'."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Add `PtrSafe` keyword: `Private Declare PtrSafe Sub Sleep Lib \"kernel32\" (ByVal dwMilliseconds As Long)`."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Passing Wrong Integer Pointer Types (Fatal Memory Crash)"}</td>
                  <td className="py-3 px-4">{"Passing a 32-bit `Long` instead of `LongPtr` to a 64-bit memory address crashes Excel immediately with a blue screen or fatal memory fault."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always use `LongPtr` for memory pointers and window handles (`HWND`)."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Freezing UI During Long Sleep Delays"}</td>
                  <td className="py-3 px-4">{"Calling `Sleep(10000)` freezes Excel completely for 10 seconds with zero responsiveness to user input."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Call `DoEvents` in smaller sleep increments."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"PtrSafe Declaration Template"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Guarantees 100% compatibility across 32-bit and 64-bit Excel."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"#If VBA7 Then Private Declare PtrSafe Sub ... #End If"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Kernel32 Sleep Timer"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Precise millisecond pause without CPU spikes."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Declare PtrSafe Sub Sleep Lib \"kernel32\" (ByVal ms As Long)"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"LongPtr for Handles"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Adapts automatically to 32-bit (4 bytes) or 64-bit (8 bytes)."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Dim hwnd As LongPtr"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why does 64-bit Microsoft Excel require the `PtrSafe` keyword in all Windows API declarations?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"How does `#If VBA7 Then ... #Else ... #End If` allow your code to run on both 32-bit and 64-bit Office installations?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the event lifecycle dynamics and system integration principles.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"What is `LongPtr`, and why is it used for memory pointers and window handles (`HWND`)?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the event lifecycle dynamics and system integration principles.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Windows API declarations in VBA (64-bit PtrSafe calls, sleep timers, file dialogs) - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Whenever you use Windows API declarations (like `Sleep` or file dialogs), always include `PtrSafe`: `Declare PtrSafe Sub Sleep Lib \"kernel32\" (ByVal ms As Long)`. This ensures your code works flawlessly on modern 64-bit Excel."}
          />
        </div>
      </div>
    </div>
