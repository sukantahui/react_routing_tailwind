      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"📝 UserForm Validation & Worksheet Write-Back"} · Topic 3
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Form Validation & Write-Back Engine"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"UserForm validation, initialization events (`UserForm_Initialize`), and data write-back to worksheets"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Building validation engines and data write-back pipelines in UserForms: UserForm_Initialize population, numeric keystroke trapping, mandatory field checks, and writing clean rows to worksheets"}. Master event-driven programming, GUI UserForm engineering, FileSystemObject automation, cross-Office integrations, and custom add-in deployment.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"UserForm Events: Private Sub UserForm_Initialize() | btnSubmit_Click() | txtAmount_KeyPress(ByVal KeyAscii As MSForms.ReturnInteger) | Unload Me"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. UserForm_Initialize (Pre-Loading Data)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Executes automatically when the form loads: populates ComboBox dropdowns from worksheet tables, sets default dates (`txtDate.Text = Format(Date, \"dd-mmm-yyyy\")`), and sets initial focus."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Point-of-Entry Validation Gates"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Blocks invalid characters in real-time (e.g. `KeyPress` trapping allowing only digits 0-9 and decimal points); validates mandatory fields before submitting."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Atomic Worksheet Write-Back"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Finds `nextRow = ws.Cells(ws.Rows.Count, \"A\").End(xlUp).Row + 1`, writes all form fields to cells in a single atomic transaction, and clears the form for the next entry."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"UserForm Write-Back Pipeline: Initialize -> In-Flight Keystroke Validation -> Next Empty Row Write-Back -> Reset"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod16_3" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod16_3" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod16_3)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Form Validation & Write-Back Engine"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod16_3)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic3_UserForm_validatio"
            title={"UserForm validation, initialization events (`UserForm_Initialize`), and data write-back to worksheets - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Client Master Data Write-Back Portal"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Pre-loads regional sales directors in `cboDirector` on initialize, validates 10-digit mobile numbers, and appends records to `Dim_Customer`."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Purchase Voucher Entry Validation Engine"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Traps numeric amounts, validates GST rates, and writes balanced journal entries across general ledger sheets."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Machine Calibration Log Submission"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Enforces non-empty temperature readings before writing inspection results to ISO compliance logs."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Freight Consignment Dispatch Writer"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Validates driver licenses and appends trip dispatch records."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Implement the event procedure <code className="text-cyan-300 font-mono font-bold">{"UserForm Events: Private Sub UserForm_Initialize() | btnSubmit_Click() | txtAmount_KeyPress(ByVal KeyAscii As MSForms.ReturnInteger) | Unload Me"}</code> with appropriate guard rails.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Writing UserForm Text Directly as Numbers to Worksheet"}</td>
                  <td className="py-3 px-4">{"Writing `ws.Cells(r, 2).Value = txtAmount.Text` stores the number as TEXT with green error flags on the sheet."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Convert text to numbers explicitly: `ws.Cells(r, 2).Value = CDbl(txtAmount.Text)` or `Val(txtAmount.Text)`."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Using UserForm_Activate Instead of UserForm_Initialize"}</td>
                  <td className="py-3 px-4">{"`Activate` fires repeatedly every time the form regains focus, re-adding duplicate items to ComboBoxes."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Populate lists inside `UserForm_Initialize`."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Using Hide Instead of Unload Me on Close"}</td>
                  <td className="py-3 px-4">{"`UserForm.Hide` keeps the form in RAM with stale data; `Unload Me` completely flushes and resets the form."}</td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Populate ComboBox on Initialize"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Loads entire table list into dropdown in 1 line of code."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"cboDept.List = Range(\"DeptTable\").Value"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Convert Data Types"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Guarantees proper numeric and date storage in worksheet cells."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"CDbl(txtAmount.Text) / CDate(txtDate.Text)"}</kbd>
            </div>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              ...
              <p className="text-slate-300 leading-relaxed">{"Closes form and flushes memory cleanly."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Unload Me"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why must you use `CDbl()` or `Val()` when transferring numbers from a UserForm TextBox into a worksheet cell?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"Why is `UserForm_Initialize` preferred over `UserForm_Activate` for populating ComboBox dropdown lists?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the event lifecycle dynamics and system integration principles.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"What is the difference between `UserForm.Hide` (hide visually) and `Unload Me` (flush memory)?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the event lifecycle dynamics and system integration principles.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"UserForm validation, initialization events (`UserForm_Initialize`), and data write-back to worksheets - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Remember: Everything inside a TextBox is TEXT! If you write `ws.Cells(1,1).Value = txtPrice.Text`, Excel will store it as text and formulas won't calculate. Always convert it: `ws.Cells(1,1).Value = CDbl(txtPrice.Text)`."}
          />
        </div>
      </div>
    </div>
