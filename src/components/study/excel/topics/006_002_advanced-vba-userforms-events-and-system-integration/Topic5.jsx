      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"📁 File System Automation (FileSystemObject / FSO)"} · Topic 5
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"File System Automation using `Scripting.FileSystemObject` (FSO): Creating, moving, reading, and deleting files/folders"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Automating operating system file management from Excel VBA using Scripting.FileSystemObject (FSO): creating folders, moving files, checking existence, and reading/writing text/CSV logs"}. Master event-driven programming, GUI UserForm engineering, FileSystemObject automation, cross-Office integrations, and custom add-in deployment.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"FileSystemObject: Set fso = CreateObject(\"Scripting.FileSystemObject\") | fso.FileExists(path) | fso.FolderExists | fso.CopyFile | fso.OpenTextFile"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The Microsoft Scripting Runtime Engine"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Instantiated via Late Binding (`CreateObject(\"Scripting.FileSystemObject\")`) or Early Binding (`Tools > References > Microsoft Scripting Runtime`)."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. High-Speed File & Folder Manipulation"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Methods: `.FileExists()`, `.FolderExists()`, `.CreateFolder()`, `.CopyFile()`, `.MoveFile()`, `.DeleteFile()`."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Reading & Writing Text/CSV Streams"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Methods: `.OpenTextFile(path, IOMode, Create)` to stream text line-by-line (`ReadLine`, `WriteLine`) directly from disk without opening in Excel."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"FileSystemObject (FSO) Architecture: VBA Layer -> Scripting Runtime -> Windows File System"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod16_5" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod16_5" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod16_5)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"OS & File System Automation"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod16_5)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic5_File_System_Automa"
            title={"File System Automation using `Scripting.FileSystemObject` (FSO): Creating, moving, reading, and deleting files/folders - Interactive Practice Grid"}
            rowsPerPage={10}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-amber-400">{"Case 1 · Chief Financial Automation Lead"}</span>
                <span className="text-xs font-mono text-slate-400">{"Barrackpore HQ"}</span>
              </div>
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Automated Monthly Folder Structure Generator"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses FSO to dynamically create nested corporate directory trees (`C:\\Financials\\2026\\Q1\\March\\`) and archive closed workbooks."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Direct CSV Audit Log Streamer"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Streams 500,000 lines of raw server transaction logs via `fso.OpenTextFile` directly into memory arrays in 1.8 seconds."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Sensor Telemetry File Archiver"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Moves completed daily sensor text files from staging to archive directories using `fso.MoveFile`."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Waybill Manifest Existence Verifier"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Checks if driver manifest PDF attachments exist before generating dispatch emails."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Implement the event procedure <code className="text-cyan-300 font-mono font-bold">{"FileSystemObject: Set fso = CreateObject(\"Scripting.FileSystemObject\") | fso.FileExists(path) | fso.FolderExists | fso.CopyFile | fso.OpenTextFile"}</code> with appropriate guard rails.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Failing to Check if Directory Exists Before Creating"}</td>
                  <td className="py-3 px-4">{"Calling `fso.CreateFolder(\"C:\\A\\B\\C\")` when parent folder `C:\\A\\B` does not exist throws runtime error."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Create nested parent folders iteratively or verify with `If Not fso.FolderExists(path) Then`."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Leaving OpenTextFile Streams Unclosed in RAM"}</td>
                  <td className="py-3 px-4">{"Forgetting `ts.Close` locks the external file on disk, preventing other applications from accessing it."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always include `ts.Close: Set ts = Nothing` in clean-up blocks."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Overwriting Existing Files Without Overwrite Parameter"}</td>
                  <td className="py-3 px-4">{"Calling `fso.CopyFile` over an existing file without setting `Overwrite:=True` throws 'File already exists'."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Explicitly pass `fso.CopyFile src, dest, True`."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Late Binding FSO"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Runs on all client machines without reference errors."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Set fso = CreateObject(\"Scripting.FileSystemObject\")"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Check File Exists"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Eliminates file-not-found runtime crashes."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"If fso.FileExists(filePath) Then ..."}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Fast Text File Write"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Fastest way to write text/CSV logs from VBA."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Set ts = fso.CreateTextFile(path, True): ts.WriteLine text: ts.Close"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why is `Scripting.FileSystemObject` vastly more powerful than VBA's legacy `Dir()` and `Kill` functions?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"Why should you always call `ts.Close` after reading or writing text streams with FSO?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the event lifecycle dynamics and system integration principles.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"What is the difference between Late Binding (`CreateObject`) and Early Binding for FSO?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the event lifecycle dynamics and system integration principles.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"File System Automation using `Scripting.FileSystemObject` (FSO): Creating, moving, reading, and deleting files/folders - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"FileSystemObject (FSO) is the best way to handle files and folders in Windows. Use `fso.FileExists(path)` to verify files before opening, and `fso.CreateFolder` to build folder structures automatically."}
          />
        </div>
      </div>
    </div>
