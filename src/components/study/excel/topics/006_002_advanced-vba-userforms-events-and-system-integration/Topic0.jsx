      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"⚡ Workbook Events & Lifecycle Triggers"} · Topic 0
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Event-Driven Workbook Lifecycle"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Event-Driven Programming: Workbook events (`Workbook_Open`, `Workbook_BeforeSave`, `Workbook_BeforeClose`)"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Engineering event-driven automation at the workbook lifecycle level: intercepting Workbook_Open for automatic data refreshes, Workbook_BeforeSave for mandatory compliance audits, and Workbook_BeforeClose"}. Master event-driven programming, GUI UserForm engineering, FileSystemObject automation, cross-Office integrations, and custom add-in deployment.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Workbook Events (ThisWorkbook Module): Private Sub Workbook_Open() | Workbook_BeforeSave(ByVal SaveAsUI As Boolean, Cancel As Boolean) | Workbook_BeforeClose"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The Event-Driven Paradigm"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Code executes automatically in response to user or system actions (opening file, saving, closing) without manual button clicks."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. The `Cancel = True` Interceptor Mechanism"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Setting `Cancel = True` inside `Workbook_BeforeSave` or `Workbook_BeforeClose` blocks the save/close action if critical compliance checks fail."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Placement in ThisWorkbook Module"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Workbook events MUST reside exclusively inside the `ThisWorkbook` code module in Project Explorer to be recognized by Excel."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Workbook Lifecycle Event Architecture: Open Event -> Operational State -> Save/Close Interceptors"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod16_0" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod16_0" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod16_0)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Event-Driven Workbook Lifecycle"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod16_0)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic0_Event_Driven_Progr"
            title={"Event-Driven Programming: Workbook events (`Workbook_Open`, `Workbook_BeforeSave`, `Workbook_BeforeClose`) - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Automated Morning Dashboard SQL Sync on Open"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses `Workbook_Open()` to refresh central financial databases, unprotect sheets, update timestamps, and re-protect automatically on launch."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Mandatory PAN/GST Validation Before Save"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses `Workbook_BeforeSave(..., Cancel)` to block saving if mandatory tax reconciliation checkboxes remain unverified."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Automated Timestamped Backup on Close"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses `Workbook_BeforeClose()` to copy a timestamped backup archive to corporate network drives automatically."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Fleet Dispatch Session Audit Logger"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Logs user identity (`Environ(\"USERNAME\")`) and login timestamp upon workbook opening."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Implement the event procedure <code className="text-cyan-300 font-mono font-bold">{"Workbook Events (ThisWorkbook Module): Private Sub Workbook_Open() | Workbook_BeforeSave(ByVal SaveAsUI As Boolean, Cancel As Boolean) | Workbook_BeforeClose"}</code> with appropriate guard rails.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Writing Workbook Events in Standard Modules (Fails)"}</td>
                  <td className="py-3 px-4">{"Placing `Private Sub Workbook_Open()` in Module1 causes it to be ignored completely."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Double-click `ThisWorkbook` in Project Explorer and write event procedures there."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Infinite Loop Triggered Inside BeforeSave"}</td>
                  <td className="py-3 px-4">{"Calling `ThisWorkbook.Save` inside `Workbook_BeforeSave` triggers an infinite recursive save loop."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Do not call `.Save` inside BeforeSave; or wrap in `Application.EnableEvents = False`."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Trapping Users with Un-closable Workbooks"}</td>
                  <td className="py-3 px-4">{"Setting `Cancel = True` unconditionally in `Workbook_BeforeClose` prevents anyone from ever closing the file."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Ensure `Cancel = True` executes ONLY when explicit error conditions are met."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Place in ThisWorkbook"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Mandatory location for all workbook lifecycle event handlers."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Project Explorer > Double-click ThisWorkbook"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Cancel Save on Error"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Aborts saving if critical audit validations fail."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Cancel = True"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Disable Events in Event Code"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Prevents recursive cascading event loops."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Application.EnableEvents = False"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why must `Workbook_Open` be placed in the `ThisWorkbook` code module rather than a Standard Module?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"How does setting `Cancel = True` inside `Workbook_BeforeSave` enforce data entry compliance before saving?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the event lifecycle dynamics and system integration principles.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"What dangerous recursive loop happens if you call `ThisWorkbook.Save` inside `Workbook_BeforeSave` without disabling events?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the event lifecycle dynamics and system integration principles.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Event-Driven Programming: Workbook events (`Workbook_Open`, `Workbook_BeforeSave`, `Workbook_BeforeClose`) - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Workbook events are incredible for enterprise governance. You can make Excel say 'Welcome Swadeep!' on open, automatically refresh your SQL tables, and block the user from saving if they forgot to fill in cell A1."}
          />
        </div>
      </div>
    </div>
