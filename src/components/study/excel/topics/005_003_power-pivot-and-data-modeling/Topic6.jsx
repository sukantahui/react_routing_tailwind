      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"⚖️ Calculated Columns vs Calculated Measures"} · Topic 6
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Computational Governance & Memory"}
            </span>
          ...
          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Mastering the fundamental architecture difference between Calculated Columns (computed at data refresh, stored in RAM) and Calculated Measures (computed dynamically on-the-fly at query time)"}. Master the complete VertiPaq in-memory engine, Star Schema dimensional modeling, 1-to-Many relationships, Date dimensions, and enterprise model governance.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Calculated Column (Row Context): Sales[Margin] = Sales[Revenue] - Sales[Cost] | Measure (Filter Context): TotalMargin := SUM(Sales[Revenue]) - SUM(Sales[Cost])"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. Calculated Columns (RAM Storage & Row Context)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Evaluated row-by-row during data refresh. Appends a physical column to the table in RAM. Increases workbook file size."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Calculated Measures (CPU on-the-fly & Filter Context)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Evaluated dynamically when an end-user pivots data. Consumes ZERO storage RAM. Recalculates dynamically based on active slicers and row/column headers."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. The Golden Rule of Modeling"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Use Calculated Columns ONLY for slicer and category grouping attributes. Use Calculated Measures for ALL mathematical sums, averages, ratios, and KPIs."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Computational Architecture: Calculated Column (RAM / Row Context) vs Calculated Measure (CPU / Filter Context)"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod13_6" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod13_6" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod13_6)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Computational Governance & Memory"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod13_6)" fill="none" />
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Enterprise Memory Footprint Optimization"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Cuts corporate Data Model memory footprint from 450 MB down to 45 MB by replacing 25 redundant Calculated Columns with explicit DAX Measures."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Dynamic Margin % Ratio Measure"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Constructs explicit `ProfitMargin% := DIVIDE([TotalProfit], [TotalRevenue])` measure that aggregates correctly at all pivot levels."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Machine Age Categorization Column"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses a Calculated Column for `=IF(Machinery[Years] > 10, \"Legacy\", \"Modern\")` to allow slicing by machine age."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Dynamic Fuel Cost per KM Measure"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Calculates fleet fuel burn efficiency on-the-fly across dynamic truck and route slicer combinations."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                Open Power Pivot window (<kbd className="px-1.5 py-0.5 rounded bg-slate-800 font-mono text-xs text-sky-300">Alt + B + M</kbd>), switch to Diagram View, and drag Primary Keys to Foreign Keys <code className="text-cyan-300 font-mono font-bold">{"Calculated Column (Row Context): Sales[Margin] = Sales[Revenue] - Sales[Cost] | Measure (Filter Context): TotalMargin := SUM(Sales[Revenue]) - SUM(Sales[Cost])"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Averaging a Calculated Column Ratio (The Fatal Math Error)"}</td>
                  <td className="py-3 px-4">{"Creating a column `Margin% = Profit / Rev` and selecting `AVERAGE(Margin%)` in PivotTable computes the arithmetic average of percentages rather than the true weighted margin."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always compute financial ratios as explicit DAX measures: `Margin% := DIVIDE([TotalProfit], [TotalRevenue])`."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Creating Calculated Columns for Simple Math"}</td>
                  <td className="py-3 px-4">{"Adding `Sales[Total] = Sales[Qty] * Sales[Price]` across 10 million rows adds 80MB of wasted RAM."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use DAX Iterator measure: `TotalSales := SUMX(Sales, Sales[Qty] * Sales[Price])`."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Using Measures as Slicer Categories"}</td>
                  <td className="py-3 px-4">{"Attempting to drag a Calculated Measure into a PivotTable Slicer or Row label area (Excel only allows Columns in Slicers)."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use a Calculated Column if the attribute is required as a categorical slicer."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Columns for Slicers"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Allows slicing and grouping by custom attributes."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Use Calculated Columns for categories & bins"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Measures for Math"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Zero memory overhead and dynamically weighted accuracy."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Use Measures for all numeric KPIs & ratios"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"DIVIDE() for Safe Division"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Safe division handling zero denominators with zero errors."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"=DIVIDE(Num, Denom, 0)"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why does calculating the average of a Calculated Column percentage produce mathematically incorrect results in PivotTables?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"Why do Calculated Measures consume zero storage space in the Excel workbook file?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the columnar compression architecture and relational data modeling principles.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"When is a Calculated Column strictly necessary instead of a Calculated Measure?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the columnar compression architecture and relational data modeling principles.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Calculated Columns vs Calculated Measures: Memory footprint vs CPU computation at query time - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Memorize this rule: If you want to SLICE by it, make it a Column. If you want to AGGREGATE it (sum, average, ratio), make it a Measure! Never average a column of percentages."}
          />
        </div>
      </div>
    </div>
