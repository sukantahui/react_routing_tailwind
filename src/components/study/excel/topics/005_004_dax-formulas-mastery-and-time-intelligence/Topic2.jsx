      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🔄 Context Transition & CALCULATE"} · Topic 2
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Advanced Context Transformation"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Context Transition: Transforming Row Context into Filter Context with CALCULATE"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Understanding Context Transition: how invoking `CALCULATE()` (or calling an explicit measure) inside a Row Context transforms the current row's values into an equivalent Filter Context"}. Master evaluation contexts, Context Transition, CALCULATE filter overriding, X-iterator engines, Time Intelligence comparisons, and ranking.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Context Transition: Inside Row Context -> Invoke CALCULATE([Measure]) -> Row Context transforms into an Equivalent Filter Context"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The Context Transition Mechanism"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"When `CALCULATE()` executes inside a Row Context, it takes all column values of the current row and applies them as an exact Filter Context on the underlying tables."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Automatic Measure Invocation Wrap"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Every time an explicit measure `[TotalSales]` is called, Excel silently wraps it in an invisible `CALCULATE([TotalSales])`, automatically triggering Context Transition."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Iterating and Filtering Simultaneously"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Enables computing row-level metrics that depend on filtered table aggregates (e.g. ranking a customer against the total sales of their specific category)."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Context Transition Engine: Row Context (Current Record Key) -> CALCULATE() -> Active Filter Context"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod14_2" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod14_2" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_2)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Advanced Context Transformation"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_2)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic2_Context_Transition"
            title={"Context Transition: Transforming Row Context into Filter Context with CALCULATE - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Customer Lifetime Value Decile Tagging"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses Context Transition in a Calculated Column `=CALCULATE([TotalRevenue])` on `Dim_Customer` to compute each customer's total spend across all history."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Vendor Multi-Year Aggregate Audit"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Computes total historical vendor purchases directly inside `Dim_VendorMaster` using Context Transition."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Machine Breakdown Frequency Ranking"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Transforms machine row context into filter context to calculate historical breakdown counts per machine."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Driver Cumulative Kilometers Metric"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Calculates total driver lifetime mileage inside driver dimension tables."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                In the formula bar, type your measure name and formula: <code className="text-cyan-300 font-mono font-bold">{"Context Transition: Inside Row Context -> Invoke CALCULATE([Measure]) -> Row Context transforms into an Equivalent Filter Context"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Unintended Context Transition in Large Fact Tables"}</td>
                  <td className="py-3 px-4">{"Calling `[TotalSales]` inside a Calculated Column on a 10-million row fact table triggers 10 million individual CALCULATE transitions, freezing Excel for minutes."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Avoid Context Transition inside massive Fact tables; use it on small Dimension tables."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Thinking `SUM(Sales[Amount])` Triggers Context Transition"}</td>
                  <td className="py-3 px-4">{"Writing `SUM(Sales[Amount])` in a calculated column computes the grand total of the whole table, NOT the current row."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Wrap in `CALCULATE(SUM(Sales[Amount]))` or call the explicit measure `[TotalSales]` to trigger Context Transition."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Ignoring Duplicate Row Keys in Dimension Tables"}</td>
                  <td className="py-3 px-4">{"If the dimension table has duplicate rows, Context Transition filters multiple identical records simultaneously."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Ensure dimension tables have 100% unique primary keys."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Measures Have Hidden CALCULATE"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Calling any measure inside a calculated column always triggers Context Transition."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"[TotalSales] = CALCULATE([TotalSales])"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Dimension Customer Spend"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"The cleanest way to add total customer lifetime revenue to customer dimension."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Customer[TotalSpend] = [TotalSales]"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Performance Warning"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Prevents CPU freezing on massive multi-million row fact tables."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Only use on Dimension tables (<100k rows)"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why does writing `[TotalSales]` inside a Calculated Column calculate the sales for THAT specific customer rather than the grand total?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"Why does raw `=SUM(Sales[Amount])` in a Calculated Column return the grand total of the entire table instead?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"What is the hidden `CALCULATE()` wrapper that Microsoft places around every explicit measure call?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Context Transition: Transforming Row Context into Filter Context with CALCULATE - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Context Transition is DAX's superpower. When you write `[TotalSales]` in a customer table column, DAX secretly puts `CALCULATE` around it, transforms that customer's ID into a filter, and calculates their individual sales total instantly."}
          />
        </div>
      </div>
    </div>
