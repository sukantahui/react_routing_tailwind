      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🔄 Iterator Functions (X-Functions)"} · Topic 5
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Row-by-Row Iterator Mechanics"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Iterator functions (X-functions): SUMX, AVERAGEX, MINX, MAXX, and COUNTX"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Mastering DAX Iterator functions (X-functions): SUMX, AVERAGEX, MINX, MAXX, and COUNTX to evaluate row-by-row expressions across temporary virtual tables with zero RAM storage"}. Master evaluation contexts, Context Transition, CALCULATE filter overriding, X-iterator engines, Time Intelligence comparisons, and ranking.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Iterator Functions: SUMX(Table, Expression) | AVERAGEX(Table, Expression) | MINX | MAXX | COUNTX"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. Row-by-Row Iteration Engine"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Iterators accept a Table as their 1st argument and an Expression as their 2nd argument. They loop through the table row-by-row, evaluate the expression, and aggregate the results."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Eliminating Calculated Columns"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`SUMX(Fact_Sales, Fact_Sales[Qty] * Fact_Sales[UnitPrice] * (1 - Fact_Sales[Discount]))` calculates total revenue dynamically without adding stored columns in RAM."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Iterating Filtered Virtual Tables"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Iterators can iterate dynamically filtered tables: `SUMX(FILTER(Sales, Sales[Qty] > 100), Sales[Amount])`."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Iterator Function Pipeline: Virtual Table Ingestion -> Row Context Expression Loop -> Final Scalar Aggregation"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod14_5" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod14_5" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_5)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Row-by-Row Iterator Mechanics"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_5)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic5_Iterator_functions"
            title={"Iterator functions (X-functions): SUMX, AVERAGEX, MINX, MAXX, and COUNTX - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Line-Level Net Discounted Revenue Iterator"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Computes `TotalNetSales := SUMX(Fact_Sales, Fact_Sales[Quantity] * Fact_Sales[UnitPrice] * (1 - Fact_Sales[DiscountPct]))` across 5 million transactions in 0.2 seconds."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Weighted Average Vendor Tax Rate"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses `AVERAGEX(Dim_VendorMaster, [VendorEffectiveTaxRate])` to calculate true weighted compliance averages."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Peak Daily Machine Scrap Volatility"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Finds the maximum single-day scrap loss using `MAXX(Dim_Calendar, [DailyScrapMT])`."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Average Delivery Route Distance"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Calculates average shipment mileage using `AVERAGEX(Fact_Shipments, Fact_Shipments[EndKM] - Fact_Shipments[StartKM])`."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                In the formula bar, type your measure name and formula: <code className="text-cyan-300 font-mono font-bold">{"Iterator Functions: SUMX(Table, Expression) | AVERAGEX(Table, Expression) | MINX | MAXX | COUNTX"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Using SUMX When Standard SUM Suffices"}</td>
                  <td className="py-3 px-4">{"Writing `SUMX(Sales, Sales[Amount])` when `SUM(Sales[Amount])` accomplishes the exact same thing faster."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use `SUM()` for single-column additions; reserve `SUMX()` for multi-column math or expressions."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Iterating Massive Unfiltered Fact Tables with Complex Measures"}</td>
                  <td className="py-3 px-4">{"Calling a complex measure inside SUMX over 10 million rows causes high CPU latency."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Iterate small Dimension tables (`SUMX(Dim_Customer, ...)`), or push row math to raw columns."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Confusing SUM of Averages with Average of Sums"}</td>
                  <td className="py-3 px-4">{"Writing `AVERAGEX(Sales, [Margin%])` averages transactions equally rather than weighting by dollar volume."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use the ratio of sums: `DIVIDE([TotalMargin], [TotalSales])`."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Row Math Without Columns"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Calculates line-item revenue on the fly with zero RAM storage overhead."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"SUMX(Sales, Qty * Price)"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Iterate Dimension Tables"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"High-performance pattern to compute customer-level distributions."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"SUMX(Dim_Customer, [CustomerRevenue])"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"MAXX / MINX for Peak Analysis"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Finds highest single-day sales volume across historical calendar."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"MAXX(Dim_Calendar, [DailySales])"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why does `SUMX(Sales, Sales[Qty] * Sales[Price])` consume zero storage space in the Excel file?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"When should you use standard `SUM()` versus iterator `SUMX()` in DAX modeling?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"How do `MAXX()` and `MINX()` allow modelers to identify peak performance days across a calendar table?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Iterator functions (X-functions): SUMX, AVERAGEX, MINX, MAXX, and COUNTX - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"SUM only takes ONE column: `SUM(Sales[Amount])`. If you need to multiply two columns (like `Qty * Price`), use SUMX: `SUMX(Sales, Sales[Qty] * Sales[Price])`. It is clean, fast, and saves RAM."}
          />
        </div>
      </div>
    </div>
