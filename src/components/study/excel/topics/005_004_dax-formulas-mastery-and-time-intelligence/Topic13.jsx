      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🏆 Advanced Ranking: RANKX & TOPN"} · Topic 13
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Analytical Ranking & Segmentation"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Advanced ranking and top-performer DAX: RANKX and TOPN"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Building dynamic ranking and executive Top-N filtration models using RANKX and TOPN: ranking products/customers dynamically across changing slicer contexts with tie-break handling"}. Master evaluation contexts, Context Transition, CALCULATE filter overriding, X-iterator engines, Time Intelligence comparisons, and ranking.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Ranking & Top N: RANKX(ALL(Dim_Product), [TotalSales], , DESC, Dense) | TOPN(10, Dim_Customer, [TotalSales], DESC)"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. RANKX Architecture & Context"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`RANKX(Table, Expression, [Value], [Order], [Ties])`. Table argument must typically be wrapped in `ALL()` or `ALLSELECTED()` to establish the full ranking population."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Dynamic Slicer Re-Ranking"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Wrapping in `ALLSELECTED(Dim_Customer[Name])` re-ranks customers dynamically from 1 to N within the active regional slicer selection."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. TOPN Virtual Table Constructor"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`TOPN(10, Dim_Product, [TotalSales])` generates a temporary 10-row table of top-performing products, passed to `CALCULATE()` to compute 'Top 10 Sales Share'."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"RANKX Ranking Engine: Master Entity Table -> Dynamic Measure Sorting -> Numerical Integer Rank Assignment"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod14_13" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod14_13" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_13)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Analytical Ranking & Segmentation"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_13)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic13_Advanced_ranking_a"
            title={"Advanced ranking and top-performer DAX: RANKX and TOPN - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: National Branch Sales Performance Ranking"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Builds `BranchRank := RANKX(ALL(Dim_Branch[Name]), [TotalSales], , DESC, Dense)` ranking 50 branches across India."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Top 10 Vendor Spend Concentration"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Computes `Top10VendorSpend := CALCULATE([TotalSpend], TOPN(10, ALL(Dim_VendorMaster), [TotalSpend], DESC))`."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Machine Scrap Severity Ranking"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Ranks manufacturing machines dynamically based on monthly scrap rate percentage."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Freight Corridor Volume Rank"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Ranks 100 shipping corridors dynamically by total transported tonnage."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                In the formula bar, type your measure name and formula: <code className="text-cyan-300 font-mono font-bold">{"Ranking & Top N: RANKX(ALL(Dim_Product), [TotalSales], , DESC, Dense) | TOPN(10, Dim_Customer, [TotalSales], DESC)"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"The 'Every Row is Rank 1' Bug (The Classic RANKX Trap)"}</td>
                  <td className="py-3 px-4">{"Passing a table without `ALL()` (e.g. `RANKX(Dim_Product, [TotalSales])`) restricts the ranking population to the single row in the pivot, so every product ranks #1 against itself."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always wrap the ranking entity in `ALL()` or `ALLSELECTED()`: `RANKX(ALL(Dim_Product[Name]), [TotalSales])`."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Ranking Columns with Identical Values (Ties)"}</td>
                  <td className="py-3 px-4">{"Multiple entities with identical sales receiving identical rank numbers."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Configure the 5th parameter `[Ties]` as `Dense` or `Skip`, or add secondary decimal tie-breakers."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Performance Lag on 500,000 Row RANKX"}</td>
                  <td className="py-3 px-4">{"Running RANKX over massive fact tables."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Run RANKX exclusively over Dimension tables (`Dim_Customer`, `Dim_Product`)."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"The ALL() Wrap Mandatory"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Guarantees proper evaluation across the entire population."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"RANKX(ALL(Dim[Name]), [Measure])"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Top 10 Filter Measure"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Filters PivotTable to show only the top 10 entities."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"IF([Rank] <= 10, [TotalSales], BLANK())"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"TOPN Aggregation"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Directly computes sales for top 5 products."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"CALCULATE([Sales], TOPN(5, ALL(Product), [Sales]))"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why does `RANKX(Dim_Product, [TotalSales])` return Rank 1 for every single row in a PivotTable?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"Why is `ALL(Dim_Product[Name])` required inside RANKX to establish the ranking population?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"How does `TOPN()` allow modelers to compute the percentage of total company sales driven by the top 10 customers?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Advanced ranking and top-performer DAX: RANKX and TOPN - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"The #1 trap in DAX ranking: If your RANKX returns '1' for every row, you forgot `ALL()`! Always write: `ProductRank := RANKX(ALL(Dim_Product[Name]), [TotalSales])`. Now your products rank 1, 2, 3... perfectly."}
          />
        </div>
      </div>
    </div>
