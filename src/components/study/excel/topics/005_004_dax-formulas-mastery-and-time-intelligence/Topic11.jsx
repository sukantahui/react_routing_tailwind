      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🌊 Moving Averages & Rolling 12-Month Totals"} · Topic 11
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Moving averages and rolling totals: DATESINPERIOD and DATESBETWEEN"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Constructing rolling time-window analytics using DATESINPERIOD and DATESBETWEEN: 3-month moving averages, rolling 12-month (R12) revenue, and smoothing seasonal volatility"}. Master evaluation contexts, Context Transition, CALCULATE filter overriding, X-iterator engines, Time Intelligence comparisons, and ranking.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Rolling Window: CALCULATE([Measure], DATESINPERIOD(Dim_Calendar[Date], MAX(Dim_Calendar[Date]), -3, MONTH)) | DATESBETWEEN"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. DATESINPERIOD Dynamic Window Engine"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`DATESINPERIOD(DateCol, StartDate, NumberOfIntervals, Interval)`. Anchors on `MAX(Dim_Calendar[Date])` and looks backward by -3 MONTH or -12 MONTH."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Rolling 12-Month Total (R12 / TTM)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Sums the trailing 12 months of performance, eliminating seasonal quarterly peaks and valleys for executive trend analysis."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. 3-Month Moving Average (Smoothing)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`DIVIDE(CALCULATE([TotalSales], DATESINPERIOD(Dim_Calendar[Date], MAX(Dim_Calendar[Date]), -3, MONTH)), 3)`."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Rolling Window Engine: Current Anchor Date -> Step Back N Periods -> Dynamic Window Aggregation"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod14_11" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod14_11" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_11)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Moving Averages & Smoothing"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_11)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic11_Moving_averages_an"
            title={"Moving averages and rolling totals: DATESINPERIOD and DATESBETWEEN - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Enterprise Trailing 12-Month (TTM) Revenue"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Constructs `TTM_Revenue := CALCULATE([TotalSales], DATESINPERIOD(Dim_Calendar[Date], MAX(Dim_Calendar[Date]), -12, MONTH))` for executive board reporting."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: 3-Month Moving Average Cost Smoother"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Smooths seasonal electricity and utility expenditure spikes using 3-month rolling averages."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: 30-Day Rolling Scrap Rate Average"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Tracks trailing 30-day moving average scrap percentages to evaluate Six Sigma process stability."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: 90-Day Rolling On-Time Delivery SLA"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Monitors 90-day moving average carrier delivery punctuality."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                In the formula bar, type your measure name and formula: <code className="text-cyan-300 font-mono font-bold">{"Rolling Window: CALCULATE([Measure], DATESINPERIOD(Dim_Calendar[Date], MAX(Dim_Calendar[Date]), -3, MONTH)) | DATESBETWEEN"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Omitting MAX(Date) Anchor in DATESINPERIOD"}</td>
                  <td className="py-3 px-4">{"Passing a static date instead of `MAX(Dim_Calendar[Date])` freezes the rolling window to a fixed historical date."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always anchor with `MAX(Dim_Calendar[Date])` or `LASTDATE(Dim_Calendar[Date])`."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Distortion in Initial Historical Months (Ramp-Up Glitch)"}</td>
                  <td className="py-3 px-4">{"Calculating a 12-month rolling average in Month 2 averages only 2 months of data, distorting trend charts."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Check if 12 full months of historical data exist before calculating."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Using DATESBETWEEN with Hardcoded Dates"}</td>
                  <td className="py-3 px-4">{"Hardcoding start/end dates prevents the rolling window from moving dynamically with the calendar."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use `DATESINPERIOD` for dynamic relative rolling windows."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Rolling 12-Month Total (TTM)"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Trailing 12-month revenue sum."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"CALCULATE([Sales], DATESINPERIOD(Calendar[Date], MAX(Calendar[Date]), -12, MONTH))"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"3-Month Moving Average"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Smooths short-term monthly volatility."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"DIVIDE(CALCULATE([Sales], DATESINPERIOD(Calendar[Date], MAX(Calendar[Date]), -3, MONTH)), 3)"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Anchor on MAX(Date)"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Ensures the window slides dynamically with active pivot rows."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"MAX(Dim_Calendar[Date])"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why is `MAX(Dim_Calendar[Date])` used as the anchor date in `DATESINPERIOD()` formulas?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"How do Trailing 12-Month (TTM) revenue metrics help executives see true underlying business growth by stripping out seasonality?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"What is the difference between `DATESINPERIOD()` (relative sliding window) and `DATESBETWEEN()` (fixed boundary window)?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Moving averages and rolling totals: DATESINPERIOD and DATESBETWEEN - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Rolling 12-Month (TTM) is the gold standard of corporate finance. To build it: `CALCULATE([TotalSales], DATESINPERIOD(Dim_Calendar[Date], MAX(Dim_Calendar[Date]), -12, MONTH))`. It removes all seasonal dips and shows true underlying growth."}
          />
        </div>
      </div>
    </div>
