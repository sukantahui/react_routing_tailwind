      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"👑 The CALCULATE Function (Filter Modification)"} · Topic 3
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"The Crown Jewel of DAX"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"The CALCULATE function: Modifying and overriding filter contexts dynamically"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Mastering CALCULATE, the most powerful and fundamental function in DAX: modifying, overriding, injecting, and removing filter contexts to perform advanced analytical calculations"}. Master evaluation contexts, Context Transition, CALCULATE filter overriding, X-iterator engines, Time Intelligence comparisons, and ranking.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"CALCULATE(Expression, Filter1, Filter2, ...) | CALCULATE([TotalSales], Dim_Product[Category] = \"Electronics\", Dim_Calendar[Year] = 2026)"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The Master Filter Overrider"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"CALCULATE evaluates a given measure expression in a modified filter context created by its filter arguments."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Filter Injection & Replacement"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"If a filter argument specifies a column already in the Filter Context, CALCULATE OVERRIDES the existing filter with the new condition."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Boolean vs Table Filter Arguments"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Simple Boolean filters (`Dim[Category] = \"Valves\"`) are converted internally by DAX into `FILTER(ALL(Dim[Category]), Dim[Category] = \"Valves\")`."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"CALCULATE Engine: Incoming Filter Context -> Filter Overrides / Injections -> Target Measure Evaluation"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod14_3" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod14_3" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_3)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"The Crown Jewel of DAX"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_3)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic3_The_CALCULATE_func"
            title={"The CALCULATE function: Modifying and overriding filter contexts dynamically - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Selective High-Margin Division Sales Metric"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Constructs `CorporateSales := CALCULATE([TotalSales], Dim_Customer[Tier] = \"Enterprise\", Dim_Product[Status] = \"Active\")`."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Exempt Inter-State Tax Turnover"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Calculates total tax-exempt sales using `CALCULATE([TotalRevenue], Dim_TaxRates[Rate] = 0)` across all regional branches."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Severe Scrap Defect Volume Measure"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Isolates grade-A casting defects using `CALCULATE([TotalScrapMT], Dim_Defects[Severity] = \"Critical\")`."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Express Highway Transit Volume"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Measures express corridor shipments: `CALCULATE([ShipmentCount], Dim_ServiceType[Name] = \"Express\")`."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                In the formula bar, type your measure name and formula: <code className="text-cyan-300 font-mono font-bold">{"CALCULATE(Expression, Filter1, Filter2, ...) | CALCULATE([TotalSales], Dim_Product[Category] = \"Electronics\", Dim_Calendar[Year] = 2026)"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Attempting Multiple Columns in Simple Boolean Filter"}</td>
                  <td className="py-3 px-4">{"Writing `CALCULATE([Sales], Product[Price] > Product[Cost])` throws a DAX syntax error (simple Boolean filters cannot compare two columns directly)."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Wrap multi-column comparisons inside the `FILTER()` table function: `CALCULATE([Sales], FILTER(Product, Product[Price] > Product[Cost]))`."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Measure References in Boolean Filter Argument"}</td>
                  <td className="py-3 px-4">{"Writing `CALCULATE([Sales], [Margin%] > 0.2)` throws an error (measures cannot be used as simple Boolean filter parameters)."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use `FILTER(Dim_Product, [Margin%] > 0.2)`."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Order of Operations Confusion"}</td>
                  <td className="py-3 px-4">{"CALCULATE modifies the filter context BEFORE evaluating the primary expression."}</td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"The King of DAX"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Over 90% of complex DAX calculations rely on CALCULATE."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"CALCULATE([Measure], Conditions...)"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Multiple Conditions = AND"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Passing multiple filter arguments applies a logical AND condition."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"CALCULATE([Sales], Cond1, Cond2)"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Logical OR in CALCULATE"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Use the double pipe || operator for OR conditions."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"CALCULATE([Sales], Cond1 || Cond2)"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why is CALCULATE considered the single most important function in the entire DAX language?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"How does CALCULATE resolve conflicts when an incoming pivot slicer filters for Year=2025 but the formula specifies Year=2026?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"Why does DAX reject comparing two columns directly inside a simple CALCULATE Boolean filter?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"The CALCULATE function: Modifying and overriding filter contexts dynamically - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"If you only learn one function in DAX, make it CALCULATE. It allows you to change the rules of the game: you can ignore slicers, inject new filters, and compare anything against anything."}
          />
        </div>
      </div>
    </div>
