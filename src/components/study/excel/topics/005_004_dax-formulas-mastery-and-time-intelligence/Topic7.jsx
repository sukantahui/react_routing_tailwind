      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🔀 USERELATIONSHIP (Active vs Inactive Relationships)"} · Topic 7
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Role-Playing Dimensions & Inactive Paths"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Handling multiple relationships with USERELATIONSHIP (Active vs Inactive relationships)"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Handling multiple date relationships (Role-Playing Dimensions) using USERELATIONSHIP: analyzing Order Date, Ship Date, and Delivery Date against a single master Calendar table"}. Master evaluation contexts, Context Transition, CALCULATE filter overriding, X-iterator engines, Time Intelligence comparisons, and ranking.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"USERELATIONSHIP(Fact_Table[Date_FK], Dim_Calendar[Date_PK]) | ShipSales := CALCULATE([TotalSales], USERELATIONSHIP(Fact_Sales[ShipDate], Dim_Calendar[Date]))"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The Single Active Relationship Constraint"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Power Pivot permits only ONE active relationship between two tables (solid line). Additional relationships remain Inactive (dotted lines)."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Dynamic Relationship Activation with USERELATIONSHIP"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`USERELATIONSHIP()` temporarily deactivates the default active relationship and activates the specified inactive dotted-line relationship during formula execution."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Eliminating Multiple Redundant Calendar Tables"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Allows a single master Calendar table to analyze sales by Order Date, Ship Date, Due Date, and Return Date simultaneously."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Role-Playing Date Architecture: Single Calendar Table -> Active (Order Date) vs Inactive (Ship Date) Linkages"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod14_7" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod14_7" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_7)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Role-Playing Dimensions & Inactive Paths"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_7)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic7_Handling_multiple_"
            title={"Handling multiple relationships with USERELATIONSHIP (Active vs Inactive relationships) - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Order Date vs Delivery Date Revenue Recognition"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Builds `OrdersBooked := [TotalSales]` (Active: OrderDate) and `RevenueRecognized := CALCULATE([TotalSales], USERELATIONSHIP(Fact_Sales[DeliveryDate], Dim_Calendar[Date]))`."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Invoice Date vs Payment Realization Date Audit"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Compares statutory tax liabilities on Invoice Date versus actual bank realization dates."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Casting Date vs Quality Inspection Sign-Off Date"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Tracks manufacturing batch tonnage by Production Date vs Lab Clearance Date."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Dispatch Date vs Proof-of-Delivery Date SLAs"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Measures consignment volume by Dispatch Date vs Customer Delivery Date."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                In the formula bar, type your measure name and formula: <code className="text-cyan-300 font-mono font-bold">{"USERELATIONSHIP(Fact_Table[Date_FK], Dim_Calendar[Date_PK]) | ShipSales := CALCULATE([TotalSales], USERELATIONSHIP(Fact_Sales[ShipDate], Dim_Calendar[Date]))"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Using USERELATIONSHIP Outside of CALCULATE"}</td>
                  <td className="py-3 px-4">{"Writing `USERELATIONSHIP()` as a standalone measure throws a DAX syntax error."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"USERELATIONSHIP can ONLY be passed as a filter argument inside `CALCULATE()`."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Duplicating 5 Calendar Tables (Anti-Pattern)"}</td>
                  <td className="py-3 px-4">{"Creating `Calendar_Order`, `Calendar_Ship`, `Calendar_Pay` tables bloats the model and fragments slicers."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use 1 Master Calendar table with inactive relationships and `USERELATIONSHIP()`."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Data Type Mismatch on Inactive Relationship Keys"}</td>
                  <td className="py-3 px-4">{"Key data types differing between foreign and primary keys breaks activation."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Ensure both key columns share identical data types in Power Query."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"USERELATIONSHIP Syntax"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Temporarily activates an inactive dotted-line relationship."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"CALCULATE([Measure], USERELATIONSHIP(FK, PK))"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"1 Master Calendar"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"The enterprise standard for clean role-playing date modeling."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Single Date Dimension for all date fields"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Solid vs Dotted Lines"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Visual indicator in Power Pivot Diagram View."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Solid = Active by default; Dotted = Inactive"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why does Power Pivot permit only ONE active relationship between two tables at a time?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"How does `USERELATIONSHIP()` allow a single Calendar table to analyze sales by Order Date, Ship Date, and Payment Date?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"Why is creating multiple redundant calendar tables considered an amateur modeling anti-pattern?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Handling multiple relationships with USERELATIONSHIP (Active vs Inactive relationships) - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Do not create 3 different calendar tables! Create ONE Calendar table, link OrderDate (Active - solid line) and ShipDate (Inactive - dotted line). Then write: `SalesByShipDate := CALCULATE([TotalSales], USERELATIONSHIP(Fact[ShipDate], Calendar[Date]))`. Pure elegance."}
          />
        </div>
      </div>
    </div>
