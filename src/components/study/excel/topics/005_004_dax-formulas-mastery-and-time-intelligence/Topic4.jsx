      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🌐 ALL, ALLEXCEPT, ALLSELECTED & REMOVEFILTERS"} · Topic 4
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Filter Removal & Market Share Math"}
            </span>
          ...
          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Controlling and removing filter context using ALL, ALLEXCEPT, ALLSELECTED, and REMOVEFILTERS: calculating % of Total, category contributions, and market share ratios"}. Master evaluation contexts, Context Transition, CALCULATE filter overriding, X-iterator engines, Time Intelligence comparisons, and ranking.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Filter Modifiers: ALL(Table/Col) | ALLEXCEPT(Table, Col) | ALLSELECTED(Table/Col) | REMOVEFILTERS(Table/Col)"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. ALL (Total Filter Removal)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Ignores ALL filters on the specified table or column, returning the global grand total regardless of row headers or slicers."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. REMOVEFILTERS (Clear Intent Modifier)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Modern DAX alias for `ALL()` used inside `CALCULATE` to explicitly declare that filters are being stripped."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. ALLSELECTED (Visual Slicer Total)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Removes row/column grid filters but PRESERVES user slicer selections, essential for calculating accurate '% of Filtered Slicer Total'."}</p>
            </div>
            
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"4. ALLEXCEPT (Preserve Specific Dimension)"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Removes all filters on a table EXCEPT for the specified column(s), perfect for computing '% of Department Total'."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Filter Removal Architecture: CALCULATE([Sales], ALL(Dim_Product)) -> Total Category Denominator"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod14_4" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod14_4" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_4)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Filter Removal & Market Share Math"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_4)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            title={"Modifying filters with ALL, ALLEXCEPT, ALLSELECTED, and REMOVEFILTERS - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Product % of Total Revenue Contribution"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Computes `% of Total := DIVIDE([TotalSales], CALCULATE([TotalSales], ALL(Dim_Product)))` to measure product share."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Regional Tax Contribution % Metric"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Calculates regional tax liability as percentage of national grand total using `REMOVEFILTERS(Dim_Branch)`."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Departmental Scrap Share Ratio"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses `ALLEXCEPT(Dim_Machinery, Dim_Machinery[Department])` to calculate each machine's % share of its department's scrap."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: % of Sliced Regional Freight Volume"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Uses `ALLSELECTED(Dim_Routes)` so freight % shares dynamically sum to 100% when users select arbitrary route subsets."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                In the formula bar, type your measure name and formula: <code className="text-cyan-300 font-mono font-bold">{"Filter Modifiers: ALL(Table/Col) | ALLEXCEPT(Table, Col) | ALLSELECTED(Table/Col) | REMOVEFILTERS(Table/Col)"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Using ALL() When ALLSELECTED() is Required"}</td>
                  <td className="py-3 px-4">{"Using `ALL()` causes '% of Total' to divide by national sales even when the user explicitly filtered for 'Eastern Region', displaying 3.2% instead of 25.0%."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use `ALLSELECTED()` so percentages sum to 100% of the active slicer selection."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Passing Table to ALL() Instead of Column"}</td>
                  <td className="py-3 px-4">{"Writing `ALL(Fact_Sales)` wipes filters across all dimensions, which is much slower than stripping a single column `ALL(Dim_Product[Category])`."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Target specific dimension columns: `ALL(Dim_Product[Category])`."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Division by Zero on Empty Categories"}</td>
                  <td className="py-3 px-4">{"Missing safe division wrap."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always wrap in `=DIVIDE(Numerator, Denominator, 0)`."}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"% of Grand Total"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Standard formula for % of grand national total."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"DIVIDE([Sales], CALCULATE([Sales], ALL(Dim_Product)))"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"% of Sliced Total"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Percentages always sum to 100% within active slicers."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"DIVIDE([Sales], CALCULATE([Sales], ALLSELECTED(Dim_Product)))"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"% of Parent Category"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Subcategory % share within its parent category."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"DIVIDE([Sales], CALCULATE([Sales], ALLEXCEPT(Product, Product[Category])))"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"What is the crucial behavioral difference between `ALL()` and `ALLSELECTED()` when users click slicer buttons?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"How does `ALLEXCEPT()` calculate the percentage share of a product within its parent department?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"Why is `REMOVEFILTERS()` preferred by modern DAX architects over `ALL()` inside CALCULATE statements?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Modifying filters with ALL, ALLEXCEPT, ALLSELECTED, and REMOVEFILTERS - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"To calculate % of Total: Write `DIVIDE([TotalSales], CALCULATE([TotalSales], ALLSELECTED(Dim_Product)))`. Now when users slice by Region or Year, the percentages will beautifully sum to exactly 100% every time."}
          />
        </div>
      </div>
    </div>
