      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2.5 mb-4">
            <span className="px-3.5 py-1 rounded-full bg-sky-950/80 border border-sky-700/60 text-sky-300 text-xs font-bold uppercase tracking-wider shadow-inner">
              {"🏦 Semi-Additive Measures (Inventory & Bank Balances)"} · Topic 12
            </span>
            <span className="px-3 py-1 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-xs font-semibold">
              {"Non-Additive Financial Metrics"}
            </span>
          ...
          <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-sky-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent leading-tight">
            {"Semi-additive measures: Opening and Closing balances for inventory and banking (CLOSINGBALANCEMONTH)"}
          </h1>

          <p className="text-slate-300 text-base sm:text-lg mt-4 leading-relaxed max-w-4xl">
            {"Mastering Semi-Additive DAX measures: handling non-additive stock-level and balance-sheet metrics (Inventory Stock, Bank Account Balances, Headcount) using CLOSINGBALANCEMONTH and LASTDATE"}. Master evaluation contexts, Context Transition, CALCULATE filter overriding, X-iterator engines, Time Intelligence comparisons, and ranking.
          </p>
        ...
        >
          ...
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/90 font-mono text-sm sm:text-base text-sky-300 overflow-x-auto shadow-inner">
            {"Semi-Additive DAX: CLOSINGBALANCEMONTH([Measure], Dim_Calendar[Date]) | OPENINGBALANCEMONTH | CALCULATE([Measure], LASTDATE(Dim_Calendar[Date]))"}
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"1. The Semi-Additive Problem"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Sales are additive over time (Jan Sales + Feb Sales = Total Sales). Bank Balances and Inventory are NON-ADDITIVE over time (Jan Balance + Feb Balance is meaningless nonsense!)."}</p>
            </div>
            
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"2. Closing Balance Architecture"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"When aggregating across time, semi-additive measures must return the CLOSING SNAPSHOT of the last active date in the period rather than summing."}</p>
            </div>
            
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
              <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider">{"3. Native Balance Functions"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"`CLOSINGBALANCEMONTH([StockValue], Dim_Calendar[Date])` and `CLOSINGBALANCEYEAR()` automatically extract the ending period snapshot."}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              ...
              Visual Dataflow: {"Semi-Additive Aggregation Engine: Additive across Products & Categories, Non-Additive across Time (Closing Snapshot)"}
            </h2>
          ...
          <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 flex flex-col items-center justify-center overflow-x-auto shadow-inner">
            <svg viewBox="0 0 820 220" className="w-full max-w-4xl h-auto text-slate-200 select-none font-sans">
              <defs>
                <linearGradient id="gradFlowMod14_12" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#0284c7" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="#0369a1" stopOpacity="0.8" />
                </linearGradient>
                <marker id="arrowMod14_12" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              ...
              <path d="M 245 110 L 305 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_12)" fill="none" />
              ...
              <g transform="translate(315, 30)">
                ...
                <text x="125" y="75" textAnchor="middle" fill="#7dd3fc" fontSize="13" fontFamily="monospace" fontWeight="bold">{"Non-Additive Financial Metrics"}</text>
              ...
              <path d="M 570 110 L 630 110" stroke="#38bdf8" strokeWidth="2.5" markerEnd="url(#arrowMod14_12)" fill="none" />
        ...
        >
          ...
          <ExcelFileLoader
            ...
            sheetName="Topic12_Semi_additive_meas"
            title={"Semi-additive measures: Opening and Closing balances for inventory and banking (CLOSINGBALANCEMONTH) - Interactive Practice Grid"}
            rowsPerPage={10}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key="0" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              ...
              <h3 className="font-bold text-white text-base">{"Swadeep Banerjee: Enterprise Daily Cash Balance Console"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Builds `ClosingCashBalance := CLOSINGBALANCEMONTH([CashBalance], Dim_Calendar[Date])` ensuring quarterly cash reports show ending quarter cash, not the sum of 90 daily balances."}</p>
            </div>
            ...
            <div key="1" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-amber-400">{"Case 2 · Corporate Inventory Controller"}</span>
                <span className="text-xs font-mono text-slate-400">{"Shyamnagar Plant"}</span>
              </div>
              <h3 className="font-bold text-white text-base">{"Tuhina Mukherjee: Factory Warehouse Ending Inventory Valuation"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Constructs semi-additive measures tracking warehouse closing stock values at month-end."}</p>
            </div>
            ...
            <div key="2" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-amber-400">{"Case 3 · HR Analytics Lead"}</span>
                <span className="text-xs font-mono text-slate-400">{"Ichapur Works"}</span>
              </div>
              <h3 className="font-bold text-white text-base">{"Abhronila Das: Active Plant Headcount Metric"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Tracks active monthly employee headcount snapshots across manufacturing shifts."}</p>
            </div>
            ...
            <div key="3" className="p-5 rounded-xl bg-slate-950/70 border border-slate-800 hover:border-amber-500/40 transition-all duration-300 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-amber-400">{"Case 4 · Logistics Asset Manager"}</span>
                <span className="text-xs font-mono text-slate-400">{"Naihati Logistics"}</span>
              </div>
              <h3 className="font-bold text-white text-base">{"Debangshu Roy: Active Fleet Truck Availability Snapshot"}</h3>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">{"Monitors closing monthly operational fleet vehicle counts."}</p>
            </div>
        ...
        >
          ...
          <div className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1.5">
              ...
              <p className="text-slate-300 leading-relaxed">
                In the formula bar, type your measure name and formula: <code className="text-cyan-300 font-mono font-bold">{"Semi-Additive DAX: CLOSINGBALANCEMONTH([Measure], Dim_Calendar[Date]) | OPENINGBALANCEMONTH | CALCULATE([Measure], LASTDATE(Dim_Calendar[Date]))"}</code>.
              </p>
            </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                ...
                <tr key="0" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Summing Bank Balances Across Months (Fatal Accounting Error)"}</td>
                  <td className="py-3 px-4">{"Using standard `=SUM(Bank[Balance])` adds January ₹10L + February ₹10L = ₹20L (inventing money that does not exist)."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Always use semi-additive DAX: `CLOSINGBALANCEMONTH([BankBalance], Dim_Calendar[Date])`."}</td>
                </tr>
                
                <tr key="1" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Blank Closing Balance on Incomplete Months"}</td>
                  <td className="py-3 px-4">{"If current month has no transactions on the 31st, `CLOSINGBALANCEMONTH` can return blank."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{"Use `CALCULATE([Balance], LASTNONBLANK(Dim_Calendar[Date], [Balance]))` to find the true last active day."}</td>
                </tr>
                
                <tr key="2" className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-rose-300">{"Confusing Opening Balance with Closing Balance of Prior Month"}</td>
                  <td className="py-3 px-4">{"Opening balance of March is mathematically identical to closing balance of February."}</td>
                  <td className="py-3 px-4 font-mono text-cyan-300">{undefined}</td>
                </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Closing Month Balance"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Returns the ending balance on the last day of the month."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"CLOSINGBALANCEMONTH([Balance], Dim_Calendar[Date])"}</kbd>
            </div>
            
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Last Non-Blank Balance"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"Handles months where transactions ended before the 31st."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"CALCULATE([Balance], LASTNONBLANK(Calendar[Date], [Balance]))"}</kbd>
            </div>
            
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-2">
              <div className="font-bold text-purple-300 flex items-center gap-2">
                <span>⚡</span> {"Additive vs Semi-Additive"}
              </div>
              <p className="text-slate-300 leading-relaxed">{"The fundamental taxonomy of financial measures."}</p>
              <kbd className="inline-block px-2 py-0.5 rounded bg-slate-800 border border-slate-700 font-mono text-xs text-cyan-300 mt-1">{"Additive = Sum across all dims; Semi-Additive = Sum across dims, Snapshot over time"}</kbd>
            </div>
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-300">
            ...
            <div key="0" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 1: {"Why is standard `SUM()` completely invalid for calculating Bank Balances or Inventory stock levels across multiple months?"}
              </h3>
            ...
            <div key="1" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 2: {"What does `CLOSINGBALANCEMONTH()` do when viewing an entire year in a PivotTable?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
            ...
            <div key="2" className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
              <h3 className="font-bold text-white flex items-center gap-2">
                <span className="text-teal-400">💭</span> Question 3: {"How does `LASTNONBLANK()` prevent blank results on weekends and bank holidays?"}
              </h3>
              <p className="text-slate-400 leading-relaxed">
                Reflect on the evaluation context dynamics and functional programming paradigm.
              </p>
        ...
        <div ref={(el) => (sectionsRef.current[10] = el)} className="reveal-section">
          <FAQTemplate
            title={"Semi-additive measures: Opening and Closing balances for inventory and banking (CLOSINGBALANCEMONTH) - Frequently Asked Questions"}
            questions={questions}
          />
        </div>
        ...
        <div ref={(el) => (sectionsRef.current[11] = el)} className="reveal-section">
          <Teacher
            note={"Never use standard SUM for Inventory or Bank Balances! If you have ₹10 Lakhs in January and ₹10 Lakhs in February, you have ₹10 Lakhs—not ₹20 Lakhs! Always use CLOSINGBALANCEMONTH."}
          />
        </div>
      </div>
    </div>
