    ...
    <div className="dark bg-slate-950 text-slate-100 min-h-screen py-8 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-5xl mx-auto space-y-10">
        <header ref={(el) => (sectionsRef.current[0] = el)} className="rounded-3xl p-6 sm:p-10 bg-slate-900 border border-slate-800">
          <span className="px-3 py-1 rounded-full bg-emerald-950 text-emerald-300 text-xs font-bold uppercase">DGET · Topic 6</span>
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-300 mt-4">Mastering DGET</h1>
          <p className="text-slate-300 mt-4 text-base sm:text-lg">Extracts single unique record matching criteria.</p>
        </header>
        ...
        <section ref={(el) => (sectionsRef.current[1] = el)} className="rounded-3xl p-6 sm:p-8 bg-slate-900 border border-slate-800">
          ...
          <div className="bg-slate-950 p-4 rounded-xl font-mono text-emerald-400 border border-slate-800">=DGET(database, field, criteria)</div>
        </section>
        ...
        {questions && (
          <div className="max-w-5xl mx-auto">
            <FAQTemplate title="DGET - Concept Checks & Questions" questions={questions} />
          </div>
        )}
