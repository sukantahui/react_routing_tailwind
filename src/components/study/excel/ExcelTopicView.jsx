...
========================================================== */
class ExcelTopicView extends Component {
  constructor(props) {
    super(props);
  ...
  // ---------------------------------------------------------
  // LOAD PROGRESS + LOAD TOPIC + SCROLL
  // ---------------------------------------------------------
  componentDidMount() {
    this.loadProgress();
  ...
  // (Now dynamic using roadmapData.folder)
  // ---------------------------------------------------------
  loadProgress() {
    const { moduleSlug, topicIndex } = this.props;

    const storageKey = `${roadmapData.folder}-progress-${moduleSlug}`;
    let parsed = [];
  ...
  resetProgress = () => {
    ...
    localStorage.removeItem(`${roadmapData.folder}-progress-${moduleSlug}`);
    this.setState({ completedTopics: [] });
  };

  // ---------------------------------------------------------
  // Lazy load topic component (dynamic folder & moduleSlug)
  // ---------------------------------------------------------
  loadTopicComponent() {
    const { moduleSlug, topicIndex } = this.props;
...
/* ==========================================================
   WRAPPER → Inject Router Params into Class Component
========================================================== */
export default function ExcelTopicViewWithParams() {
