 ...
 * Shows the balanced tree with O(n) work per level.
 * Recurrence: T(n) = 2T(n/2) + O(n) → O(n log n)
 */
public class RecursionTreeMergeSort {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Each level does O(n) work");
        System.out.println("Total: n * (log₂(n) + 1) = O(n log n)");

        System.out.println("\nTracing calls (showing tree structure):");
        mergeCalls = 0;
        depth = 0;
        maxDepth = 0;
        mergeSort(arr, 0, arr.length - 1);
        System.out.println("Total merge calls: " + mergeCalls);
