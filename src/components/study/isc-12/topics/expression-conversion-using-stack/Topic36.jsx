...
// Local files
import postfixToPrefixAlgorithmJava from "./topic36_files/PostfixToPrefixAlgorithm.java?raw";
...
function postfixToPrefixWithTrace(postfix) {
  const traces = [];
  let step = 0;
  const stack = [];
  ...
  for (const token of tokens) {
    ...
    if (token.match(/^[a-zA-Z0-9]+$/)) {
      ...
      const prefix = `${token} ${left} ${right}`;
      stack.push(prefix);
      action = `Pop right '${right}', left '${left}', combine as '${prefix}', push result`;
    }
  ...
  const finalPrefix = stack.length === 1 ? stack[0] : '';
    ...
    <div className="mt-4 p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-2">
          ...
          {output !== '—' && output !== '' && (
            <div className="mb-2">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Current Prefix</h3>
...
export default function Topic36() {
  ...
  const handleConvert = () => {
    setError('');
    try {
      const result = postfixToPrefixWithTrace(postfix);
  ...
  const javaCode = typeof postfixToPrefixAlgorithmJava === 'string'
    ? postfixToPrefixAlgorithmJava
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 space-y-12">
        ...
        <header className="animate-fadeUp">
          ...
          <p className="mt-2 text-lg text-gray-600 dark:text-gray-400">
            A complete algorithm to transform postfix expressions to prefix using a stack.
          </p>
        </header>
        ...
        <section className="animate-fadeUp delay-100">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/10 dark:hover:shadow-indigo-400/10">
            ...
            <div className="mt-4 space-y-4">
              <p>
                The algorithm for converting postfix to prefix is the mirror of prefix‑to‑postfix conversion.
                We use a <strong>stack of strings</strong> to hold sub‑expressions. Scanning from <strong>left to right</strong>,
                operands are pushed, and when an operator is encountered, we pop two operands (first popped is the
                <strong>right</strong> operand, second is the <strong>left</strong>), combine them in prefix order
                (<code>operator + " " + left + " " + right</code>), and push the result back onto the stack.
              </p>
              <p>
                After processing all tokens, the stack contains exactly one string – the prefix expression.
              </p>
              <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 shadow-sm">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  ...
                  but the combination order is <code>operator + left + right</code> instead of <code>left + operator + right</code>.
                </p>
              </div>
            </div>
          </div>
...
{`algorithm postfixToPrefix(postfix):
    ...
    tokens = split(postfix)   // space-separated tokens
    ...
    for each token in tokens:
        if token is operand:
            ...
            right = stack.pop()   // first popped is right operand
            left = stack.pop()    // second popped is left operand
            prefix = token + " " + left + " " + right
            stack.push(prefix)
    ...
    return stack.pop()`}
            ...
            <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">
              This algorithm is concise and mirrors postfix‑to‑infix with a different combination order.
            </p>
          </div>
        </section>
        ...
        <section className="animate-fadeUp delay-300">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-blue-500/10 dark:hover:shadow-blue-400/10">
            ...
            <div className="mt-4 space-y-5">
              ...
              <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                <h3 className="font-bold text-gray-700 dark:text-gray-300">Step 2: Scan from Left to Right</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  Process each token from the beginning of the postfix expression to the end.
                </p>
              </div>
              ...
              <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                ...
                <ul className="list-disc pl-5 text-sm text-gray-600 dark:text-gray-400 mt-1 space-y-1">
                  <li>Pop the <strong>right</strong> operand from the stack (first popped).</li>
                  <li>Pop the <strong>left</strong> operand from the stack (second popped).</li>
                  <li>Build a new string: <code>operator + " " + left + " " + right</code>.</li>
              ...
              <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  After processing all tokens, the stack will contain exactly one element – the prefix expression.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="animate-fadeUp delay-500">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-amber-500/10 dark:hover:shadow-amber-400/10">
            ...
            <ul className="mt-4 list-disc pl-6 space-y-2 text-gray-700 dark:text-gray-300">
              ...
              <li><strong>Expression evaluation</strong> – you may have a postfix expression but need prefix for evaluation.</li>
              <li><strong>Code generation</strong> – some code generators produce postfix, but the target expects prefix.</li>
            ...
            <div className="mt-3 bg-amber-50 dark:bg-amber-900/20 p-3 rounded-lg border border-amber-200 dark:border-amber-800">
              <p className="text-sm">
                <span className="font-medium">🏫 Classroom story:</span> Debangshu from Naihati was converting postfix
                to prefix for his compiler project and found this algorithm very helpful.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="animate-fadeUp delay-200">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-red-500/10 dark:hover:shadow-red-400/10">
            ...
            <ul className="mt-4 list-disc pl-5 space-y-2 text-gray-700 dark:text-gray-300">
              ...
              <li>Swapping the order of operands – first pop is right, second is left.</li>
        ...
        <section className="animate-fadeUp delay-100">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800">
            ...
            <div className="mt-4">
              <JavaFileLoader
                fileModule={javaCode}
                title="PostfixToPrefixAlgorithm.java"
                highlightLines={[]}
              />
            </div>
            <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">
        ...
        <div className="animate-fadeUp delay-200">
          <Teacher
            note={
              "Postfix‑to‑prefix is a great example of how the same stack technique can be adapted for different conversions. In Barrackpore, I tell my students: 'Just change the combination order, and you're done.' Use the interactive tool to visualise the stack and build confidence."
            }
          />
        </div>
        ...
        <div className="animate-fadeUp delay-300">
          <FAQTemplate
            title="Postfix to Prefix Algorithm – FAQs"
            questions={questions}
          />
        </div>
