...
// Local files
import prefixToPostfixAlgorithmJava from "./topic32_files/PrefixToPostfixAlgorithm.java?raw";
...
function prefixToPostfixWithTrace(prefix) {
  const traces = [];
  let step = 0;
  const stack = [];
  ...
  for (let i = tokens.length - 1; i >= 0; i--) {
    ...
    if (token.match(/^[a-zA-Z0-9]+$/)) {
      ...
      const postfix = `${left} ${right} ${token}`;
      stack.push(postfix);
      action = `Pop left '${left}', right '${right}', combine as '${postfix}', push result`;
    }
  ...
  const finalPostfix = stack.length === 1 ? stack[0] : '';
  return { traces, postfix: finalPostfix };
}
    ...
    <div className="mt-4 p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-2">
          ...
          {output !== '—' && output !== '' && (
            <div className="mb-2">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Current Postfix</h3>
...
export default function Topic32() {
  ...
  const handleConvert = () => {
    setError('');
    try {
      const result = prefixToPostfixWithTrace(prefix);
  ...
  const javaCode = typeof prefixToPostfixAlgorithmJava === 'string'
    ? prefixToPostfixAlgorithmJava
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 space-y-12">
        ...
        <header className="animate-fadeUp">
          ...
          <p className="mt-2 text-lg text-gray-600 dark:text-gray-400">
            A complete algorithm to transform prefix expressions to postfix using a stack.
          </p>
        </header>
        ...
        <section className="animate-fadeUp delay-100">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/10 dark:hover:shadow-indigo-400/10">
            ...
            <div className="mt-4 space-y-4">
              <p>
                The algorithm for converting prefix to postfix is symmetric to prefix‑to‑infix conversion.
                We use a <strong>stack of strings</strong> to hold sub‑expressions. Scanning from <strong>right to left</strong>,
                operands are pushed, and when an operator is encountered, we pop two operands (first popped is the
                <strong>left</strong> operand, second is the <strong>right</strong>), combine them in postfix order
                (<code>left + " " + right + " " + operator</code>), and push the result back onto the stack.
              </p>
              <p>
                After processing all tokens, the stack contains exactly one string – the postfix expression.
              </p>
              <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 shadow-sm">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  <span className="font-medium">💡 Key insight:</span> The algorithm is identical to prefix‑to‑infix,
                  but the combination order is left + right + operator instead of '(' + left + operator + right + ')'.
                </p>
              </div>
            </div>
          </div>
...
{`algorithm prefixToPostfix(prefix):
    ...
    return stack.pop()`}
            ...
            <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">
              This algorithm is concise and mirrors prefix‑to‑infix with a different combination order.
            </p>
          </div>
        </section>
            ...
            <div className="mt-4 space-y-5">
              ...
              <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                ...
                <ul className="list-disc pl-5 text-sm text-gray-600 dark:text-gray-400 mt-1 space-y-1">
                  ...
                  <li>Build a new string: <code>left + " " + right + " " + operator</code>.</li>
              ...
              <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  After processing all tokens, the stack will contain exactly one element – the postfix expression.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800">
            ...
            {postfix && (
              <div className="mt-4 p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg">
                <div className="text-sm font-medium text-emerald-700 dark:text-emerald-300">✅ Postfix Result</div>
                <div className="mt-1 font-mono text-xl font-bold text-emerald-800 dark:text-emerald-200">{postfix}</div>
              </div>
            )}
        ...
        <section className="animate-fadeUp delay-500">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-amber-500/10 dark:hover:shadow-amber-400/10">
            ...
            <ul className="mt-4 list-disc pl-6 space-y-2 text-gray-700 dark:text-gray-300">
              ...
              <li><strong>Expression evaluation</strong> – you may have a prefix expression but a postfix evaluator.</li>
              <li><strong>Code generation</strong> – some code generators produce prefix, but the target architecture expects postfix.</li>
            ...
            <div className="mt-3 bg-amber-50 dark:bg-amber-900/20 p-3 rounded-lg border border-amber-200 dark:border-amber-800">
              <p className="text-sm">
                <span className="font-medium">🏫 Classroom story:</span> Swadeep from Barrackpore built a compiler
                that used prefix internally but needed postfix for code generation – this algorithm was exactly what he needed.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="animate-fadeUp delay-100">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-amber-500/10 dark:hover:shadow-amber-400/10">
            ...
            <ul className="mt-4 list-disc pl-5 space-y-2 text-gray-700 dark:text-gray-300">
              <li>Always scan from <strong>right to left</strong> – this is the key.</li>
        ...
        <section className="animate-fadeUp delay-200">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-red-500/10 dark:hover:shadow-red-400/10">
            ...
            <ul className="mt-4 list-disc pl-5 space-y-2 text-gray-700 dark:text-gray-300">
              ...
              <li>Swapping the order of operands – first popped is left, second is right.</li>
        ...
        <section className="animate-fadeUp delay-500">
          <div className="rounded-2xl bg-indigo-50 dark:bg-indigo-950/30 p-6 sm:p-8 border border-indigo-200 dark:border-indigo-800 transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/10 dark:hover:shadow-indigo-400/10">
            ...
            <ul className="mt-4 list-disc pl-5 space-y-1 text-gray-700 dark:text-gray-300">
              ...
              <li>Try converting <code>+ A * B C</code> to postfix manually.</li>
            </ul>
          </div>
        </section>
        ...
        <section className="animate-fadeUp delay-100">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800">
            ...
            <div className="mt-4">
              <JavaFileLoader
                fileModule={javaCode}
                title="PrefixToPostfixAlgorithm.java"
                highlightLines={[]}
              />
            </div>
            <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">
        ...
        <div className="animate-fadeUp delay-200">
          <Teacher
            note={
              "Prefix‑to‑postfix is a great example of how the same stack technique can be adapted for different conversions. In Barrackpore, I tell my students: 'Just change the combination order, and you're done.' Use the interactive tool to visualise the stack and build confidence."
            }
          />
        </div>
        ...
        <div className="animate-fadeUp delay-300">
          <FAQTemplate
            title="Prefix to Postfix Algorithm – FAQs"
            questions={questions}
          />
        </div>
