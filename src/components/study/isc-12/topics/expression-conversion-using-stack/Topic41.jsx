...
// Local files
import prefixEvaluationJava from "./topic41_files/PrefixEvaluation.java?raw";
...
// ---------- Prefix Evaluation Engine with Trace ----------
function evaluatePrefixWithTrace(prefix, values) {
  const traces = [];
  let step = 0;
  const stack = [];
  ...
  for (let i = tokens.length - 1; i >= 0; i--) {
    ...
    if (token.match(/^[a-zA-Z]+$/)) {
      ...
      // Operator - for prefix, first popped is LEFT operand
      ...
      action = `Pop left=${left}, right=${right}, apply '${token}' → ${resultVal}`;
      result = resultVal;
    }
    ...
    traces.push({
      ...
      result: result,
      scanDirection: '←' // Indicates scanning right to left
    });
  }
...
function VisualStack({ traces, currentStep }) {
  ...
  const scanDir = trace.scanDirection || '';
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-2">
          ...
          <div className="text-xs text-gray-400 dark:text-gray-500">
            <span className="font-medium">Scan direction:</span> Right to left (operands are pushed, operators pop left then right)
          </div>
        </div>
      </div>
    </div>
...
function PrefixEvaluator() {
  ...
  const handleEvaluate = () => {
    ...
    try {
      ...
      const resultData = evaluatePrefixWithTrace(expression, values);
  ...
  const exampleVarSets = ['A=5, B=3', 'A=2, B=3, C=4', 'A=10, B=5', 'A=2, B=3, C=4, D=5'];

  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
      <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">🧮 Prefix Expression Evaluator</h3>
...
// ---------- Main Component ----------
export default function Topic41() {
  const javaCode = typeof prefixEvaluationJava === 'string'
    ? prefixEvaluationJava
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 space-y-12">
        ...
        <header className="animate-fadeUp">
          ...
          <p className="mt-2 text-lg text-gray-600 dark:text-gray-400">
            Evaluating prefix expressions is the mirror image of postfix evaluation – scan right to left, and pop left first.
          </p>
        </header>
        ...
        <section className="animate-fadeUp delay-100">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/10 dark:hover:shadow-indigo-400/10">
            <h2 className="text-2xl font-semibold text-gray-800 dark:text-gray-200 flex items-center gap-2">
              ...
              Why Prefix is Perfect for Stack Evaluation
            </h2>
            <div className="mt-4 space-y-4">
              <p>
                Prefix notation (Polish notation) places operators <strong>before</strong> their operands.
                This makes evaluation straightforward with a stack, but the algorithm is the mirror of postfix evaluation.
              </p>
              <p>
                The algorithm is:
              </p>
              <ol className="list-decimal pl-6 space-y-2 text-gray-700 dark:text-gray-300">
                ...
                <li>If the token is an operator, pop the <strong>left</strong> operand, then the <strong>right</strong> operand, apply the operator, and push the result.</li>
              ...
              <p>
                ...
                The key difference from postfix is: <strong>first popped is the left operand</strong>.
              </p>
              <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 shadow-sm">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  <span className="font-medium">💡 Key insight:</span> Prefix evaluation is postfix evaluation in reverse – both in scanning direction and operand order.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="mt-4 space-y-4">
              <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  Split the prefix expression into tokens (operands and operators) separated by whitespace.
                </p>
              </div>
              <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                <h3 className="font-bold text-gray-700 dark:text-gray-300">Step 3: Scan Tokens from Right to Left</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  For each token from right to left:
                </p>
                <ul className="list-disc pl-5 text-sm text-gray-600 dark:text-gray-400 mt-1 space-y-1">
                  ...
                  <li><strong>Operator:</strong> Pop the <strong>left</strong> operand, then the <strong>right</strong> operand. Apply the operator, push the result.</li>
                </ul>
                <div className="mt-2 p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded border border-yellow-200 dark:border-yellow-800">
                  <p className="text-xs text-yellow-700 dark:text-yellow-300">
                    ⚠️ <span className="font-medium">Important:</span> In prefix, the first popped operand is the <strong>left</strong> operand.
                  </p>
                </div>
              </div>
              <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="mt-4 space-y-6">
              <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="mt-2 space-y-1 text-sm">
                  <p><span className="font-medium">Scan R→L:</span> 3 → push [3]</p>
                  <p><span className="font-medium">Scan R→L:</span> 2 → push [3, 2]</p>
                  <p><span className="font-medium">Scan R→L:</span> + → pop 2 (left), 3 (right) → 2+3=5 → push [5]</p>
              ...
              <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="mt-2 space-y-1 text-sm">
                  <p><span className="font-medium">Scan R→L:</span> 4 → push [4]</p>
                  <p><span className="font-medium">Scan R→L:</span> 3 → push [4, 3]</p>
                  <p><span className="font-medium">Scan R→L:</span> 2 → push [4, 3, 2]</p>
                  <p><span className="font-medium">Scan R→L:</span> + → pop 2 (left), 3 (right) → 2+3=5 → push [4, 5]</p>
                  <p><span className="font-medium">Scan R→L:</span> * → pop 5 (left), 4 (right) → 5*4=20 → push [20]</p>
              ...
              <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="mt-2 space-y-1 text-sm">
                  <p><span className="font-medium">Scan R→L:</span> C=4 → push [4]</p>
                  <p><span className="font-medium">Scan R→L:</span> B=3 → push [4, 3]</p>
                  <p><span className="font-medium">Scan R→L:</span> * → pop 3 (left), 4 (right) → 3*4=12 → push [12]</p>
                  <p><span className="font-medium">Scan R→L:</span> A=2 → push [12, 2]</p>
                  <p><span className="font-medium">Scan R→L:</span> + → pop 2 (left), 12 (right) → 2+12=14 → push [14]</p>
        ...
        <section className="animate-fadeUp delay-500">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-purple-500/10 dark:hover:shadow-purple-400/10">
            ...
            <div className="mt-4 space-y-3 text-gray-700 dark:text-gray-300">
              <p>
                Just like postfix evaluation, prefix evaluation uses a <strong>symbol table</strong> (a dictionary/map)
                to store variable values. When an operand token is a variable, we look up its value from the map and push it.
              </p>
              <p>
                The variable handling is identical to postfix – only the scanning direction and popping order are different.
              </p>
              <div className="bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg border border-purple-200 dark:border-purple-800">
                <p className="text-sm">
                  <span className="font-medium">💡 Tip:</span> The same symbol table approach works for both prefix and postfix evaluation.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="animate-fadeUp delay-100">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-amber-500/10 dark:hover:shadow-amber-400/10">
            ...
            <ul className="mt-4 list-disc pl-5 space-y-2 text-gray-700 dark:text-gray-300">
              ...
              <li>Remember: the first popped operand is the <strong>left</strong> operand.</li>
              <li>Validate that the expression is well‑formed (e.g., enough operands for each operator).</li>
        ...
        <section className="animate-fadeUp delay-200">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-red-500/10 dark:hover:shadow-red-400/10">
            ...
            <ul className="mt-4 list-disc pl-5 space-y-2 text-gray-700 dark:text-gray-300">
              <li><strong>Scanning left to right:</strong> Prefix evaluation requires right‑to‑left scanning.</li>
              <li><strong>Popping order:</strong> First popped is the <strong>left</strong> operand (not right).</li>
          ...
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800">
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                ...
                "I know the order of popping operands (left first).",
                "I can handle variables using a symbol table.",
                "I can implement prefix evaluation in Java.",
              ].map((item, idx) => (
        ...
        <section className="animate-fadeUp delay-100">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800">
            ...
            <div className="mt-4">
              <JavaFileLoader
                fileModule={javaCode}
                title="PrefixEvaluation.java"
                highlightLines={[]}
              />
            </div>
            <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">
              This program implements prefix evaluation with variable support and a trace mode.
            </p>
          </div>
        </section>
        ...
        <div className="animate-fadeUp delay-200">
          <Teacher
            note={
              "Prefix evaluation is the mirror image of postfix evaluation. In Barrackpore, I tell my students: 'Just reverse everything – scan from right to left, and pop left first.' The interactive tool helps visualise the stack as it grows and shrinks. Practice with different expressions to build intuition for the right‑to‑left flow."
            }
          />
        </div>
