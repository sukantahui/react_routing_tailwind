...
function infixToPrefixWithTrace(infix) {
  ...
  traces.push({ step: ++step, token: 'REVERSE', action: `Reverse infix: "${infix}" → "${reversed}"`, stack: '—', output: '—', reversed: reversed, swapped: null });

  const swapped = swapParentheses(reversed);
  traces.push({ step: ++step, token: 'SWAP', action: `Swap parentheses: "${reversed}" → "${swapped}"`, stack: '—', output: '—', reversed: reversed, swapped: swapped });

  let output = '';
  const stack = [];
  traces.push({ step: ++step, token: 'START', action: 'Start processing swapped expression', stack: '[]', output: '""', reversed: reversed, swapped: swapped });
  ...
  traces.push({ step: ++step, token: 'REVERSE', action: `Reverse output "${output}" → "${finalPrefix}"`, stack: '—', output: finalPrefix, reversed: reversed, swapped: swapped, stackArray: [], outputString: finalPrefix });
...
function VisualStack({ traces, currentStep }) {
  ...
  const currentChar = trace.currentChar || '';
  const action = trace.action || '';
  const swappedExpr = trace.swapped || '';
  const reversedExpr = trace.reversed || '';
    ...
    <div className="mt-6 p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2">
          <div className="mb-3">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Current Token</h3>
            <div className="text-lg font-mono font-bold text-indigo-600 dark:text-indigo-400">
              {currentChar === 'END' ? '⏹ End' : currentChar || '—'}
            </div>
          </div>
          <div className="mb-3">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Output (postfix‑like)</h3>
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Action</h3>
          ...
          <div className="mt-3 text-xs text-gray-500 dark:text-gray-400">
            <span className="font-medium">Swapped expression:</span> {swappedExpr}
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 space-y-12">
        ...
        <header className="animate-fadeUp">
          ...
          <p className="mt-2 text-lg text-gray-600 dark:text-gray-400">
            Interactive visual tool to trace the conversion step by step.
          </p>
        </header>
          ...
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/10 dark:hover:shadow-indigo-400/10">
            ...
            <div className="mt-4 space-y-4">
              <p>
                ...
                places the operator before its operands. It eliminates the need for parentheses and precedence rules.
              </p>
              <p>
                The conversion is a mirror of the postfix algorithm: <strong>reverse → swap parentheses → apply modified postfix → reverse again.</strong>
              </p>
              <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 shadow-sm">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  <span className="font-medium">💡 Key insight:</span> Equal‑precedence operators are <strong>not</strong> popped during the modified postfix step – this preserves right‑associativity.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800">
            ...
            {traces.length &gt; 0 && (
              <div className="mt-6">
                ...
                {/* Small table showing the step details (optional) */}
        ...
        {/* TRACE TABLE (kept for reference) */}
        {traces.length &gt; 0 && (
                ...
                <table className="w-full text-sm border-collapse">
                  <thead className="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300">
                    <tr>
                      ...
                      <th className="p-3 border border-gray-200 dark:border-gray-700 text-left font-mono">Stack</th>
                      <th className="p-3 border border-gray-200 dark:border-gray-700 text-left font-mono">Output</th>
                    </tr>
                  </thead>
                  <tbody className="text-gray-600 dark:text-gray-400">
                    {traces.map((trace) => (
                      <tr key={trace.step} className={`hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors duration-150 ${trace.step === currentStep + 1 ? 'bg-indigo-50 dark:bg-indigo-950/30' : ''}`}>
                        <td className="p-3 border border-gray-200 dark:border-gray-700 text-center">{trace.step}</td>
                        <td className="p-3 border border-gray-200 dark:border-gray-700 font-mono">{trace.token}</td>
                        <td className="p-3 border border-gray-200 dark:border-gray-700 text-xs">{trace.action}</td>
                        <td className="p-3 border border-gray-200 dark:border-gray-700 font-mono text-xs">{trace.stack}</td>
                        <td className="p-3 border border-gray-200 dark:border-gray-700 font-mono text-xs">{trace.output}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <section className="animate-fadeUp delay-400">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-amber-500/10 dark:hover:shadow-amber-400/10">
            ...
            <ol className="mt-4 list-decimal pl-6 space-y-2 text-gray-700 dark:text-gray-300">
              ...
              <li><strong>Swap</strong> '(' and ')' in the reversed expression.</li>
              <li>Apply a <strong>modified postfix</strong> algorithm (<em>do not</em> pop equal precedence).</li>
              <li><strong>Reverse</strong> the result to get the prefix expression.</li>
            </ol>
            <div className="mt-3 bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-200 dark:border-blue-800">
              <p className="text-sm">
                🔑 <span className="font-medium">Key difference from postfix:</span> Equal precedence operators are
                <strong>not</strong> popped – preserves right‑associativity.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="animate-fadeUp delay-500">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-amber-500/10 dark:hover:shadow-amber-400/10">
            ...
            <ul className="mt-4 list-disc pl-5 space-y-2 text-gray-700 dark:text-gray-300">
              <li>Always reverse first – the most common oversight.</li>
              <li>Swap parentheses after reversing to maintain grouping.</li>
              <li>Do not pop equal precedence – prefix is right‑associative.</li>
              <li>Test with <code>A+B</code> → <code>+AB</code> to verify.</li>
            </ul>
          </div>
        </section>
        ...
        <section className="animate-fadeUp delay-100">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-red-500/10 dark:hover:shadow-red-400/10">
            ...
            <ul className="mt-4 list-disc pl-5 space-y-2 text-gray-700 dark:text-gray-300">
              <li>Forgetting to reverse – wrong output.</li>
              <li>Not swapping parentheses – broken nesting.</li>
              <li>Using postfix associativity – equal precedence must not be popped.</li>
              <li>Forgetting final reverse – gives reversed prefix.</li>
            </ul>
          </div>
        </section>
        ...
        <section className="animate-fadeUp delay-200">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800 transition-all duration-300 hover:shadow-xl hover:shadow-green-500/10 dark:hover:shadow-green-400/10">
            ...
            <ul className="mt-4 list-disc pl-5 space-y-2 text-gray-700 dark:text-gray-300">
              <li>Implement conversion as reusable functions.</li>
              <li>Write unit tests for various expressions.</li>
              <li>Add comments explaining reversal and associativity.</li>
              <li>Use the visual tool to validate manual conversions.</li>
            </ul>
          </div>
        </section>
        ...
        <section className="animate-fadeUp delay-300">
          <div className="rounded-2xl bg-gray-50 dark:bg-gray-900/60 p-6 sm:p-8 border border-gray-200 dark:border-gray-800">
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                "I know the 4 steps of conversion.",
                "I understand why we reverse and swap.",
                "I know not to pop equal precedence.",
                "I can trace the algorithm on simple expressions.",
              ].map((item, idx) => (
        ...
        <section className="animate-fadeUp delay-400">
          <div className="rounded-2xl bg-indigo-50 dark:bg-indigo-950/30 p-6 sm:p-8 border border-indigo-200 dark:border-indigo-800 transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/10 dark:hover:shadow-indigo-400/10">
            ...
            <ul className="mt-4 list-disc pl-5 space-y-1 text-gray-700 dark:text-gray-300">
              <li>Why do we reverse the expression?</li>
              <li>What if we didn't swap parentheses?</li>
              <li>How does associativity differ from postfix?</li>
              <li>Try converting <code>A+B*C</code> using the tool.</li>
            </ul>
          </div>
        </section>
        ...
        <div className="animate-fadeUp delay-100">
          <Teacher
            note={
              "The visual tool is invaluable for building intuition. I encourage students to step through each conversion slowly and watch the stack. The key takeaway: prefix is postfix in a mirror. Practice with different expressions and verify the output."
            }
          />
        </div>
