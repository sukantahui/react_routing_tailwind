const questions = [
  {
    ...
    shortAnswer: "The special cases are: unique optimal, multiple optimal, infeasible, unbounded, redundant constraints, and degenerate solutions.",
  ...
  {
    question: "Can the simplex method handle degenerate solutions?",
    shortAnswer: "Yes, but degeneracy can cause cycling in the simplex method if not handled properly.",
    explanation: "Degenerate solutions can cause the simplex method to cycle between bases without improving the objective. Special techniques like Bland's rule prevent cycling.",
    hint: "Degeneracy can cause cycling in simplex.",
    level: "advanced",
    codeExample: "Standard simplex may cycle in degenerate cases\nBland's rule prevents cycling"
  },
  {
  ...
  {
    question: "Can a redundant constraint ever become binding?",
    shortAnswer: "No, by definition, redundant constraints can never become binding.",
    ...
    hint: "Redundant means never binding.",
    level: "intermediate",
    codeExample: "If x + y ≤ 20 is redundant, it never limits the solution"
  },
  {
  ...
  {
    question: "How do you identify redundancy graphically?",
    shortAnswer: "A redundant constraint line never touches the feasible region boundary.",
    explanation: "On a graph, redundant constraints are lines that lie outside or beyond the feasible region. They don't form any part of the region's boundary.",
    hint: "Line outside feasible region = redundant.",
    level: "intermediate",
    codeExample: "Feasible region: x + y ≤ 10\nRedundant: x + y ≤ 20 (line beyond region)"
  },
  {
    question: "What is the importance of special cases in real-world LP?",
    shortAnswer: "Special cases help identify problem formulation issues and provide decision-making flexibility.",
    explanation: "Recognizing special cases helps practitioners understand when a problem needs reformulation (infeasible/unbounded) or when there's flexibility in decisions (multiple optimal).",
    hint: "Special cases guide problem-solving and decision-making.",
    level: "advanced",
    codeExample: "Infeasible → Problem needs adjustment\nMultiple optima → Flexibility in decisions"
  },
  {
    question: "Can a problem have both infeasible and unbounded characteristics?",
    shortAnswer: "No, a problem is either infeasible, unbounded, or has an optimal solution. They are mutually exclusive.",
    explanation: "Infeasible means no feasible region. Unbounded means an infinite feasible region with no optimum. A problem can only be one of these (or have an optimal solution).",
    hint: "Mutually exclusive outcomes.",
    level: "intermediate",
    codeExample: "Either: no solution, infinite solution, or optimal solution"
  },
  {
    question: "What is the role of non-negativity constraints in special cases?",
    shortAnswer: "Non-negativity constraints can affect special cases by bounding the feasible region or creating degeneracy.",
    explanation: "Non-negativity constraints (x ≥ 0, y ≥ 0) can create additional binding constraints, leading to degeneracy. They also help bound the feasible region.",
    hint: "Non-negativity can create binding constraints.",
    level: "intermediate",
    codeExample: "At (0, 5), x ≥ 0 is binding\nCan cause degeneracy if other constraints also binding"
  },
  {
    question: "How does the objective function slope affect special cases?",
    shortAnswer: "The objective function slope determines whether we have unique or multiple optimal solutions.",
    explanation: "If the objective function slope is unique (not parallel to any constraint), we get a unique optimal solution. If it's parallel to a constraint, we get multiple optimal solutions.",
    hint: "Slope determines uniqueness of optimal solution.",
    level: "advanced",
  ...
  },
  {
    ...
    shortAnswer: "Degeneracy indicates that some resources are 'wasted' or that there's slack even when constraints are binding.",
    explanation: "In practice, degeneracy means there are unused resources even at the optimal solution, despite seemingly binding constraints. This affects sensitivity analysis.",
    hint: "Wasted resources despite binding constraints.",
    level: "advanced",
    codeExample: "3 binding constraints but 2 variables\nOne constraint doesn't really limit the solution"
  },
  {
    question: "Can adding a constraint create an infeasible problem?",
    shortAnswer: "Yes, adding constraints can make a previously feasible problem infeasible.",
    explanation: "Each new constraint restricts the feasible region further. If too restrictive, the constraints may become contradictory and no solution exists.",
    hint: "Too many constraints can eliminate all solutions.",
    level: "intermediate",
    codeExample: "Original: x + y ≤ 10\nAdd: x + y ≥ 12\nNow infeasible"
  },
  {
    question: "What is the difference between unique optimal and multiple optimal in terms of sensitivity?",
    shortAnswer: "Unique optimal solutions are sensitive to parameter changes, while multiple optimal solutions provide some robustness.",
    explanation: "With unique optimal, small changes in objective coefficients can change the optimal point. With multiple optimal, there's a range of optimal solutions that provides flexibility.",
    hint: "Unique = sensitive, Multiple = robust.",
    level: "advanced",
    codeExample: "Unique: Any change may change optimal point\nMultiple: Range of solutions available"
  },
  {
    question: "How do you verify if a constraint is truly redundant?",
    shortAnswer: "Check if removing the constraint changes the feasible region or optimal solution.",
    explanation: "Remove the suspected redundant constraint and solve the problem again. If the optimal solution and feasible region remain unchanged, the constraint is redundant.",
    hint: "Test by removing the constraint.",
    level: "intermediate",
    codeExample: "Remove constraint x ≤ 20\nIf solution unchanged → Redundant"
  },
  {
    question: "What causes unboundedness in minimization problems?",
    shortAnswer: "Unboundedness in minimization occurs when the objective can decrease indefinitely in the feasible region.",
    explanation: "If the feasible region extends to infinity in the direction where the objective decreases, the problem is unbounded. This typically happens with missing lower bounds.",
    hint: "Objective can go to negative infinity.",
    level: "intermediate",
    codeExample: "Min Z = x\nConstraint: y ≥ 0\nZ can go to -∞ as x → -∞"
  }
];
