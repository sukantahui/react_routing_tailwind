// src/components/study/common/PracticeEngineMath.jsx
...
function prepareQuiz(questions, difficulty = "all", topic = "all", questionCount) {
    ...
    if (topic && topic !== "all") {
        pool = pool.filter(
            (q) => q.topic && q.topic.toLowerCase() === topic.toLowerCase()
        );
    }
    ...
    return shuffledQuestions.slice(0, sliceSize).map((q) => {
...
// ===================================================================
export default function PracticeEngineMath({
    title = "Quiz Test",
    questions = [],
    testId = "test_default",
    ...
    useEffect(() => {
        ...
        setOpenExplanation({});
    }, [topic, difficulty, questionCount]);   // ONLY topic change auto-refresh
    ...
    useEffect(() => {
        ...
        if (saved) {
            try {
                ...
                const savedTopic = parsed.topic || "all";     // ⭐ ADD
        ...
        const fresh = prepareQuiz(questions, "all", topic, initialCount);
        setQuiz(fresh);
        setActiveQuestionIndex(0);
        setElapsedSeconds(0);
    ...
    const applySettings = () => {
        ...
        if (timerMode === "countdown") setRemainingSeconds(countdownSeconds);
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="flex flex-wrap gap-2 max-h-28 overflow-y-auto pr-1">
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-full text-xs border ${topic === "all"
                            ...
                            {allTopics.map((t) => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-full text-xs border ${topic === t.toLowerCase()
                                        ? "bg-violet-600 text-white border-violet-400"
                                        : "bg-slate-900 text-slate-300 border-slate-700 hover:bg-slate-800"
                                        }`}
                                >
