...
/**
 * Helper: decide if answer should be displayed as a C code block.
 * Checks for standard C constructs (includes, main, printf, scanf, pointers, structs, etc.)
 */
const isCCode = (answer = "") => {
  ...
  return (
    ...
    answer.includes("if(") ||
    answer.includes("struct ") ||
    answer.includes("typedef ") ||
    answer.includes("return 0;") ||
    answer.includes("malloc(") ||
    answer.includes("free(") ||
    answer.includes("char ") ||
    answer.includes("int ") ||
    answer.includes("float ") ||
    answer.includes("double ") ||
    answer.includes("void ")
  );
};
...
const getFileName = (project) => {
  if (project?.answerFile) {
    const parts = project.answerFile.split("/");
    return parts[parts.length - 1];
  }
  if (project?.projectId) {
    return `${project.projectId}.c`;
  }
  return "answer.c";
};

/**
 * Rich Formatter for C Logic & Code Walkthrough text
 */
            ...
            {lines.map((line, lIdx) => {
              ...
              if (isNumberedHeader) {
                return (
                  <div
                    key={lIdx}
                    className="font-semibold text-amber-300 flex items-start gap-2 pt-1.5"
                  >
...
export default function CProjectAnswerTemplateEnhanced({ data }) {
  const {
    projectCategory = "C Language Practical Projects & Lab Exercises",
    subject = "C Programming & System Logic",
    board = "ICSE / ISC / WBBSE / University",
    class: className = "Class 10 / 11 / 12 / BCA / B.Tech",
    tools = ["GCC Compiler", "VS Code", "Code::Blocks", "Turbo C++"],
    institute = {
      author: "Sukanta Hui",
      name: "Coder & AccoTax",
  ...
  const storageKey = `c_projects_${(projectCategory || "default")
  ...
  // Copied state for sample input/output strings
  ...
  const resetProgress = () => {
    if (
      window.confirm(
        "Are you sure you want to reset your practice completion progress for this C programming module?"
      )
    ) {
      setCompletedMap({});
      setBookmarkMap({});
  ...
  const filteredProjects = useMemo(() => {
    return projects.filter((p) => {
      ...
      const matchSearch =
        ...
        (p.answerFile && p.answerFile.toLowerCase().includes(q)) ||
    ...
    <div className="space-y-8 pb-16 max-w-7xl mx-auto px-2 sm:px-4">
        ...
        <div className="relative z-10 space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="inline-flex items-center gap-2 rounded-full border border-sky-500/30 bg-sky-500/10 px-3.5 py-1.5 text-xs font-semibold tracking-wide text-sky-300">
              ...
              <span>C PROGRAMMING PRACTICAL LAB &amp; EXERCISES</span>
            </div>

            <div className="flex items-center gap-2 text-xs font-mono text-slate-400">
              <Clock size={14} className="text-slate-400" />
              <span>Est. Lab Completion: ~45 mins</span>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="mt-2 text-sm sm:text-base text-slate-300 max-w-4xl leading-relaxed">
              Step-by-step interactive C programming problems with raw <code>.c</code> source code solutions, algorithm breakdowns, execution steps, and memory management mechanics.
            </p>
          </div>
          ...
          {/* Tools & Compilers Badges */}
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <span className="text-slate-400 font-medium mr-1">Compilers &amp; IDEs:</span>
            {tools.map((tool, idx) => (
              <span
                key={idx}
                className="rounded-md border border-slate-800 bg-slate-950/70 px-2.5 py-1 font-mono text-sky-300"
              >
                {tool}
              </span>
            ))}
      ...
      <div className="sticky top-16 z-30 flex flex-col gap-4 rounded-2xl border border-slate-800 bg-slate-900/90 p-4 shadow-xl backdrop-blur-xl md:flex-row md:items-center md:justify-between">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search by title, C file (e.g. answer1.c), ID, or keywords..."
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <Bookmark
              size={14}
              className={onlyBookmarks ? "fill-amber-400 text-amber-400" : ""}
            />
      ...
      {filteredProjects.length !== projects.length && (
        <div className="flex items-center justify-between text-xs text-slate-400 px-2">
          <span>
            Showing <b>{filteredProjects.length}</b> of <b>{projects.length}</b> matching C exercises
          </span>
      ...
      {/* ========================================================================= */}
      {/* 3. C PROJECT PROBLEM CARDS LIST                                           */}
      {/* ========================================================================= */}
      <div className="space-y-6">
        {filteredProjects.length === 0 ? (
          <div className="rounded-3xl border border-slate-800 bg-slate-900/60 p-12 text-center">
            ...
            <h3 className="text-lg font-bold text-white mb-1">No matching C exercises found</h3>
            <p className="text-sm text-slate-400 max-w-md mx-auto">
              Try adjusting your search keywords or switching difficulty filters to see more C projects.
            </p>
            <button
              onClick={() => {
                setSearchQuery("");
          ...
          filteredProjects.map((project) => {
            ...
            const cFileName = getFileName(project);
            const rawAnswer = project.answer || "";
                ...
                <div className="p-6 md:p-8 space-y-6">
                  ...
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="flex items-start gap-3.5">
                      {/* Completion check mark toggle button */}
                      <button
                      ...
                      {/* Project ID & C File Badge */}
                      <div className="space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="inline-flex items-center gap-1 rounded-md border border-indigo-500/30 bg-indigo-500/10 px-2.5 py-0.5 font-mono text-xs font-bold text-indigo-300">
                            {project.projectId}
                          </span>

                          <span className="inline-flex items-center gap-1 rounded-md border border-sky-500/30 bg-sky-500/10 px-2.5 py-0.5 font-mono text-xs text-sky-300">
                    ...
                    {/* Action Buttons: Bookmark + View Solution */}
                  ...
                  {(project.exampleText || project.exampleOutput) && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
                      {/* Sample Input / Pre-conditions Box */}
                      {project.exampleText && (
                  ...
                  {/* ---------- 3.5 CORE LOGIC & ALGORITHM MECHANICS ---------- */}
                    ...
                    <div className="mt-6 rounded-2xl border border-indigo-500/30 bg-slate-950 overflow-hidden shadow-2xl transition-all duration-300">
                      ...
                      <div className="flex items-center justify-between border-b border-slate-800 bg-slate-900/90 px-4 py-3 text-xs">
                        <div className="flex items-center gap-2 font-semibold text-sky-300">
                          ...
                          <span>Complete Runnable Solution ({cFileName})</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-slate-400 font-mono">
                            Language: C Language (GCC)
                          </span>
                        </div>
                      </div>
                        ...
                        {isCCode(rawAnswer) ? (
                          ...
                          <div className="p-4 rounded-xl bg-slate-900 border border-slate-800">
                            <p className="text-xs text-amber-400 font-mono mb-2">
                              ⚡ Note: Raw solution output or explanation:
                            </p>
                            <pre className="text-sm text-slate-300 leading-relaxed font-mono whitespace-pre-wrap">
                              {rawAnswer || `// Solution file "${cFileName}" is loading or empty.`}
                            </pre>
                          </div>
                        )}
                      </div>
      ...
      {/* ========================================================================= */}
      {/* 4. INSTRUCTOR MENTORSHIP & C COMPILATION GUIDANCE CARD                    */}
      {/* ========================================================================= */}
          ...
          <div className="space-y-1.5 flex-1">
            <h3 className="text-base sm:text-lg font-bold text-white flex items-center gap-2">
              ...
              <span className="text-xs font-normal text-emerald-400 bg-emerald-900/30 px-2.5 py-0.5 rounded-full border border-emerald-500/20">
                {institute.author} · {institute.name} ({institute.location})
              </span>
            </h3>
            <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
              To compile and execute C programs locally, open your command terminal and run:{" "}
              <kbd className="bg-slate-800 border border-slate-700 px-1.5 py-0.5 rounded text-amber-300 font-mono text-xs">
                gcc answer1.c -o program &amp;&amp; ./program
              </kbd>
              . Always include required header files (e.g. <code>#include &lt;stdio.h&gt;</code>, <code>#include &lt;stdlib.h&gt;</code>), ensure proper <code>main()</code> signature, check return values for <code>scanf</code> input operations, and remember to free dynamically allocated memory!
            </p>
          </div>
        </div>
      </section>
