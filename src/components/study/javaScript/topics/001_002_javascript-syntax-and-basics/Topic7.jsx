...
import demoCode from "./topic7_files/NumbersInJavascriptArithmeticNanInfinityPrecisionDemo.js?raw";

/**
 * Topic7 – Numbers in JavaScript — IEEE 754 Arithmetic, NaN, Infinity & Precision
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 001_002_javascript-syntax-and-basics · Topic 7</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Numbers in JavaScript — IEEE 754 Arithmetic, NaN, Infinity & Precision"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Master 64-bit floating-point numbers, 0.1 + 0.2 precision drift, Number.EPSILON, and safe integer limits.
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <p className="text-slate-300 leading-relaxed mb-4">
              Mastering <strong className="text-amber-300">Numbers & IEEE 754</strong> provides the rock-solid foundation necessary for advanced JavaScript and TypeScript development. In high-scale web applications, subtle syntax nuances, type coercions, and lexical scoping rules dictate how the V8 engine allocates memory and compiles bytecode.
            </p>
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                During an interactive lab session, student <strong>Abhronila</strong> encountered unexpected runtime behavior. Abhronila wrote if (0.1 + 0.2 === 0.3) for an ecommerce checkout cart and the condition evaluated to false! Sukanta Hui explained IEEE 754 binary floating-point representation and how to use Number.EPSILON for financial calculations.
              </p>
              <p>
                Mentor <strong>Sukanta Hui</strong> demonstrated the step-by-step memory allocation on the whiteboard, showing how mastering <em>Numbers & IEEE 754</em> guarantees deterministic code execution.
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            <h2 className="text-lg sm:text-xl font-bold text-sky-400 mb-4 flex items-center gap-2">
              <span>📊</span> IEEE 754 Double-Precision (64-bit) Binary Floating-Point Format
            </h2>
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 840 280" className="w-full h-auto" role="img" aria-label="Numbers in JavaScript — IEEE 754 Arithmetic, NaN, Infinity & Precision Architecture">
                ...
                <text x="420" y="28" fill="#f8fafc" fontSize="15" fontWeight="bold" textAnchor="middle">
                  IEEE 754 Double-Precision (64-bit) Binary Floating-Point Format
                </text>
                
                <g transform="translate(60, 60)">
                  {/* Sign bit */}
                  <rect x="0" y="0" width="80" height="80" rx="8" fill="#1e293b" stroke="#f43f5e" strokeWidth="1.5" />
                  <text x="40" y="28" fill="#f43f5e" fontSize="11" fontWeight="bold" textAnchor="middle">Sign (1-bit)</text>
                  <text x="40" y="52" fill="#fda4af" fontSize="14" fontFamily="monospace" textAnchor="middle">0 / 1</text>
                  <text x="40" y="70" fill="#94a3b8" fontSize="8" textAnchor="middle">Positive / Negative</text>

                  {/* Exponent */}
                  <rect x="95" y="0" width="220" height="80" rx="8" fill="#1e293b" stroke="#f59e0b" strokeWidth="1.5" />
                  <text x="205" y="28" fill="#fbbf24" fontSize="11" fontWeight="bold" textAnchor="middle">Exponent (11-bits)</text>
                  <text x="205" y="52" fill="#fde68a" fontSize="13" fontFamily="monospace" textAnchor="middle">Bias 1023 (Range 2^-1022 to 2^1023)</text>
                  <text x="205" y="70" fill="#94a3b8" fontSize="8" textAnchor="middle">Determines magnitude &amp; decimal point</text>

                  {/* Mantissa / Fraction */}
                  <rect x="330" y="0" width="390" height="80" rx="8" fill="#1e293b" stroke="#10b981" strokeWidth="1.5" />
                  <text x="525" y="28" fill="#34d399" fontSize="11" fontWeight="bold" textAnchor="middle">Mantissa / Fraction (52-bits + 1 hidden)</text>
                  <text x="525" y="52" fill="#a7f3d0" fontSize="13" fontFamily="monospace" textAnchor="middle">53-bit Precision (Number.MAX_SAFE_INTEGER: 2^53 - 1)</text>
                  <text x="525" y="70" fill="#94a3b8" fontSize="8" textAnchor="middle">Precision limit causes 0.1 + 0.2 = 0.30000000000000004</text>
                </g>

                <g transform="translate(60, 160)">
                  <rect width="720" height="60" rx="8" fill="#0f172a" stroke="#334155" />
                  <text x="360" y="26" fill="#38bdf8" fontSize="11" fontWeight="bold" textAnchor="middle">Safe Float Comparison Formula: Math.abs(a - b) &lt; Number.EPSILON</text>
                  <text x="360" y="46" fill="#94a3b8" fontSize="10" textAnchor="middle">Math.abs((0.1 + 0.2) - 0.3) &lt; Number.EPSILON === true (Determines mathematical equality safely)</text>
                </g>
    
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 7.1: Runtime execution pipeline, memory layout, and engine rules for Numbers &amp; IEEE 754.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="NumbersPrecisionAndNaNDemo.js"
            />
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  Number.isSafeInteger() and 2^53 - 1 Boundary
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              JavaScript integers above 9,007,199,254,740,991 (Number.MAX_SAFE_INTEGER) lose precision because the 53-bit mantissa overflows into the exponent. For larger IDs, always use BigInt.
            </p>

            <div className="rounded-xl border border-amber-900/50 bg-slate-950 p-4 font-mono text-xs text-amber-200 overflow-x-auto">
              <pre>{`// 💎 SENIOR SECRET:
console.log(9007199254740991 + 1); // 9007199254740992
console.log(9007199254740991 + 2); // 9007199254740992 (Precision lost!)`}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Numbers & IEEE 754"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Numbers & IEEE 754"}
            downloadFileName="001_002_javascript-syntax-and-basics-topic7-note.txt"
          />
        </section>
