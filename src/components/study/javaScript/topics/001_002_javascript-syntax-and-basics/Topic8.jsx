...
import demoCode from "./topic8_files/SymbolBigintPrimitivesUniqueIdentifiersArbitraryPrecisionIntegersDemo.js?raw";

/**
 * Topic8 – Symbol & BigInt Primitives: Unique Identifiers & Arbitrary Precision
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 001_002_javascript-syntax-and-basics · Topic 8</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Symbol & BigInt Primitives: Unique Identifiers & Arbitrary Precision"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Master guaranteed unique Symbol properties, well-known symbols, and arbitrary-precision BigInt integers for financial and cryptographic math.
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <p className="text-slate-300 leading-relaxed mb-4">
              Mastering <strong className="text-amber-300">Symbol & BigInt</strong> provides the rock-solid foundation necessary for advanced JavaScript and TypeScript development. In high-scale web applications, subtle syntax nuances, type coercions, and lexical scoping rules dictate how the V8 engine allocates memory and compiles bytecode.
            </p>
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                During an interactive lab session, student <strong>Debangshu</strong> encountered unexpected runtime behavior. Debangshu parsed a 64-bit Twitter Snowflake ID with JSON.parse and the last digits were corrupted due to 53-bit float limits. Sukanta Hui showed how BigInt preserves full precision without rounding errors.
              </p>
              <p>
                Mentor <strong>Sukanta Hui</strong> demonstrated the step-by-step memory allocation on the whiteboard, showing how mastering <em>Symbol & BigInt</em> guarantees deterministic code execution.
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            <h2 className="text-lg sm:text-xl font-bold text-sky-400 mb-4 flex items-center gap-2">
              <span>📊</span> Symbol Guaranteed Uniqueness & BigInt Heap Layout
            </h2>
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 840 280" className="w-full h-auto" role="img" aria-label="Symbol & BigInt Primitives: Unique Identifiers & Arbitrary Precision Architecture">
                ...
                <text x="420" y="28" fill="#f8fafc" fontSize="15" fontWeight="bold" textAnchor="middle">
                  Symbol Guaranteed Uniqueness & BigInt Heap Layout
                </text>
                
                <g transform="translate(40, 50)">
                  <rect width="360" height="190" rx="10" fill="#1e293b" stroke="#f59e0b" strokeWidth="1.5" />
                  <text x="180" y="26" fill="#fbbf24" fontSize="13" fontWeight="bold" textAnchor="middle">Symbol: Guaranteed Unique Primitive</text>
                  <rect x="20" y="45" width="320" height="120" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="35" y="70" fill="#fde68a" fontSize="11" fontFamily="monospace">const id1 = Symbol('id');</text>
                  <text x="35" y="90" fill="#fde68a" fontSize="11" fontFamily="monospace">const id2 = Symbol('id');</text>
                  <text x="35" y="115" fill="#f87171" fontSize="11" fontFamily="monospace">id1 === id2 // FALSE (Never equals!)</text>
                  <text x="35" y="140" fill="#34d399" fontSize="10">Hidden from for...in &amp; Object.keys()</text>
                </g>

                <g transform="translate(440, 50)">
                  <rect width="360" height="190" rx="10" fill="#1e293b" stroke="#38bdf8" strokeWidth="1.5" />
                  <text x="180" y="26" fill="#38bdf8" fontSize="13" fontWeight="bold" textAnchor="middle">BigInt: Arbitrary Precision Integers</text>
                  <rect x="20" y="45" width="320" height="120" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="35" y="70" fill="#7dd3fc" fontSize="11" fontFamily="monospace">const huge = 9007199254740991n;</text>
                  <text x="35" y="95" fill="#7dd3fc" fontSize="11" fontFamily="monospace">huge + 2n // 9007199254740993n (Exact!)</text>
                  <path d="M 35 115 L 325 115" stroke="#334155" />
                  <text x="35" y="135" fill="#fda4af" fontSize="10">⚠️ Cannot mix Number and BigInt without casting</text>
                  <text x="35" y="152" fill="#94a3b8" fontSize="9">Math.* methods do not accept BigInt</text>
                </g>
    
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 8.1: Runtime execution pipeline, memory layout, and engine rules for Symbol &amp; BigInt.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-gradient-to-br from-amber-950/30 via-slate-900 to-purple-950/20 border border-amber-500/30 rounded-2xl p-6 md:p-8 shadow-xl relative overflow-hidden">
            ...
            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              Unlike Symbol() which always creates a brand new unique symbol, Symbol.for('key') looks up or creates a shared symbol in the runtime's global symbol registry across iframe boundaries and realms.
            </p>
...
console.log(sym1 === sym2); // true (Shared across modules!)`}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Symbol & BigInt"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Symbol & BigInt"}
            downloadFileName="001_002_javascript-syntax-and-basics-topic8-note.txt"
          />
        </section>
