...
import demoCode from "./topic6_files/TypeConversionImplicitVsExplicitInJavascriptDemo.js?raw";

/**
 * Topic6 – Type Conversion: Implicit vs Explicit in JavaScript
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 001_002_javascript-syntax-and-basics · Topic 6</span>
          </div>
          ...
          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Understand ToPrimitive, ToNumber, ToString abstract coercion algorithms and explicit type casting wrappers.
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <p className="text-slate-300 leading-relaxed mb-4">
              Mastering <strong className="text-amber-300">Type Conversion</strong> provides the rock-solid foundation necessary for advanced JavaScript and TypeScript development. In high-scale web applications, subtle syntax nuances, type coercions, and lexical scoping rules dictate how the V8 engine allocates memory and compiles bytecode.
            </p>
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                During an interactive lab session, student <strong>Swadeep</strong> encountered unexpected runtime behavior. Swadeep added a form input string value '100' to a bonus number 50: const total = input.value + 50 resulting in '10050'. Sukanta Hui demonstrated why explicit conversion Number(input.value) is mandatory.
              </p>
              <p>
                Mentor <strong>Sukanta Hui</strong> demonstrated the step-by-step memory allocation on the whiteboard, showing how mastering <em>Type Conversion</em> guarantees deterministic code execution.
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            <h2 className="text-lg sm:text-xl font-bold text-sky-400 mb-4 flex items-center gap-2">
              <span>📊</span> ECMAScript Type Coercion & ToPrimitive Abstract Algorithm
            </h2>
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 840 280" className="w-full h-auto" role="img" aria-label="Type Conversion: Implicit vs Explicit in JavaScript Architecture">
                ...
                <text x="420" y="28" fill="#f8fafc" fontSize="15" fontWeight="bold" textAnchor="middle">
                  ECMAScript Type Coercion & ToPrimitive Abstract Algorithm
                </text>
                
                <g transform="translate(40, 60)">
                  <rect width="210" height="100" rx="10" fill="#1e293b" stroke="#38bdf8" strokeWidth="1.5" />
                  <text x="105" y="26" fill="#38bdf8" fontSize="12" fontWeight="bold" textAnchor="middle">1. Input Operands</text>
                  <rect x="15" y="40" width="180" height="45" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="105" y="58" fill="#7dd3fc" fontSize="10" textAnchor="middle">"100" + 50</text>
                  <text x="105" y="74" fill="#94a3b8" fontSize="9" textAnchor="middle">String + Number Binary Op</text>
                </g>

                <path d="M 250 110 L 320 110" fill="none" stroke="#f59e0b" strokeWidth="2.5" />

                <g transform="translate(325, 60)">
                  <rect width="210" height="100" rx="10" fill="#1e293b" stroke="#f59e0b" strokeWidth="1.5" />
                  <text x="105" y="26" fill="#fbbf24" fontSize="12" fontWeight="bold" textAnchor="middle">2. Engine Coercion</text>
                  <rect x="15" y="40" width="180" height="45" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="105" y="58" fill="#fde68a" fontSize="10" textAnchor="middle">ToString(50) -&gt; "50"</text>
                  <text x="105" y="74" fill="#94a3b8" fontSize="9" textAnchor="middle">+ favors string concatenation</text>
                </g>

                <path d="M 535 110 L 605 110" fill="none" stroke="#f43f5e" strokeWidth="2.5" />

                <g transform="translate(610, 60)">
                  <rect width="190" height="100" rx="10" fill="#1e293b" stroke="#f43f5e" strokeWidth="1.5" />
                  <text x="95" y="26" fill="#f87171" fontSize="12" fontWeight="bold" textAnchor="middle">3. Evaluated Output</text>
                  <rect x="15" y="40" width="160" height="45" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="95" y="58" fill="#fda4af" fontSize="11" fontFamily="monospace">"10050" (String!)</text>
                  <text x="95" y="74" fill="#94a3b8" fontSize="9" textAnchor="middle">Fix: Number(val) + 50</text>
                </g>
    
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 6.1: Runtime execution pipeline, memory layout, and engine rules for Type Conversion.
            </p>
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  Symbol.toPrimitive Custom Object Conversion
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              You can fully customize how your JavaScript objects coerce to number, string, or default by defining the [Symbol.toPrimitive](hint) method on the object.
            </p>
...
console.log(String(wallet));   // '$500' (string)`}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Type Conversion"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Type Conversion"}
            downloadFileName="001_002_javascript-syntax-and-basics-topic6-note.txt"
          />
        </section>
