...
import demoCode from "./topic4_files/ArrowFunctionsSyntaxLexicalThisDemoDemo.js?raw";
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 002_001_functions-basics · Topic 4</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Arrow Functions: Syntax, Lexical this Binding & Implicit Returns"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            {"Master ES6 arrow functions: concise syntax, implicit returns, lexical this/arguments capture, and why arrow functions cannot be used as constructors or methods."}
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                {"Swadeep used an arrow function as an object method expecting `this` to refer to the object, but `this` resolved to the global/undefined window."}
              </p>
              <p>
                {"Sukanta Hui clarified that arrow functions do NOT have their own `[[ThisValue]]` or `arguments` binding; they capture `this` lexically from their enclosing lexical context."}
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            ...
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 800 240" className="w-full h-auto" role="img" aria-label={"Arrow Functions: Syntax, Lexical this Binding & Implicit Returns"}>
                <defs>
            ...
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure: Step-by-step architectural execution and state transitions for {"Arrow Functions: Syntax, Lexical this Binding & Implicit Returns"}.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">
              The ECMAScript specification mandates precise abstract operations during the lifecycle of {"Arrow Functions: Syntax, Lexical this Binding & Implicit Returns"}. The table below outlines formal specification rules, engine runtime dynamics, and actionable developer invariants.
            </p>
              ...
              <table className="w-full text-left text-sm text-slate-300 border-collapse border border-slate-800">
                ...
                <tbody className="divide-y divide-slate-800 font-mono text-xs">
                  ...
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"Standard Function"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Dynamic this binding determined by call-site"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Creates new [[ThisValue]] on invocation"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Object methods, constructor functions"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"Arrow Function"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Lexical this binding inherited from outer scope"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"No internal [[Construct]] or [[ThisValue]]"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Callbacks, array pipeline transforms, async closures"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"new Keyword Compatibility"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Arrow functions lack [[Construct]] slot"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Throws TypeError if invoked with new"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Pure functions and callbacks only"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="mt-6 p-5 rounded-xl bg-slate-900/80 border border-rose-900/40 space-y-3">
              ...
              <ul className="list-disc list-inside space-y-2 text-sm text-slate-300 leading-relaxed">
                <li><strong className="text-rose-300">{"TypeError on 'new () => {}':"}</strong> {"Calling `new` on an arrow function throws `TypeError: fn is not a constructor` because arrow functions lack a `prototype` property."}</li>
                <li><strong className="text-rose-300">{"Loss of Context in Object Methods:"}</strong> {"Defining object methods as arrow functions binds `this` to the enclosing scope (often global or module), breaking object state access."}</li>
                <li><strong className="text-rose-300">{"Lack of arguments Object:"}</strong> {"Referencing `arguments` inside an arrow function refers to the outer standard function's arguments, not the arrow function's."}</li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="ArrowFunctionsSyntaxLexicalThisDemoDemo.js"
            />
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-rose-950/20 border border-rose-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-rose-400 font-bold">
                <span>❌</span>
                <span>Anti-Pattern: {"Using Arrow Functions for Object Methods or Prototypes"}</span>
              </div>
...
{"// ❌ AVOID: this is undefined or global\nconst user = {\n  name: \"Swadeep\",\n  getName: () => this.name // undefined!\n};"}
            ...
            <div className="bg-emerald-950/20 border border-emerald-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-emerald-400 font-bold">
                <span>✓</span>
                <span>Senior Pro Practice: {"Use Method Shorthand for Objects & Arrow for Callbacks"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
...
{"// ✓ RECOMMENDED: Perfect balance\nconst user = {\n  name: \"Swadeep\",\n  getName() {\n    return this.name;\n  },\n  getDelayedName() {\n    setTimeout(() => console.log(this.name), 100); // lexical this works!\n  }\n};"}
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  {"Arrow Functions in Class Fields Auto-Bind Instance Context"}
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              {"Assigning an arrow function to a class field defines the method directly on each instance with `this` permanently bound to that instance, solving React/event callback detachment."}
            </p>

            <div className="rounded-xl border border-amber-900/50 bg-slate-950 p-4 font-mono text-xs text-amber-200 overflow-x-auto">
              <pre>{"class Counter {\n  count = 0;\n  // Bound per-instance at construction time\n  increment = () => {\n    this.count++;\n    return this.count;\n  };\n}\nconst c = new Counter();\nconst detached = c.increment;\nconsole.log(detached()); // 1 (no need for .bind())"}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-indigo-950/20 border border-indigo-800/40 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed mb-4">
              {"Can you change the `this` context of an arrow function using `.call()`, `.apply()`, or `.bind()`? Why or why not?"}
            </p>
            <div className="p-4 rounded-xl bg-slate-900 border border-indigo-900/50 text-xs sm:text-sm text-indigo-300 font-mono">
              💡 Hint: {"Arrow functions do not have an internal [[ThisValue]]; their this binding is permanently established during lexical scope resolution."}
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Arrow Functions: Syntax, Lexical this Binding & Implicit Returns"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Arrow Functions: Syntax, Lexical this Binding & Implicit Returns"}
            downloadFileName="002-001-topic4-note.txt"
          />
        </section>
