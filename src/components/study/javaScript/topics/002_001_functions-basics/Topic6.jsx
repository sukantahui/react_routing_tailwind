...
import demoCode from "./topic6_files/FirstClassFunctionsCallbacksHofDemoDemo.js?raw";
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 002_001_functions-basics · Topic 6</span>
          </div>
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"First-Class Functions, Callbacks & Higher-Order Functions (HOFs)"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            {"Leverage JavaScript's first-class function nature: passing functions as arguments, returning functions from functions, and architecting reusable higher-order utilities."}
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                {"Abhronila passed `calcTotal()` instead of `calcTotal` as a callback to `setTimeout`, causing the function to run immediately on registration instead of after the delay."}
              </p>
              <p>
                {"Sukanta Hui highlighted the difference between passing a function reference (first-class value) and executing a function invocation."}
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            ...
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 800 240" className="w-full h-auto" role="img" aria-label={"First-Class Functions, Callbacks & Higher-Order Functions (HOFs)"}>
                <defs>
                ...
                <text x="400" y="30" fill="#f8fafc" fontSize="14" fontWeight="bold" textAnchor="middle">&#123;"First-Class Functions, Callbacks & Higher-Order Functions (HOFs)"&#125; · Architecture &amp; State Transitions</text>
                ...
                <text x="140" y="135" fill="#fef3c7" fontSize="11" textAnchor="middle">&#123;"Function as Value Reference"&#125;</text>
                ...
                <text x="400" y="135" fill="#e0f2fe" fontSize="11" textAnchor="middle">&#123;"Higher-Order Function Wrapper"&#125;</text>
                ...
                <text x="660" y="135" fill="#d1fae5" fontSize="11" textAnchor="middle">&#123;"Deferred Callback Execution"&#125;</text>
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure: Step-by-step architectural execution and state transitions for {"First-Class Functions, Callbacks & Higher-Order Functions (HOFs)"}.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">
              The ECMAScript specification mandates precise abstract operations during the lifecycle of {"First-Class Functions, Callbacks & Higher-Order Functions (HOFs)"}. The table below outlines formal specification rules, engine runtime dynamics, and actionable developer invariants.
            </p>
              ...
              <table className="w-full text-left text-sm text-slate-300 border-collapse border border-slate-800">
                ...
                <tbody className="divide-y divide-slate-800 font-mono text-xs">
                  ...
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"First-Class Citizen"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Functions can be assigned, passed, and returned"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Function objects allocated on heap with [[Call]] slot"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Core architecture across all JS software"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"Callback Function"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Function passed into another to be called back later"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Invoked synchronously or asynchronously by host"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Event handling, array methods, async tasks"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"Higher-Order Function"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Function accepting or returning functions"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Creates closure over enclosing parameters"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Composition, currying, middleware, memoization"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="mt-6 p-5 rounded-xl bg-slate-900/80 border border-rose-900/40 space-y-3">
              ...
              <ul className="list-disc list-inside space-y-2 text-sm text-slate-300 leading-relaxed">
                <li><strong className="text-rose-300">{"Immediate Invocation Trap:"}</strong> {"Passing `fn()` instead of `fn` causes the return value to be passed instead of the callable function."}</li>
                <li><strong className="text-rose-300">{"Loss of Context in Callbacks:"}</strong> {"Passing an un-bound object method as a callback causes `this` to lose its original object context."}</li>
                <li><strong className="text-rose-300">{"Callback Hell (Inversion of Control):"}</strong> {"Deeply nested asynchronous callbacks make error handling and stack tracing brittle."}</li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="FirstClassFunctionsCallbacksHofDemoDemo.js"
            />
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-rose-950/20 border border-rose-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-rose-400 font-bold">
                <span>❌</span>
                <span>Anti-Pattern: {"Accidentally Invoking Callbacks During Registration"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                {"Calling `element.addEventListener('click', handleClick())` binds the return value instead of the handler."}
              </p>
              <pre className="p-4 rounded-xl bg-slate-950 border border-rose-900/50 text-xs font-mono text-rose-300 overflow-x-auto">
{"// ❌ AVOID: Runs immediately, binds undefined\nbutton.addEventListener(\"click\", saveReport());"}
              </pre>
            </div>
            ...
            <div className="bg-emerald-950/20 border border-emerald-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-emerald-400 font-bold">
                <span>✓</span>
                <span>Senior Pro Practice: {"Pass Function Identifier or Arrow Wrapper"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                {"Pass the bare function name or an arrow wrapper if passing custom parameters."}
              </p>
...
{"// ✓ RECOMMENDED: Clean Callback Reference\nbutton.addEventListener(\"click\", saveReport);\nbutton.addEventListener(\"click\", () =&gt; saveReport(\"custom_id\"));"}
              </pre>
            </div>
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  {"Generic Function Decorator / Execution Timer HOF"}
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              {"HOFs can intercept, instrument, and time any arbitrary function without modifying its internal implementation."}
            </p>

            <div className="rounded-xl border border-amber-900/50 bg-slate-950 p-4 font-mono text-xs text-amber-200 overflow-x-auto">
              <pre>{"function withPerformanceMetrics(fn) {\n  return function(...args) {\n    const start = performance.now();\n    const result = fn.apply(this, args);\n    const duration = (performance.now() - start).toFixed(3);\n    console.log(`[METRIC] ${fn.name || 'Anonymous'} executed in ${duration}ms`);\n    return result;\n  };\n}\nconst heavyCalc = withPerformanceMetrics(function computePrimes(limit) {\n  return limit * limit;\n});\nconsole.log(heavyCalc(1000));"}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-indigo-950/20 border border-indigo-800/40 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed mb-4">
              {"What are the three fundamental rules a programming language must satisfy for functions to be considered 'first-class citizens'?"}
            </p>
            <div className="p-4 rounded-xl bg-slate-900 border border-indigo-900/50 text-xs sm:text-sm text-indigo-300 font-mono">
              💡 Hint: {"Assignment to variables/properties, passing as arguments, and returning from other functions."}
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "First-Class Functions, Callbacks & Higher-Order Functions (HOFs)"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "First-Class Functions, Callbacks & Higher-Order Functions (HOFs)"}
            downloadFileName="002-001-topic6-note.txt"
          />
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <Teacher
            note={"In my 27+ years of mentoring software engineers at Coder & AccoTax in Barrackpore, I have consistently seen that mastering " + "First-Class Functions, Callbacks & Higher-Order Functions (HOFs)" + " separates code monkeys from genuine software engineers. " + "Higher-order functions are the building blocks of JavaScript: map, filter, reduce, Express middleware, and React hooks all stem from this single concept."}
          />
        </section>
