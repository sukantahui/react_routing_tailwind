...
import demoCode from "./topic14_files/NewTargetConstructorFunctionGuardsDemoDemo.js?raw";
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 002_001_functions-basics · Topic 14</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"new.target Meta-Property & Constructor Function Guards"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            {"Inspect and defend constructor invocations using the ES6 new.target meta-property: preventing un-newed constructor invocations, building abstract base classes, and managing instantiation."}
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                {"Abhronila called a constructor function `User('Swadeep', 'Admin')` without the `new` keyword, polluting the global `window` object with `window.name = 'Swadeep'`."}
              </p>
              <p>
                {"Sukanta Hui introduced `new.target`, showing that inside a function invoked with `new`, `new.target` references the constructor function; when called normally, it is `undefined`."}
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            ...
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 800 240" className="w-full h-auto" role="img" aria-label={"new.target Meta-Property & Constructor Function Guards"}>
                <defs>
                ...
                <text x="400" y="30" fill="#f8fafc" fontSize="14" fontWeight="bold" textAnchor="middle">&#123;"new.target Meta-Property & Constructor Function Guards"&#125; · Architecture &amp; State Transitions</text>
                ...
                <text x="140" y="135" fill="#fef3c7" fontSize="11" textAnchor="middle">&#123;"Function Call: User() -&gt; new.target = undefined"&#125;</text>
                ...
                <text x="400" y="135" fill="#e0f2fe" fontSize="11" textAnchor="middle">&#123;"Constructor Call: new User() -&gt; new.target = User"&#125;</text>
                ...
                <text x="660" y="135" fill="#d1fae5" fontSize="11" textAnchor="middle">&#123;"Class Inheritance: new Sub() -&gt; new.target = SubClass"&#125;</text>
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure: Step-by-step architectural execution and state transitions for {"new.target Meta-Property & Constructor Function Guards"}.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">
              The ECMAScript specification mandates precise abstract operations during the lifecycle of {"new.target Meta-Property & Constructor Function Guards"}. The table below outlines formal specification rules, engine runtime dynamics, and actionable developer invariants.
            </p>
              ...
              <table className="w-full text-left text-sm text-slate-300 border-collapse border border-slate-800">
                ...
                <tbody className="divide-y divide-slate-800 font-mono text-xs">
                  ...
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"new.target in Functions"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"References constructor function if called via new; else undefined"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Meta-property resolved during Execution Context creation"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Constructor defensive auto-instantiation"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"new.target in Classes"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"References the class invoked with new (even inside super())"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Enables polymorphic constructor reflection"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Creating Abstract Base Classes in vanilla JS"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"Legacy instanceof Invariant"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"this instanceof Constructor check"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Easily fooled by Object.create() or call()"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Superseded by new.target in modern ES6+"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="mt-6 p-5 rounded-xl bg-slate-900/80 border border-rose-900/40 space-y-3">
              ...
              <ul className="list-disc list-inside space-y-2 text-sm text-slate-300 leading-relaxed">
                <li><strong className="text-rose-300">{"SyntaxError Outside Functions:"}</strong> {"Using `new.target` in top-level script code outside of a function or class constructor throws SyntaxError."}</li>
                <li><strong className="text-rose-300">{"Global Mutation on Un-Newed Call:"}</strong> {"In non-strict mode, calling a constructor without `new` mutates the global object."}</li>
                <li><strong className="text-rose-300">{"Abstract Class Instantiation Error:"}</strong> {"Instantiating an abstract class directly should throw a custom Error when `new.target === AbstractClass`."}</li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="NewTargetConstructorFunctionGuardsDemoDemo.js"
            />
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-rose-950/20 border border-rose-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-rose-400 font-bold">
                <span>❌</span>
                <span>Anti-Pattern: {"Relying on 'this instanceof Fn' for Constructor Protection"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                {"`this instanceof Fn` can be fooled if a function is called with `Fn.call(new Fn())`."}
              </p>
              <pre className="p-4 rounded-xl bg-slate-950 border border-rose-900/50 text-xs font-mono text-rose-300 overflow-x-auto">
{"// ❌ AVOID: Imperfect check\nfunction User(name) {\n  if (!(this instanceof User)) return new User(name);\n  this.name = name;\n}"}
              </pre>
            </div>
            ...
            <div className="bg-emerald-950/20 border border-emerald-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-emerald-400 font-bold">
                <span>✓</span>
                <span>Senior Pro Practice: {"Use new.target for Flawless Constructor Validation"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                {"`new.target` is bulletproof and cannot be fooled by method borrowing."}
              </p>
...
{"// ✓ RECOMMENDED: Modern & Bulletproof\nfunction User(name, email) {\n  if (!new.target) {\n    return new User(name, email); // Safe auto-instantiation\n  }\n  this.name = name;\n  this.email = email;\n}\nconsole.log(User(\"Swadeep\", \"s@lab.com\") instanceof User); // true"}
              </pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-indigo-950/20 border border-indigo-800/40 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed mb-4">
              {"What value does `new.target` hold inside a parent class constructor when a child class extends the parent and `new Child()` is executed?"}
            </p>
            <div className="p-4 rounded-xl bg-slate-900 border border-indigo-900/50 text-xs sm:text-sm text-indigo-300 font-mono">
              💡 Hint: {"`new.target` points to the constructor that was originally invoked with `new` (the Child constructor)."}
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "new.target Meta-Property & Constructor Function Guards"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "new.target Meta-Property & Constructor Function Guards"}
            downloadFileName="002-001-topic14-note.txt"
          />
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <Teacher
            note={"In my 27+ years of mentoring software engineers at Coder & AccoTax in Barrackpore, I have consistently seen that mastering " + "new.target Meta-Property & Constructor Function Guards" + " separates code monkeys from genuine software engineers. " + "The `new.target` meta-property brings true object-oriented defensive design to JavaScript, ensuring constructors and abstract classes cannot be abused."}
          />
        </section>
