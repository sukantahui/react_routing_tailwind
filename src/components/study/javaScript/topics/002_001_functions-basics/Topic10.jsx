...
import demoCode from "./topic10_files/CurryingPartialApplicationPatternsDemoDemo.js?raw";
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 002_001_functions-basics · Topic 10</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Function Currying & Partial Application Patterns"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            {"Master functional currying and partial application: transforming n-ary functions into unary function chains, building reusable pipelines, and creating generic curry helpers."}
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                {"Swadeep wrote a custom `curry` function that failed when passed multiple arguments in a single call (e.g. `curried(1, 2)(3)`), throwing a `TypeError: curried(...) is not a function`."}
              </p>
              <p>
                {"Sukanta Hui walked through variadic arity inspection using `fn.length` and argument accumulation to create a production-grade recursive curry utility."}
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            ...
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 800 240" className="w-full h-auto" role="img" aria-label={"Function Currying & Partial Application Patterns"}>
                <defs>
                ...
                <text x="400" y="30" fill="#f8fafc" fontSize="14" fontWeight="bold" textAnchor="middle">{"Function Currying & Partial Application Patterns"} · Architecture &amp; State Transitions</text>
                ...
                <text x="140" y="135" fill="#fef3c7" fontSize="11" textAnchor="middle">{"Multi-Arity Target Function (a, b, c)"}</text>
                ...
                <text x="400" y="135" fill="#e0f2fe" fontSize="11" textAnchor="middle">{"Recursive Curry Accumulator"}</text>
                ...
                <text x="660" y="135" fill="#d1fae5" fontSize="11" textAnchor="middle">{"Unary / Variadic Invocation Sequence"}</text>
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure: Step-by-step architectural execution and state transitions for {"Function Currying & Partial Application Patterns"}.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">
              The ECMAScript specification mandates precise abstract operations during the lifecycle of {"Function Currying & Partial Application Patterns"}. The table below outlines formal specification rules, engine runtime dynamics, and actionable developer invariants.
            </p>
              ...
              <table className="w-full text-left text-sm text-slate-300 border-collapse border border-slate-800">
                ...
                <tbody className="divide-y divide-slate-800 font-mono text-xs">
                  ...
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"Currying"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Translates f(a, b, c) into f(a)(b)(c)"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Chained closures each capturing one argument"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Functional composition, configurator functions"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"Partial Application"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Fixes a subset of arguments returning function for the rest"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Single closure capturing preset arguments"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Pre-configuring database connections, loggers"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"Arity (fn.length)"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Number of declared parameters before first default"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Static function property inspected at runtime"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Automated auto-currying utilities"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="mt-6 p-5 rounded-xl bg-slate-900/80 border border-rose-900/40 space-y-3">
              ...
              <ul className="list-disc list-inside space-y-2 text-sm text-slate-300 leading-relaxed">
                <li><strong className="text-rose-300">{"fn.length Desync with Default Parameters:"}</strong> {"Default and rest parameters are NOT counted in `fn.length`, causing naive curry functions to terminate too early."}</li>
                <li><strong className="text-rose-300">{"Stack Overflow on Infinite Currying:"}</strong> {"Infinite currying without value serialization or terminal invocations exhausts memory."}</li>
                <li><strong className="text-rose-300">{"Loss of this in Curried Chains:"}</strong> {"Chained nested functions default their `this` context to undefined/global unless bound."}</li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="CurryingPartialApplicationPatternsDemoDemo.js"
            />
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-rose-950/20 border border-rose-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-rose-400 font-bold">
                <span>❌</span>
                <span>Anti-Pattern: {"Relying on fn.length with Rest Parameters or Defaults"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                {"A function with `(...args)` has `fn.length === 0`, causing standard curriers to execute immediately without waiting for arguments."}
              </p>
              <pre className="p-4 rounded-xl bg-slate-950 border border-rose-900/50 text-xs font-mono text-rose-300 overflow-x-auto">
{"// ❌ AVOID: fn.length is 0\nfunction add(...nums) {}\nconst curried = autoCurry(add); // Executes immediately!"}
              </pre>
            </div>
            ...
            <div className="bg-emerald-950/20 border border-emerald-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-emerald-400 font-bold">
                <span>✓</span>
                <span>Senior Pro Practice: {undefined}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                {undefined}
              </p>
...
{undefined}
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  {"Universal Auto-Curry Utility with Multi-Argument Flexibility"}
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              {"A production-grade curry function that handles both single-argument chaining `f(1)(2)(3)` and batched arguments `f(1, 2)(3)` seamlessly."}
            </p>

            <div className="rounded-xl border border-amber-900/50 bg-slate-950 p-4 font-mono text-xs text-amber-200 overflow-x-auto">
              <pre>{"function curry(fn) {\n  return function curried(...args) {\n    if (args.length >= fn.length) {\n      return fn.apply(this, args);\n    }\n    return function(...nextArgs) {\n      return curried.apply(this, args.concat(nextArgs));\n    };\n  };\n}\nconst multiply3 = (a, b, c) => a * b * c;\nconst curriedMult = curry(multiply3);\nconsole.log(curriedMult(2)(3)(4)); // 24\nconsole.log(curriedMult(2, 3)(4)); // 24"}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-indigo-950/20 border border-indigo-800/40 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed mb-4">
              {"How does `Function.prototype.length` behave when a function has parameters `(a, b = 10, c)` vs `(a, b, c = 10)`?"}
            </p>
            <div className="p-4 rounded-xl bg-slate-900 border border-indigo-900/50 text-xs sm:text-sm text-indigo-300 font-mono">
              💡 Hint: {"`fn.length` counts parameters only up to, but not including, the first parameter with a default initializer."}
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Function Currying & Partial Application Patterns"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Function Currying & Partial Application Patterns"}
            downloadFileName="002-001-topic10-note.txt"
          />
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <Teacher
            note={"In my 27+ years of mentoring software engineers at Coder & AccoTax in Barrackpore, I have consistently seen that mastering " + "Function Currying & Partial Application Patterns" + " separates code monkeys from genuine software engineers. " + "Currying transforms rigid functions into malleable, composable logic blocks that can be pre-configured across different layers of your application."}
          />
        </section>
