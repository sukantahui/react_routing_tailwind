...
import demoCode from "./topic11_files/FunctionCompositionPipeArchitectureDemoDemo.js?raw";
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 002_001_functions-basics · Topic 11</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Function Composition & Pipe Architecture (compose, pipe)"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            {"Architect clean data transformation pipelines using function composition (compose: right-to-left) and forward piping (pipe: left-to-right), with both synchronous and asynchronous support."}
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                {"Tuhina nested 5 mathematical functions `format(round(calculateTax(applyDiscount(price))))` and struggled to read, debug, and trace values across intermediate steps."}
              </p>
              <p>
                {"Sukanta Hui refactored the pipeline using `pipe(applyDiscount, calculateTax, round, format)`, creating a clean, readable left-to-right execution stream."}
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            ...
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 800 240" className="w-full h-auto" role="img" aria-label={"Function Composition & Pipe Architecture (compose, pipe)"}>
                <defs>
                ...
                <text x="400" y="30" fill="#f8fafc" fontSize="14" fontWeight="bold" textAnchor="middle">{"Function Composition & Pipe Architecture (compose, pipe)"} · Architecture &amp; State Transitions</text>
                ...
                <text x="140" y="135" fill="#fef3c7" fontSize="11" textAnchor="middle">{"Initial Raw Input Data"}</text>
                ...
                <text x="400" y="135" fill="#e0f2fe" fontSize="11" textAnchor="middle">{"Pure Transform Functions [f1 -> f2 -> f3]"}</text>
                ...
                <text x="660" y="135" fill="#d1fae5" fontSize="11" textAnchor="middle">{"Final Aggregated & Formatted Output"}</text>
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure: Step-by-step architectural execution and state transitions for {"Function Composition & Pipe Architecture (compose, pipe)"}.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">
              The ECMAScript specification mandates precise abstract operations during the lifecycle of {"Function Composition & Pipe Architecture (compose, pipe)"}. The table below outlines formal specification rules, engine runtime dynamics, and actionable developer invariants.
            </p>
              ...
              <table className="w-full text-left text-sm text-slate-300 border-collapse border border-slate-800">
                ...
                <tbody className="divide-y divide-slate-800 font-mono text-xs">
                  ...
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"pipe(...fns)"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Left-to-right evaluation: fns.reduce((val, fn) => fn(val), input)"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Linear iteration over transform array"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Standard data processing pipelines"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"compose(...fns)"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Right-to-left evaluation: fns.reduceRight((val, fn) => fn(val), input)"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Mathematical composition: (f ∘ g)(x)"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Redux middleware, functional programming libraries"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"pipeAsync(...fns)"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Asynchronous promise-chain pipeline"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Sequential promise resolution loop"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"API workflows, database transformations"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="mt-6 p-5 rounded-xl bg-slate-900/80 border border-rose-900/40 space-y-3">
              ...
              <ul className="list-disc list-inside space-y-2 text-sm text-slate-300 leading-relaxed">
                <li><strong className="text-rose-300">{"Unary Constraint Violation:"}</strong> {"All intermediate functions in a composition pipeline must be unary (accepting exactly one input and returning one output)."}</li>
                <li><strong className="text-rose-300">{"Unhandled Promise Rejections in Async Pipelines:"}</strong> {"Failing to catch errors in an async pipe halts the entire pipeline midway."}</li>
                <li><strong className="text-rose-300">{"Type Mismatch Between Stages:"}</strong> {"If function N outputs a string but function N+1 expects an array, runtime TypeErrors occur."}</li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="FunctionCompositionPipeArchitectureDemoDemo.js"
            />
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-rose-950/20 border border-rose-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-rose-400 font-bold">
                <span>❌</span>
                <span>Anti-Pattern: {"Deeply Nested Function Calls (The Onion Anti-Pattern)"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                {"Nesting calls inside-out obscures data flow and makes step-by-step testing frustrating."}
              </p>
              <pre className="p-4 rounded-xl bg-slate-950 border border-rose-900/50 text-xs font-mono text-rose-300 overflow-x-auto">
{"// ❌ AVOID: Difficult to read inside-out flow\nconst result = sanitize(capitalize(trim(slugify(rawInput))));"}
              </pre>
            </div>
            ...
            <div className="bg-emerald-950/20 border border-emerald-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-emerald-400 font-bold">
                <span>✓</span>
                <span>Senior Pro Practice: {"Linear Composition via Pipe Utility"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                {"Use pipe to clearly express the chronological order of data transformations."}
              </p>
...
{"// ✓ RECOMMENDED: Declarative and readable\nconst processSlug = pipe(trim, slugify, capitalize, sanitize);\nconsole.log(processSlug(\"  Advanced JavaScript Patterns  \"));"}
              </pre>
            </div>
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  {"Asynchronous Pipeline Engine with Built-in Error Recovery"}
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              {"An enterprise async pipe utility that handles both synchronous functions and Promise-returning functions while catching errors gracefully."}
            </p>

            <div className="rounded-xl border border-amber-900/50 bg-slate-950 p-4 font-mono text-xs text-amber-200 overflow-x-auto">
              <pre>{"const pipeAsync = (...fns) => (initialValue) =>\n  fns.reduce(\n    async (accPromise, fn) => fn(await accPromise),\n    Promise.resolve(initialValue)\n  );\n\nconst fetchUser = async (id) => ({ id, name: \"swadeep\" });\nconst enrichRole = async (user) => ({ ...user, role: \"Admin\" });\nconst formatBadge = (user) => `[${user.role}] ${user.name.toUpperCase()}`;\n\nconst buildBadge = pipeAsync(fetchUser, enrichRole, formatBadge);\nbuildBadge(101).then(console.log); // \"[ADMIN] SWADEEP\""}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-indigo-950/20 border border-indigo-800/40 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed mb-4">
              {"Why is `compose(f, g)(x)` mathematically equivalent to `f(g(x))` while `pipe(f, g)(x)` is equivalent to `g(f(x))`?"}
            </p>
            <div className="p-4 rounded-xl bg-slate-900 border border-indigo-900/50 text-xs sm:text-sm text-indigo-300 font-mono">
              💡 Hint: {"Composition follows standard mathematical notation from right to left, whereas piping follows UNIX pipeline left-to-right conventions."}
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Function Composition & Pipe Architecture (compose, pipe)"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Function Composition & Pipe Architecture (compose, pipe)"}
            downloadFileName="002-001-topic11-note.txt"
          />
        </section>
