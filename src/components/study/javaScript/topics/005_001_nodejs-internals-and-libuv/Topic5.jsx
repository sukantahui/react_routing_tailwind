...
import demoCode from "./topic5_files/NativeDiagnosticsProfilingWithChromeDevtoolsMemoryLeakAnalysisDemo.js?raw";

/**
 * Topic5 – Native Diagnostics, Profiling with Chrome DevTools & Memory Leak Analysis
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 005_001_nodejs-internals-and-libuv · Topic 5</span>
          </div>
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Native Diagnostics, Profiling with Chrome DevTools & Memory Leak Analysis"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Master the core mechanics, V8 execution rules, and practical enterprise workflows of <strong className="text-amber-300">{"Native Diagnostics, Profiling with Chrome DevTools & Memory Leak Analysis"}</strong> in modern JavaScript.
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            ...
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 840 280" className="w-full h-auto" role="img" aria-label="Native Diagnostics, Profiling with Chrome DevTools & Memory Leak Analysis Architecture">
                ...
                <text x="420" y="28" fill="#f8fafc" fontSize="14" fontWeight="bold" textAnchor="middle">
                  Native Diagnostics, Profiling with Chrome DevTools & Memory Leak Analysis · Asynchronous Lifecycle &amp; Event Loop Flow
                </text>
                <g transform="translate(40, 50)">
                  <rect width="210" height="100" rx="8" fill="#1e293b" stroke="#38bdf8" strokeWidth="1.5" />
                  <text x="105" y="24" fill="#38bdf8" fontSize="11" fontWeight="bold" textAnchor="middle">1. Request Initiation</text>
                  <rect x="15" y="38" width="180" height="48" rx="4" fill="#0f172a" stroke="#334155" />
                  <text x="25" y="58" fill="#7dd3fc" fontSize="10" fontFamily="monospace">fetch(url) / asyncFn()</text>
                  <text x="25" y="74" fill="#94a3b8" fontSize="9">Pending Promise &lt;pending&gt;</text>
                </g>
                ...
                <g transform="translate(315, 50)">
                  ...
                  <text x="110" y="24" fill="#fbbf24" fontSize="11" fontWeight="bold" textAnchor="middle">2. Microtask Resolution</text>
                  <rect x="15" y="38" width="190" height="48" rx="4" fill="#0f172a" stroke="#334155" />
                  <text x="25" y="58" fill="#fde68a" fontSize="10" fontFamily="monospace">Promise &lt;fulfilled&gt;</text>
                  <text x="25" y="74" fill="#94a3b8" fontSize="9">Pushed to Microtask Queue</text>
                </g>
                ...
                <g transform="translate(600, 50)">
                  ...
                  <text x="100" y="24" fill="#34d399" fontSize="11" fontWeight="bold" textAnchor="middle">3. Event Loop Drain</text>
                  <rect x="15" y="38" width="170" height="48" rx="4" fill="#0f172a" stroke="#10b981" />
                  <text x="25" y="58" fill="#a7f3d0" fontSize="10" fontFamily="monospace">await pauses execution</text>
                  <text x="25" y="74" fill="#94a3b8" fontSize="9">Drains before next frame</text>
                </g>
                <g transform="translate(40, 165)">
                  <rect width="760" height="95" rx="8" fill="#0f172a" stroke="#334155" />
                  <text x="380" y="26" fill="#f8fafc" fontSize="11" fontWeight="bold" textAnchor="middle">Event Loop Invariant: Microtasks vs Macrotasks Execution Priority</text>
                  <text x="380" y="46" fill="#94a3b8" fontSize="9" textAnchor="middle">Promises and queueMicrotask execute immediately after the current synchronous script completes, before any setTimeout or I/O callbacks.</text>
                  <text x="380" y="64" fill="#38bdf8" fontSize="9" textAnchor="middle">Handling errors with try/catch inside async functions guarantees rejection boundaries without unhandled promise crashes.</text>
                </g>
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 5.1: Architectural execution lifecycle, memory flow, and runtime mechanics for Native Diagnostics, Profiling with Chrome DevTools &amp; Memory Leak Analysis.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">
              The ECMAScript specification defines formal execution invariants for <strong className="text-amber-300">{"Native Diagnostics, Profiling with Chrome DevTools & Memory Leak Analysis"}</strong>. When a script runs, the JavaScript runtime establishes an execution context consisting of variable environments, lexical scopes, and binding environments.
            </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="NativeDiagnosticsProfilingWithChromeDevtoolsMemoryLeakAnalysisDemo.js"
            />
          </div>
        </section>
...
function checkStatus(val) {
  ...
  return val.toUpperCase();
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  {"Stream pipeline() with Auto-Cleanup"}
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              {"The stream.promises.pipeline() utility automatically handles stream backpressure, error propagation, and resource cleanup without leaking file descriptors."}
            </p>

            <div className="rounded-xl border border-amber-900/50 bg-slate-950 p-4 font-mono text-xs text-amber-200 overflow-x-auto">
              <pre>{"const { pipeline } = require('stream/promises');\nconst fs = require('fs');\nawait pipeline(fs.createReadStream('src.dat'), fs.createWriteStream('dst.dat'));"}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-indigo-950/20 border border-indigo-800/40 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed mb-4">
              If an application triggers thousands of operations per second using <strong className="text-amber-300">{"Native Diagnostics, Profiling with Chrome DevTools & Memory Leak Analysis"}</strong>, how does the V8 engine manage memory allocation and Garbage Collection pressure without causing UI frame drops (jank)?
            </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · Native Diagnostics, Profiling with Chrome DevTools & Memory Leak Analysis"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · Native Diagnostics, Profiling with Chrome DevTools & Memory Leak Analysis"}
            downloadFileName="005_001_nodejs-internals-and-libuv-topic5-note.txt"
          />
        </section>
