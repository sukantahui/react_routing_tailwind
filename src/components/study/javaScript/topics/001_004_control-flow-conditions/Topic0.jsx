...
import demoCode from "./topic0_files/UnderstandingIfElseStatementsDemo.js?raw";

/**
 * Topic0 – Understanding if / else Statements in JavaScript
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 001_004_control-flow-conditions · Topic 0</span>
          </div>
          ...
          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Master binary conditional branching, ToBoolean type coercion in if conditions, and block scoping rules.
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <p className="text-slate-300 leading-relaxed mb-4">
              Mastering <strong className="text-amber-300">if / else Statements</strong> empowers developers to architect clear, bug-free, and high-performance control flow trees. In enterprise JavaScript applications, clean conditional logic determines how state transitions occur, how errors are guarded against, and how business rules execute.
            </p>
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                Mentor <strong>Sukanta Hui</strong> demonstrated the exact decision tree on the whiteboard, proving that understanding <em>if / else Statements</em> produces maintainable and robust production applications.
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            <h2 className="text-lg sm:text-xl font-bold text-sky-400 mb-4 flex items-center gap-2">
              <span>📊</span> if / else Binary Branching & Condition Evaluation Pipeline
            </h2>
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 840 280" className="w-full h-auto" role="img" aria-label="Understanding if / else Statements in JavaScript Architecture">
                ...
                <text x="420" y="28" fill="#f8fafc" fontSize="15" fontWeight="bold" textAnchor="middle">
                  if / else Binary Branching & Condition Evaluation Pipeline
                </text>
                
                <g transform="translate(60, 50)">
                  <rect width="200" height="90" rx="10" fill="#1e293b" stroke="#38bdf8" strokeWidth="1.5" />
                  <text x="100" y="26" fill="#38bdf8" fontSize="12" fontWeight="bold" textAnchor="middle">1. Evaluate Condition</text>
                  <rect x="15" y="38" width="170" height="38" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="100" y="56" fill="#f8fafc" fontSize="10" textAnchor="middle">ToBoolean(condition)</text>
                  <text x="100" y="70" fill="#94a3b8" fontSize="9" textAnchor="middle">JumpIfFalse opcode</text>
                </g>

                {/* True Branch */}
                <path d="M 260 80 L 340 80" fill="none" stroke="#10b981" strokeWidth="2.5" />
                <g transform="translate(345, 45)">
                  <rect width="190" height="70" rx="8" fill="#064e3b" fillOpacity="0.4" stroke="#10b981" strokeWidth="1.5" />
                  <text x="95" y="26" fill="#34d399" fontSize="11" fontWeight="bold" textAnchor="middle">TRUE → If Block</text>
                  <text x="95" y="48" fill="#a7f3d0" fontSize="9" textAnchor="middle">Executes then branch</text>
                </g>

                {/* False Branch */}
                <path d="M 160 140 L 160 185 L 340 185" fill="none" stroke="#f43f5e" strokeWidth="2.5" />
                <g transform="translate(345, 150)">
                  <rect width="190" height="70" rx="8" fill="#881337" fillOpacity="0.4" stroke="#f43f5e" strokeWidth="1.5" />
                  <text x="95" y="26" fill="#fecdd3" fontSize="11" fontWeight="bold" textAnchor="middle">FALSE → Else Block</text>
                  <text x="95" y="48" fill="#fda4af" fontSize="9" textAnchor="middle">Executes fallback branch</text>
                </g>

                {/* Rejoin */}
                <path d="M 535 80 L 600 80 L 600 130 L 650 130" fill="none" stroke="#38bdf8" strokeWidth="2" />
                <path d="M 535 185 L 600 185 L 600 130" fill="none" stroke="#38bdf8" strokeWidth="2" />
                <g transform="translate(655, 105)">
                  <rect width="130" height="50" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="65" y="26" fill="#7dd3fc" fontSize="10" fontWeight="bold" textAnchor="middle">Rejoin Flow</text>
                  <text x="65" y="40" fill="#94a3b8" fontSize="8" textAnchor="middle">Next statement</text>
                </g>
    
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 0.1: Runtime decision branching, bytecode flow, and control architecture for if / else Statements.
            </p>
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  Yoda Conditions (Literal First) in Legacy Codebases
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              In older codebases, developers wrote if ('admin' === role) with the literal on the left. If they accidentally typed a single =, JavaScript threw a SyntaxError: Invalid left-hand side in assignment, preventing bugs.
            </p>
...
if (5 === count) { /* safe */ }`}</pre>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "if / else Statements"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "if / else Statements"}
            downloadFileName="001_004_control-flow-conditions-topic0-note.txt"
          />
        </section>
