...
import demoCode from "./topic7_files/ShortCircuitEvaluationInJavascriptDemo.js?raw";

/**
 * Topic7 – Short-Circuit Evaluation & Guard Patterns in JavaScript
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 001_004_control-flow-conditions · Topic 7</span>
          </div>
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Short-Circuit Evaluation & Guard Patterns in JavaScript"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Leverage logical operators for conditional function execution, default fallbacks, and safe property chaining.
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <p className="text-slate-300 leading-relaxed mb-4">
              Mastering <strong className="text-amber-300">Short-Circuit Evaluation</strong> empowers developers to architect clear, bug-free, and high-performance control flow trees. In enterprise JavaScript applications, clean conditional logic determines how state transitions occur, how errors are guarded against, and how business rules execute.
            </p>
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                During an intensive lab test, student <strong>Swadeep</strong> encountered a subtle architectural issue. Swadeep called user && user.profile.update(). When user.profile was undefined, it threw a TypeError. Sukanta Hui demonstrated modern optional chaining user?.profile?.update() combined with short-circuit evaluation.
              </p>
              <p>
                Mentor <strong>Sukanta Hui</strong> demonstrated the exact decision tree on the whiteboard, proving that understanding <em>Short-Circuit Evaluation</em> produces maintainable and robust production applications.
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            <h2 className="text-lg sm:text-xl font-bold text-sky-400 mb-4 flex items-center gap-2">
              <span>📊</span> Short-Circuit Guard Evaluation & Optional Chaining Flow
            </h2>
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 840 280" className="w-full h-auto" role="img" aria-label="Short-Circuit Evaluation & Guard Patterns in JavaScript Architecture">
                ...
                <text x="420" y="28" fill="#f8fafc" fontSize="15" fontWeight="bold" textAnchor="middle">
                  Short-Circuit Guard Evaluation & Optional Chaining Flow
                </text>
                
                <g transform="translate(60, 50)">
                  <rect width="330" height="180" rx="10" fill="#1e293b" stroke="#38bdf8" strokeWidth="1.5" />
                  <text x="165" y="26" fill="#38bdf8" fontSize="12" fontWeight="bold" textAnchor="middle">Short-Circuit Guard: isReady &amp;&amp; init()</text>
                  <rect x="15" y="42" width="300" height="120" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="25" y="65" fill="#7dd3fc" fontSize="10" fontFamily="monospace">const a = false &amp;&amp; fetchAPI();</text>
                  <text x="25" y="85" fill="#94a3b8" fontSize="10">fetchAPI() is NEVER called (Bypassed!)</text>
                  <text x="25" y="110" fill="#a7f3d0" fontSize="10" fontFamily="monospace">const b = true &amp;&amp; fetchAPI();</text>
                  <text x="25" y="130" fill="#34d399" fontSize="10">fetchAPI() runs and its return value is assigned</text>
                </g>

                <g transform="translate(440, 50)">
                  <rect width="340" height="180" rx="10" fill="#1e293b" stroke="#10b981" strokeWidth="1.5" />
                  <text x="170" y="26" fill="#34d399" fontSize="12" fontWeight="bold" textAnchor="middle">Modern Optional Chaining (?.) + Nullish (??)</text>
                  <rect x="15" y="42" width="310" height="120" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="25" y="65" fill="#a7f3d0" fontSize="10" fontFamily="monospace">const city = user?.address?.city ?? "N/A";</text>
                  <text x="25" y="90" fill="#e2e8f0" fontSize="10">• Safely navigates deep nested objects</text>
                  <text x="25" y="110" fill="#e2e8f0" fontSize="10">• Zero TypeError exceptions on null/undefined</text>
                  <text x="25" y="135" fill="#34d399" fontSize="9" fontWeight="bold">Replaces verbose 5-level if checks</text>
                </g>
    
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 7.1: Runtime decision branching, bytecode flow, and control architecture for Short-Circuit Evaluation.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="ShortCircuitEvaluationDemo.js"
            />
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  Safe Function Invocation with ?.()
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              You can safely execute callback functions that might be optional using optional chaining: onComplete?.(payload). If onComplete is undefined, it silently skips execution without throwing an error.
            </p>
...
function submitForm(onSuccess) {
  ...
  onSuccess?.({ status: 200 });
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Short-Circuit Evaluation"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Short-Circuit Evaluation"}
            downloadFileName="001_004_control-flow-conditions-topic7-note.txt"
          />
        </section>
