...
import demoCode from "./topic7_files/IterablesIteratorsTheSymboliteratorProtocolDemo.js?raw";

/**
 * Topic7 – Iterables, Iterators & the Symbol.iterator Protocol
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 001_005_loops-and-iteration · Topic 7</span>
          </div>
          ...
          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Master the ECMAScript iteration protocol, custom iterators, and generator functions consumed by for...of and spread syntax.
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                During an intensive lab session, student <strong>Abhronila</strong> tackled a complex real-world implementation challenge. Abhronila needed to make a custom Tree data structure iterable with for...of. Sukanta Hui guided her to implement [Symbol.iterator]() returning a generator that yields tree nodes via depth-first search.
              </p>
              <p>
                Mentor <strong>Sukanta Hui</strong> demonstrated the precise runtime execution trace on the whiteboard, proving that understanding <em>Iterables & Symbol.iterator</em> removes ambiguity and produces deterministic software architecture.
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            <h2 className="text-lg sm:text-xl font-bold text-sky-400 mb-4 flex items-center gap-2">
              <span>📊</span> ES6 Iterator Protocol Execution Architecture
            </h2>
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 840 280" className="w-full h-auto" role="img" aria-label="Iterables, Iterators & the Symbol.iterator Protocol Architecture">
                ...
                <text x="420" y="28" fill="#f8fafc" fontSize="15" fontWeight="bold" textAnchor="middle">
                  ES6 Iterator Protocol Execution Architecture
                </text>
                
                {/* Step 1: Iterable Object */}
                <g transform="translate(40, 60)">
                  <rect width="210" height="100" rx="10" fill="#1e293b" stroke="#38bdf8" strokeWidth="1.5" />
                  <text x="105" y="26" fill="#38bdf8" fontSize="12" fontWeight="bold" textAnchor="middle">1. Iterable Object</text>
                  <rect x="15" y="40" width="180" height="45" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="105" y="58" fill="#7dd3fc" fontSize="10" textAnchor="middle">obj[Symbol.iterator]()</text>
                  <text x="105" y="74" fill="#94a3b8" fontSize="9" textAnchor="middle">Returns Iterator Instance</text>
                </g>

                <path d="M 250 110 L 320 110" fill="none" stroke="#38bdf8" strokeWidth="2.5" />

                {/* Step 2: Iterator.next() */}
                <g transform="translate(325, 60)">
                  <rect width="210" height="100" rx="10" fill="#1e293b" stroke="#10b981" strokeWidth="1.5" />
                  <text x="105" y="26" fill="#34d399" fontSize="12" fontWeight="bold" textAnchor="middle">2. Iterator Object</text>
                  <rect x="15" y="40" width="180" height="45" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="105" y="58" fill="#a7f3d0" fontSize="10" textAnchor="middle">iterator.next()</text>
                  <text x="105" y="74" fill="#94a3b8" fontSize="9" textAnchor="middle">Called on every cycle</text>
                </g>

                <path d="M 535 110 L 605 110" fill="none" stroke="#f59e0b" strokeWidth="2.5" />

                {/* Step 3: Iterator Result */}
                <g transform="translate(610, 60)">
                  <rect width="190" height="100" rx="10" fill="#1e293b" stroke="#f59e0b" strokeWidth="1.5" />
                  <text x="95" y="26" fill="#fbbf24" fontSize="12" fontWeight="bold" textAnchor="middle">3. IteratorResult</text>
                  <rect x="15" y="40" width="160" height="45" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="95" y="58" fill="#fde68a" fontSize="10" textAnchor="middle">&#123; value, done &#125;</text>
                  <text x="95" y="74" fill="#94a3b8" fontSize="9" textAnchor="middle">done: false (continue) / true (exit)</text>
                </g>
    
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 7.1: Runtime execution lifecycle, memory flow, and control branching for Iterables &amp; Symbol.iterator.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="IterablesAndSymbolIteratorDemo.js"
            />
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  Generator Functions (function*) as Auto-Iterators
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              Writing a manual iterator object with closures can be verbose. Using a generator function (function*) automatically implements both the Iterable and Iterator protocols natively with yield.
            </p>
...
for (const x of myCollection) console.log(x); // 1, 2, 3`}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Iterables & Symbol.iterator"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Iterables & Symbol.iterator"}
            downloadFileName="001_005_loops-and-iteration-topic7-note.txt"
          />
        </section>
