...
import demoCode from "./topic5_files/ForofForinOverviewDemo.js?raw";

/**
 * Topic5 – for...of & for...in (Overview)
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 001_005_loops-and-iteration · Topic 5</span>
          </div>
          ...
          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Understand the core distinction between iterating over collection values (for...of) and object property keys (for...in).
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                During an intensive lab session, student <strong>Tuhina</strong> tackled a complex real-world implementation challenge. Tuhina wanted to print all characters of a string. Using for...in gave her indices '0', '1', '2'. Switching to for...of gave her the characters 'J', 'S', 'P' directly. Sukanta Hui explained the value-vs-key paradigm.
              </p>
              <p>
                Mentor <strong>Sukanta Hui</strong> demonstrated the precise runtime execution trace on the whiteboard, proving that understanding <em>for...of & for...in</em> removes ambiguity and produces deterministic software architecture.
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            <h2 className="text-lg sm:text-xl font-bold text-sky-400 mb-4 flex items-center gap-2">
              <span>📊</span> Values vs Keys Traversal Paradigm
            </h2>
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 840 280" className="w-full h-auto" role="img" aria-label="for...of & for...in (Overview) Architecture">
                ...
                <text x="420" y="28" fill="#f8fafc" fontSize="15" fontWeight="bold" textAnchor="middle">
                  Values vs Keys Traversal Paradigm
                </text>
                
                {/* for...of */}
                <g transform="translate(40, 50)">
                  <rect width="360" height="190" rx="10" fill="#1e293b" stroke="#10b981" strokeWidth="1.5" />
                  <text x="180" y="26" fill="#34d399" fontSize="13" fontWeight="bold" textAnchor="middle">for (const item of iterable)</text>
                  <rect x="20" y="45" width="320" height="120" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="35" y="70" fill="#38bdf8" fontSize="11" fontWeight="bold">Target: Iterables (Array, String, Set, Map)</text>
                  <text x="35" y="92" fill="#e2e8f0" fontSize="10">Yields: Element Values directly</text>
                  <text x="35" y="114" fill="#a7f3d0" fontSize="10">Protocol: Uses Symbol.iterator</text>
                  <text x="35" y="136" fill="#94a3b8" fontSize="10">Safe for Arrays: YES (ignores custom properties)</text>
                </g>

                {/* for...in */}
                <g transform="translate(440, 50)">
                  <rect width="360" height="190" rx="10" fill="#1e293b" stroke="#f59e0b" strokeWidth="1.5" />
                  <text x="180" y="26" fill="#fbbf24" fontSize="13" fontWeight="bold" textAnchor="middle">for (const key in object)</text>
                  <rect x="20" y="45" width="320" height="120" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="35" y="70" fill="#fbbf24" fontSize="11" fontWeight="bold">Target: Plain Objects</text>
                  <text x="35" y="92" fill="#e2e8f0" fontSize="10">Yields: String Property Keys (names)</text>
                  <text x="35" y="114" fill="#fca5a5" fontSize="10">Traversal: Walks [[Prototype]] chain</text>
                  <text x="35" y="136" fill="#f87171" fontSize="10">Safe for Arrays: NO (returns string indices)</text>
                </g>
    
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 5.1: Runtime execution lifecycle, memory flow, and control branching for for...of &amp; for...in.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="ForofAndForinOverviewDemo.js"
            />
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  Object.keys() + for...of vs for...in
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              Modern JavaScript development almost universally favors for (const key of Object.keys(obj)) over for...in because Object.keys() only returns own properties and completely ignores prototype chain pollution.
            </p>
...
for (const key of Object.keys(user)) {
  console.log(key, user[key]);
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "for...of & for...in"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "for...of & for...in"}
            downloadFileName="001_005_loops-and-iteration-topic5-note.txt"
          />
        </section>
