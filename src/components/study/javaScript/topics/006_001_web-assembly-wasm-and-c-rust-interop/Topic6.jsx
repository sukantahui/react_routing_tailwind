...
import demoCode from "./topic6_files/WasiWebassemblySystemInterfaceRunningWebassemblyOutsideTheBrowserDemo.js?raw";

/**
 * Topic6 – WASI (WebAssembly System Interface): Running WebAssembly Outside the Browser
 * Module: 006_001_web-assembly-wasm-and-c-rust-interop
 *
 * @component
...
export default function Topic6() {
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 006_001_web-assembly-wasm-and-c-rust-interop · Topic 6</span>
          </div>
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"WASI (WebAssembly System Interface): Running WebAssembly Outside the Browser"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Master the core mechanics, V8 execution rules, and practical enterprise workflows of <strong className="text-amber-300">{"WASI (WebAssembly System Interface): Running WebAssembly Outside the Browser"}</strong> in modern JavaScript.
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            ...
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 800 240" className="w-full h-auto" role="img" aria-label="WebAssembly Execution Pipeline">
  <defs>
    <linearGradient id="wasmGrad_6" x1="0%" y1="0%" x2="100%" y2="100%">
  ...
  <text x="400" y="32" fill="#f8fafc" fontSize="16" fontWeight="bold" textAnchor="middle">WebAssembly (WASM) Linear Memory &amp; JS Interop Architecture</text>
  <g transform="translate(50, 65)">
    <rect width="180" height="145" rx="12" fill="#1e293b" stroke="#f59e0b" strokeWidth="1.5" />
    <text x="90" y="30" fill="#fbbf24" fontSize="13" fontWeight="bold" textAnchor="middle">C++ / Rust Source</text>
    <rect x="20" y="48" width="140" height="24" rx="4" fill="#0f172a" />
    <text x="90" y="64" fill="#e2e8f0" fontSize="10" textAnchor="middle">wasm-pack / emcc</text>
    <rect x="20" y="80" width="140" height="24" rx="4" fill="#0f172a" />
    <text x="90" y="96" fill="#cbd5e1" fontSize="10" textAnchor="middle">LLVM Compilation</text>
    <rect x="20" y="112" width="140" height="24" rx="4" fill="#d97706" />
    <text x="90" y="128" fill="#fffbeb" fontSize="10" fontWeight="bold" textAnchor="middle">.wasm Binary Module</text>
  </g>
  <g transform="translate(290, 65)">
    <rect width="220" height="145" rx="12" fill="url(#wasmGrad_6)" stroke="#818cf8" strokeWidth="1.5" />
    <text x="110" y="30" fill="#e0e7ff" fontSize="13" fontWeight="bold" textAnchor="middle">Shared Linear Memory</text>
    <rect x="20" y="48" width="180" height="85" rx="8" fill="#0f172a" stroke="#4f46e5" />
    <text x="110" y="75" fill="#38bdf8" fontSize="11" fontWeight="bold" textAnchor="middle">WebAssembly.Memory</text>
    <text x="110" y="95" fill="#a5b4fc" fontSize="10" textAnchor="middle">ArrayBuffer (Zero-Copy Pointer)</text>
    <text x="110" y="115" fill="#34d399" fontSize="10" textAnchor="middle">SIMD 128-bit Vectors</text>
  </g>
  <g transform="translate(570, 65)">
    <rect width="180" height="145" rx="12" fill="#1e293b" stroke="#38bdf8" strokeWidth="1.5" />
    <text x="90" y="30" fill="#38bdf8" fontSize="13" fontWeight="bold" textAnchor="middle">JavaScript Engine (V8)</text>
    <rect x="20" y="48" width="140" height="24" rx="4" fill="#0369a1" />
    <text x="90" y="64" fill="#e0f2fe" fontSize="10" textAnchor="middle">WebAssembly.instantiate()</text>
    <rect x="20" y="80" width="140" height="24" rx="4" fill="#0284c7" />
    <text x="90" y="96" fill="#e0f2fe" fontSize="10" textAnchor="middle">TypedArray Views (Uint8)</text>
    <rect x="20" y="112" width="140" height="24" rx="4" fill="#38bdf8" />
    <text x="90" y="128" fill="#082f49" fontSize="10" fontWeight="bold" textAnchor="middle">60fps Physics / DSP</text>
  </g>
</svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 6.1: Architectural execution lifecycle, memory flow, and runtime mechanics for WASI (WebAssembly System Interface): Running WebAssembly Outside the Browser.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">
              The ECMAScript specification defines formal execution invariants for <strong className="text-amber-300">{"WASI (WebAssembly System Interface): Running WebAssembly Outside the Browser"}</strong>. When a script runs, the JavaScript runtime establishes an execution context consisting of variable environments, lexical scopes, and binding environments.
            </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="WasiWebassemblySystemInterfaceRunningWebassemblyOutsideTheBrowserDemo.js"
            />
          </div>
        </section>
...
function checkStatus(val) {
  ...
  return val.toUpperCase();
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  {"Direct WASM Memory Buffer Views"}
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              {"Directly reading WASM linear memory in JavaScript using TypedArray views without copying a single byte across boundaries."}
            </p>

            <div className="rounded-xl border border-amber-900/50 bg-slate-950 p-4 font-mono text-xs text-amber-200 overflow-x-auto">
              <pre>{"const wasmMemory = new Uint8Array(wasmInstance.exports.memory.buffer, offset, length);\nconsole.log('Zero-copy bytes read from WASM:', wasmMemory.slice(0, 10));"}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-indigo-950/20 border border-indigo-800/40 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed mb-4">
              If an application triggers thousands of operations per second using <strong className="text-amber-300">{"WASI (WebAssembly System Interface): Running WebAssembly Outside the Browser"}</strong>, how does the V8 engine manage memory allocation and Garbage Collection pressure without causing UI frame drops (jank)?
            </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · WASI (WebAssembly System Interface): Running WebAssembly Outside the Browser"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · WASI (WebAssembly System Interface): Running WebAssembly Outside the Browser"}
            downloadFileName="006_001_web-assembly-wasm-and-c-rust-interop-topic6-note.txt"
          />
        </section>
