...
import demoCode from "./topic6_files/CryptographicSignaturesAsymmetricKeyPairsRsaecdsaWebCryptoDemo.js?raw";

/**
 * Topic6 – Cryptographic Signatures, Asymmetric Key Pairs (RSAECDSA) & Web Crypto
 * Module: 005_004_authentication-security-and-cryptography
 *
 * @component
...
export default function Topic6() {
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 005_004_authentication-security-and-cryptography · Topic 6</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Cryptographic Signatures, Asymmetric Key Pairs (RSA/ECDSA) & Web Crypto"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Master the core mechanics, V8 execution rules, and practical enterprise workflows of <strong className="text-amber-300">{"Cryptographic Signatures, Asymmetric Key Pairs (RSA/ECDSA) & Web Crypto"}</strong> in modern JavaScript.
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            ...
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 800 240" className="w-full h-auto" role="img" aria-label="AI &amp; WebGPU Parallel Compute Pipeline">
  <defs>
    <linearGradient id="aiGrad_6" x1="0%" y1="0%" x2="100%" y2="100%">
  ...
  <text x="400" y="32" fill="#f8fafc" fontSize="16" fontWeight="bold" textAnchor="middle">Client-Side AI &amp; WebGPU Compute Pipeline (In-Browser Execution)</text>
  <g transform="translate(50, 65)">
    <rect width="200" height="145" rx="12" fill="#1e293b" stroke="#f43f5e" strokeWidth="1.5" />
    <text x="100" y="30" fill="#fda4af" fontSize="13" fontWeight="bold" textAnchor="middle">Quantized Weights</text>
    <rect x="20" y="48" width="160" height="24" rx="4" fill="#0f172a" />
    <text x="100" y="64" fill="#cbd5e1" fontSize="10" textAnchor="middle">ONNX / SafeTensors (q4f16)</text>
    <rect x="20" y="80" width="160" height="24" rx="4" fill="#0f172a" />
    <text x="100" y="96" fill="#cbd5e1" fontSize="10" textAnchor="middle">BPE Tokenizer in JS</text>
    <rect x="20" y="112" width="160" height="24" rx="4" fill="#e11d48" />
    <text x="100" y="128" fill="#fff" fontSize="10" fontWeight="bold" textAnchor="middle">Embedding Vectors (768d)</text>
  </g>
  <g transform="translate(300, 65)">
    <rect width="200" height="145" rx="12" fill="url(#aiGrad_6)" stroke="#f472b6" strokeWidth="1.5" />
    <text x="100" y="30" fill="#fdf2f8" fontSize="13" fontWeight="bold" textAnchor="middle">WebGPU Compute Shader</text>
    <rect x="20" y="48" width="160" height="85" rx="8" fill="#0f172a" stroke="#db2777" />
    <text x="100" y="75" fill="#f472b6" fontSize="11" fontWeight="bold" textAnchor="middle">WGSL Matrix Multiplication</text>
    <text x="100" y="95" fill="#fbcfe8" fontSize="10" textAnchor="middle">Parallel Workgroups (64)</text>
    <text x="100" y="115" fill="#34d399" fontSize="10" textAnchor="middle">Direct GPU VRAM Buffer</text>
  </g>
  <g transform="translate(550, 65)">
    <rect width="200" height="145" rx="12" fill="#1e293b" stroke="#38bdf8" strokeWidth="1.5" />
    <text x="100" y="30" fill="#38bdf8" fontSize="13" fontWeight="bold" textAnchor="middle">Client Application</text>
    <rect x="20" y="48" width="160" height="24" rx="4" fill="#0284c7" />
    <text x="100" y="64" fill="#e0f2fe" fontSize="10" textAnchor="middle">Cosine Similarity Search</text>
    <rect x="20" y="80" width="160" height="24" rx="4" fill="#0369a1" />
    <text x="100" y="96" fill="#e0f2fe" fontSize="10" textAnchor="middle">Local In-Browser LLM Chat</text>
    <rect x="20" y="112" width="160" height="24" rx="4" fill="#38bdf8" />
    <text x="100" y="128" fill="#082f49" fontSize="10" fontWeight="bold" textAnchor="middle">Zero Cloud Server Costs</text>
  </g>
</svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 6.1: Architectural execution lifecycle, memory flow, and runtime mechanics for Cryptographic Signatures, Asymmetric Key Pairs (RSA/ECDSA) &amp; Web Crypto.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">
              The ECMAScript specification defines formal execution invariants for <strong className="text-amber-300">{"Cryptographic Signatures, Asymmetric Key Pairs (RSA/ECDSA) & Web Crypto"}</strong>. When a script runs, the JavaScript runtime establishes an execution context consisting of variable environments, lexical scopes, and binding environments.
            </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="CryptographicSignaturesAsymmetricKeyPairsRsaecdsaWebCryptoDemo.js"
            />
          </div>
        </section>
...
function checkStatus(val) {
  ...
  return val.toUpperCase();
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  {"Argon2id Memory Hardness Parameter Tuning"}
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              {"Argon2id combines resistance to side-channel and GPU-cracking attacks by enforcing configurable memory cost parameters (m=65536, t=3, p=4)."}
            </p>

            <div className="rounded-xl border border-amber-900/50 bg-slate-950 p-4 font-mono text-xs text-amber-200 overflow-x-auto">
              <pre>{"import argon2 from 'argon2';\nconst hash = await argon2.hash('SecretStudentPassword#2026', { type: argon2.argon2id, memoryCost: 2 ** 16 });"}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-indigo-950/20 border border-indigo-800/40 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed mb-4">
              If an application triggers thousands of operations per second using <strong className="text-amber-300">{"Cryptographic Signatures, Asymmetric Key Pairs (RSA/ECDSA) & Web Crypto"}</strong>, how does the V8 engine manage memory allocation and Garbage Collection pressure without causing UI frame drops (jank)?
            </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · Cryptographic Signatures, Asymmetric Key Pairs (RSA/ECDSA) & Web Crypto"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · Cryptographic Signatures, Asymmetric Key Pairs (RSA/ECDSA) & Web Crypto"}
            downloadFileName="005_004_authentication-security-and-cryptography-topic6-note.txt"
          />
        </section>
