...
import demoCode from "./topic0_files/ExecutionContextLexicalEnvironmentCreationVsExecutionPhasesDemo.js?raw";

/**
 * Topic0 – Execution Context & Lexical Environment (Creation vs Execution Phases)
 * Module: 004_001_javascript-internals-and-advanced-concepts
 *
 * @component
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 004_001_javascript-internals-and-advanced-concepts · Topic 0</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Execution Context & Lexical Environment (Creation vs Execution Phases)"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Master the core mechanics, V8 execution rules, and practical enterprise workflows of <strong className="text-amber-300">{"Execution Context & Lexical Environment (Creation vs Execution Phases)"}</strong> in modern JavaScript.
          </p>
  ...
  <g transform="translate(650, 70)">
    ...
    <text x="60" y="85" fill="#94a3b8" fontSize="10" textAnchor="middle">Code (ASM/JIT)</text>
            ...
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 0.1: Architectural execution lifecycle, memory flow, and runtime mechanics for Execution Context &amp; Lexical Environment (Creation vs Execution Phases).
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">
              The ECMAScript specification defines formal execution invariants for <strong className="text-amber-300">{"Execution Context & Lexical Environment (Creation vs Execution Phases)"}</strong>. When a script runs, the JavaScript runtime establishes an execution context consisting of variable environments, lexical scopes, and binding environments.
            </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="ExecutionContextLexicalEnvironmentCreationVsExecutionPhasesDemo.js"
            />
          </div>
        </section>
...
function checkStatus(val) {
  ...
  return val.toUpperCase();
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  {"V8 Hidden Classes & Monomorphic Inline Caching (10x Speed)"}
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              {"Initialize object properties in the exact same order to allow the V8 engine to reuse hidden shapes and optimize property lookups."}
            </p>

            <div className="rounded-xl border border-amber-900/50 bg-slate-950 p-4 font-mono text-xs text-amber-200 overflow-x-auto">
              <pre>{"// GOOD: Always initialize a, then b\nclass Point { constructor(x, y) { this.x = x; this.y = y; } }"}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-indigo-950/20 border border-indigo-800/40 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed mb-4">
              If an application triggers thousands of operations per second using <strong className="text-amber-300">{"Execution Context & Lexical Environment (Creation vs Execution Phases)"}</strong>, how does the V8 engine manage memory allocation and Garbage Collection pressure without causing UI frame drops (jank)?
            </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · Execution Context & Lexical Environment (Creation vs Execution Phases)"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · Execution Context & Lexical Environment (Creation vs Execution Phases)"}
            downloadFileName="004_001_javascript-internals-and-advanced-concepts-topic0-note.txt"
          />
        </section>
