...
import demoCode from "./topic8_files/BitwiseOperatorsLowLevelFlagsDemo.js?raw";

/**
 * Topic8 – Bitwise Operators (&, |, ^, ~, <<, >>, >>>) & Low-Level Flags
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 001_003_operators-and-expressions · Topic 8</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Bitwise Operators (&, |, ^, ~, <<, >>, >>>) & Low-Level Flags"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Master 32-bit signed integers, bit masks, permission flags, two's complement, and zero-fill right shift (&gt;&gt;&gt;).
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                During a lab session, student <strong>Abhronila</strong> encountered a subtle logical glitch. Abhronila needed to manage user permissions (READ, WRITE, DELETE, EXECUTE) in a fast bitfield. Sukanta Hui demonstrated how bitwise OR (|) combines flags, AND (&) tests flags, and XOR (^) toggles flags in O(1) time.
              </p>
              <p>
                Mentor <strong>Sukanta Hui</strong> demonstrated the exact step-by-step operand evaluation on the whiteboard, explaining why mastering <em>Bitwise Operators</em> eliminates ambiguity and prevents production regressions.
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            <h2 className="text-lg sm:text-xl font-bold text-sky-400 mb-4 flex items-center gap-2">
              <span>📊</span> 32-bit Bitwise Register Operations & Flag Masking
            </h2>
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 840 280" className="w-full h-auto" role="img" aria-label="Bitwise Operators (&, |, ^, ~, <<, >>, >>>) & Low-Level Flags Architecture">
                ...
                <text x="420" y="28" fill="#f8fafc" fontSize="15" fontWeight="bold" textAnchor="middle">
                  32-bit Bitwise Register Operations & Flag Masking
                </text>
                
                <g transform="translate(60, 50)">
                  <rect width="720" height="190" rx="10" fill="#1e293b" stroke="#38bdf8" strokeWidth="1.5" />
                  <text x="360" y="26" fill="#38bdf8" fontSize="13" fontWeight="bold" textAnchor="middle">Bitwise Permission Flag Architecture</text>
                  
                  <rect x="30" y="45" width="660" height="30" rx="4" fill="#0f172a" />
                  <text x="50" y="65" fill="#7dd3fc" fontSize="11" fontFamily="monospace">READ = 1 (0b0001) | WRITE = 2 (0b0010) | EXECUTE = 4 (0b0100) | ADMIN = 8 (0b1000)</text>

                  <g transform="translate(30, 85)">
                    <rect width="210" height="90" rx="6" fill="#0f172a" stroke="#10b981" />
                    <text x="105" y="24" fill="#34d399" fontSize="11" fontWeight="bold" textAnchor="middle">1. Add Flag (|)</text>
                    <text x="15" y="48" fill="#e2e8f0" fontSize="10" fontFamily="monospace">perm = READ | WRITE</text>
                    <text x="15" y="70" fill="#94a3b8" fontSize="9">Result: 0b0011 (3)</text>
                  </g>

                  <g transform="translate(255, 85)">
                    <rect width="210" height="90" rx="6" fill="#0f172a" stroke="#f59e0b" />
                    <text x="105" y="24" fill="#fbbf24" fontSize="11" fontWeight="bold" textAnchor="middle">2. Check Flag (&amp;)</text>
                    <text x="15" y="48" fill="#e2e8f0" fontSize="10" fontFamily="monospace">(perm &amp; WRITE) !== 0</text>
                    <text x="15" y="70" fill="#34d399" fontSize="9">Result: true (Has Write)</text>
                  </g>

                  <g transform="translate(480, 85)">
                    <rect width="210" height="90" rx="6" fill="#0f172a" stroke="#f43f5e" />
                    <text x="105" y="24" fill="#f43f5e" fontSize="11" fontWeight="bold" textAnchor="middle">3. Remove Flag (&amp; ~)</text>
                    <text x="15" y="48" fill="#e2e8f0" fontSize="10" fontFamily="monospace">perm &amp;= ~WRITE</text>
                    <text x="15" y="70" fill="#94a3b8" fontSize="9">Result: 0b0001 (Read only)</text>
                  </g>
                </g>
            ...
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 8.1: Runtime execution lifecycle, memory flow, and operator evaluation rules for Bitwise Operators.
            </p>
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  Fast Integer Truncation with Bitwise Double NOT (~~) or OR 0
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              Because bitwise operators coerce numbers to 32-bit integers, ~~4.9 or 4.9 | 0 quickly truncates the decimal part, performing faster than Math.floor for positive numbers in performance-critical graphics loops.
            </p>
...
console.log(~~4.9);    // 4
console.log(4.9 | 0);   // 4
console.log(15 >> 1);   // 7 (Fast integer division by 2)`}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Bitwise Operators"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Bitwise Operators"}
            downloadFileName="001_003_operators-and-expressions-topic8-note.txt"
          />
        </section>
