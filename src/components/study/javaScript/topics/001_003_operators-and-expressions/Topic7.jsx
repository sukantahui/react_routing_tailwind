...
import demoCode from "./topic7_files/LogicalAssignmentOperatorsModernCompoundShortcutsDemo.js?raw";

/**
 * Topic7 – Logical Assignment Operators (&&=, ||=, ??=) & Modern Compound Shortcuts
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 001_003_operators-and-expressions · Topic 7</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Logical Assignment Operators (&&=, ||=, ??=) & Modern Compound Shortcuts"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Master ES2021 logical assignment operators: &&= (assign if truthy), ||= (assign if falsy), and ??= (assign if nullish).
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                During a lab session, student <strong>Swadeep</strong> encountered a subtle logical glitch. Swadeep set options.port = options.port || 8080. If port was 0, it was overwritten! Sukanta Hui introduced options.port ??= 8080, which only assigns when options.port is null or undefined.
              </p>
              <p>
                Mentor <strong>Sukanta Hui</strong> demonstrated the exact step-by-step operand evaluation on the whiteboard, explaining why mastering <em>Logical Assignments</em> eliminates ambiguity and prevents production regressions.
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            <h2 className="text-lg sm:text-xl font-bold text-sky-400 mb-4 flex items-center gap-2">
              <span>📊</span> Logical Assignment Decision Pipeline (&&=, ||=, ??=)
            </h2>
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 840 280" className="w-full h-auto" role="img" aria-label="Logical Assignment Operators (&&=, ||=, ??=) & Modern Compound Shortcuts Architecture">
                ...
                <text x="420" y="28" fill="#f8fafc" fontSize="15" fontWeight="bold" textAnchor="middle">
                  Logical Assignment Decision Pipeline (&&=, ||=, ??=)
                </text>
                
                <g transform="translate(40, 50)">
                  <rect width="230" height="180" rx="10" fill="#1e293b" stroke="#38bdf8" strokeWidth="1.5" />
                  <text x="115" y="26" fill="#38bdf8" fontSize="12" fontWeight="bold" textAnchor="middle">1. a ??= b (Nullish)</text>
                  <rect x="15" y="42" width="200" height="120" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="25" y="65" fill="#7dd3fc" fontSize="10" fontFamily="monospace">a ?? (a = b)</text>
                  <text x="25" y="85" fill="#e2e8f0" fontSize="10">Assigns ONLY if a is:</text>
                  <text x="25" y="105" fill="#fda4af" fontSize="10">• null</text>
                  <text x="25" y="125" fill="#fda4af" fontSize="10">• undefined</text>
                  <text x="25" y="145" fill="#34d399" fontSize="9">Preserves 0, false, ""!</text>
                </g>

                <g transform="translate(300, 50)">
                  <rect width="230" height="180" rx="10" fill="#1e293b" stroke="#f59e0b" strokeWidth="1.5" />
                  <text x="115" y="26" fill="#fbbf24" fontSize="12" fontWeight="bold" textAnchor="middle">2. a ||= b (Or Assign)</text>
                  <rect x="15" y="42" width="200" height="120" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="25" y="65" fill="#fde68a" fontSize="10" fontFamily="monospace">a || (a = b)</text>
                  <text x="25" y="85" fill="#e2e8f0" fontSize="10">Assigns if a is falsy:</text>
                  <text x="25" y="105" fill="#fda4af" fontSize="10">• 0, false, "", NaN</text>
                  <text x="25" y="125" fill="#fda4af" fontSize="10">• null, undefined</text>
                  <text x="25" y="145" fill="#fbbf24" fontSize="9">Default string/object init</text>
                </g>

                <g transform="translate(560, 50)">
                  <rect width="240" height="180" rx="10" fill="#1e293b" stroke="#10b981" strokeWidth="1.5" />
                  <text x="120" y="26" fill="#34d399" fontSize="12" fontWeight="bold" textAnchor="middle">3. a &amp;&amp;= b (And Assign)</text>
                  <rect x="15" y="42" width="210" height="120" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="25" y="65" fill="#a7f3d0" fontSize="10" fontFamily="monospace">a &amp;&amp; (a = b)</text>
                  <text x="25" y="85" fill="#e2e8f0" fontSize="10">Assigns ONLY if a is truthy</text>
                  <text x="25" y="105" fill="#e2e8f0" fontSize="10">Used for updating state:</text>
                  <text x="25" y="125" fill="#a7f3d0" fontSize="10">user &amp;&amp;= fetchProfile(user)</text>
                  <text x="25" y="145" fill="#34d399" fontSize="9">Safe mutator shortcut</text>
                </g>
    
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 7.1: Runtime execution lifecycle, memory flow, and operator evaluation rules for Logical Assignments.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="LogicalAssignmentOperatorsDemo.js"
            />
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  No Unnecessary Setter Invocations with ??=
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              Unlike a = a ?? b which ALWAYS invokes the property setter on an object, a ??= b only performs the property assignment if the current value is nullish, avoiding trigger overhead on reactive proxies.
            </p>
...
config.maxRetries ??= 5;
console.log(config.maxRetries); // 3 (Never reassigned!)`}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Logical Assignments"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Logical Assignments"}
            downloadFileName="001_003_operators-and-expressions-topic7-note.txt"
          />
        </section>
