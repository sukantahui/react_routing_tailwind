...
import demoCode from "./topic4_files/StringConcatenationTemplateLiteralsDemo.js?raw";

/**
 * Topic4 – String Concatenation & Template Literals
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 001_003_operators-and-expressions · Topic 4</span>
          </div>
          ...
          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Master ES6 template literals, multiline interpolation, expression embedding, and tagged template functions.
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                During a lab session, student <strong>Swadeep</strong> encountered a subtle logical glitch. Swadeep built an SQL query string with standard concatenation: 'SELECT * FROM users WHERE id = ' + userId. Sukanta Hui demonstrated how tagged template literals provide compile-time sanitization preventing SQL injection.
              </p>
              <p>
                Mentor <strong>Sukanta Hui</strong> demonstrated the exact step-by-step operand evaluation on the whiteboard, explaining why mastering <em>Strings & Templates</em> eliminates ambiguity and prevents production regressions.
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            <h2 className="text-lg sm:text-xl font-bold text-sky-400 mb-4 flex items-center gap-2">
              <span>📊</span> Template Literal AST Tokenization & Tagged Function Flow
            </h2>
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 840 280" className="w-full h-auto" role="img" aria-label="String Concatenation & Template Literals Architecture">
                ...
                <text x="420" y="28" fill="#f8fafc" fontSize="15" fontWeight="bold" textAnchor="middle">
                  Template Literal AST Tokenization & Tagged Function Flow
                </text>
                
                <g transform="translate(40, 60)">
                  <rect width="220" height="100" rx="10" fill="#1e293b" stroke="#38bdf8" strokeWidth="1.5" />
                  <text x="110" y="26" fill="#38bdf8" fontSize="12" fontWeight="bold" textAnchor="middle">1. Template String</text>
                  <rect x="15" y="40" width="190" height="45" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="110" y="58" fill="#7dd3fc" fontSize="10" textAnchor="middle">`Hello \$&#123;user&#125;, total: \$&#123;n&#125;`</text>
                  <text x="110" y="74" fill="#94a3b8" fontSize="9" textAnchor="middle">Dynamic Embedded Expressions</text>
                </g>

                <path d="M 260 110 L 320 110" fill="none" stroke="#38bdf8" strokeWidth="2.5" />

                <g transform="translate(325, 60)">
                  <rect width="210" height="100" rx="10" fill="#1e293b" stroke="#f59e0b" strokeWidth="1.5" />
                  <text x="105" y="26" fill="#fbbf24" fontSize="12" fontWeight="bold" textAnchor="middle">2. Tagged Parser</text>
                  <rect x="15" y="40" width="180" height="45" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="105" y="58" fill="#fde68a" fontSize="10" textAnchor="middle">tagFn(strings, ...values)</text>
                  <text x="105" y="74" fill="#94a3b8" fontSize="9" textAnchor="middle">Inspects literal chunks &amp; vars</text>
                </g>

                <path d="M 535 110 L 595 110" fill="none" stroke="#10b981" strokeWidth="2.5" />

                <g transform="translate(600, 60)">
                  <rect width="200" height="100" rx="10" fill="#1e293b" stroke="#10b981" strokeWidth="1.5" />
                  <text x="100" y="26" fill="#34d399" fontSize="12" fontWeight="bold" textAnchor="middle">3. Sanitized Output</text>
                  <rect x="15" y="40" width="170" height="45" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="100" y="58" fill="#a7f3d0" fontSize="10" textAnchor="middle">Clean Escaped HTML / SQL</text>
                  <text x="100" y="74" fill="#94a3b8" fontSize="9" textAnchor="middle">Zero XSS / Injection risks</text>
                </g>
    
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 4.1: Runtime execution lifecycle, memory flow, and operator evaluation rules for Strings &amp; Templates.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="StringConcatenationTemplateLiteralsDemo.js"
            />
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  Tagged Template Functions for Internationalization & Styling
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              Popular libraries like styled-components and graphql use tagged template literals (css\`...\`, gql\`...\`) to parse domain-specific syntax trees at runtime.
            </p>
...
console.log(sanitize(['User said: ', ''], comment)); // Safe HTML!`}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Strings & Templates"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Strings & Templates"}
            downloadFileName="001_003_operators-and-expressions-topic4-note.txt"
          />
        </section>
