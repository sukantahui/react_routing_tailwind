...
import demoCode from "./topic0_files/ArithmeticOperatorsInJavascriptDemo.js?raw";

/**
 * Topic0 – Arithmetic Operators in JavaScript (+, -, *, /, %, **)
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 001_003_operators-and-expressions · Topic 0</span>
          </div>
          ...
          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Master arithmetic evaluation, exponentiation (**), integer division, modulo vs remainder, and unary numeric promotion.
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                During a lab session, student <strong>Swadeep</strong> encountered a subtle logical glitch. Swadeep used the modulo operator % on a negative number (-7 % 3) and expected 2 (mathematical modulo), but got -1. Sukanta Hui explained that JavaScript's % operator is the truncated remainder operator, not mathematical Euclidean modulo.
              </p>
              <p>
                Mentor <strong>Sukanta Hui</strong> demonstrated the exact step-by-step operand evaluation on the whiteboard, explaining why mastering <em>Arithmetic Operators</em> eliminates ambiguity and prevents production regressions.
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            <h2 className="text-lg sm:text-xl font-bold text-sky-400 mb-4 flex items-center gap-2">
              <span>📊</span> Arithmetic Evaluation & Remainder vs Exponentiation Pipeline
            </h2>
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 840 280" className="w-full h-auto" role="img" aria-label="Arithmetic Operators in JavaScript (+, -, *, /, %, **) Architecture">
                ...
                <text x="420" y="28" fill="#f8fafc" fontSize="15" fontWeight="bold" textAnchor="middle">
                  Arithmetic Evaluation & Remainder vs Exponentiation Pipeline
                </text>
                
                <g transform="translate(60, 50)">
                  <rect width="320" height="180" rx="10" fill="#1e293b" stroke="#38bdf8" strokeWidth="1.5" />
                  <text x="160" y="26" fill="#38bdf8" fontSize="12" fontWeight="bold" textAnchor="middle">Arithmetic Operations</text>
                  <rect x="15" y="42" width="290" height="120" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="25" y="65" fill="#e2e8f0" fontSize="10">• Addition / Subtraction: a + b, a - b</text>
                  <text x="25" y="85" fill="#e2e8f0" fontSize="10">• Multiplication / Division: a * b, a / b</text>
                  <text x="25" y="105" fill="#e2e8f0" fontSize="10">• Exponentiation: a ** b (Right-associative!)</text>
                  <text x="25" y="125" fill="#e2e8f0" fontSize="10">• Remainder: a % b (Retains sign of dividend)</text>
                  <text x="25" y="145" fill="#38bdf8" fontSize="9">Division by 0 yields Infinity or -Infinity (not crash)</text>
                </g>

                <g transform="translate(440, 50)">
                  <rect width="340" height="180" rx="10" fill="#1e293b" stroke="#f59e0b" strokeWidth="1.5" />
                  <text x="170" y="26" fill="#fbbf24" fontSize="12" fontWeight="bold" textAnchor="middle">⚠️ Truncated Remainder (%) vs Euclidean Modulo</text>
                  <rect x="15" y="42" width="310" height="120" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="25" y="68" fill="#fda4af" fontSize="11" fontFamily="monospace">-7 % 3 === -1 (Remainder in JS)</text>
                  <text x="25" y="92" fill="#a7f3d0" fontSize="11" fontFamily="monospace">((n % d) + d) % d === 2 (True Modulo)</text>
                  <text x="25" y="116" fill="#fde68a" fontSize="10">2 ** 3 ** 2 === 512 (Evaluates 3^2 first = 9, then 2^9)</text>
                  <text x="25" y="140" fill="#34d399" fontSize="10" fontWeight="bold">Always wrap negative circular indices with modulo helper</text>
                </g>
    
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 0.1: Runtime execution lifecycle, memory flow, and operator evaluation rules for Arithmetic Operators.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="ArithmeticOperatorsDemo.js"
            />
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  The Right-Associativity of Exponentiation (**)
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              Unlike +, -, *, /, which are left-associative, the ** operator is right-associative: 2 ** 3 ** 2 is evaluated as 2 ** (3 ** 2) = 512, NOT (2 ** 3) ** 2 = 64.
            </p>

            <div className="rounded-xl border border-amber-900/50 bg-slate-950 p-4 font-mono text-xs text-amber-200 overflow-x-auto">
              <pre>{`// 💎 SENIOR SECRET:
console.log(2 ** 3 ** 2);   // 512
console.log((2 ** 3) ** 2); // 64`}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Arithmetic Operators"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Arithmetic Operators"}
            downloadFileName="001_003_operators-and-expressions-topic0-note.txt"
          />
        </section>
