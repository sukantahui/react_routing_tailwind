...
import demoCode from "./topic6_files/BasicInputOutputConsoleAlertPromptDemo.js?raw";

/**
 * Topic6 – Basic Input & Output: console, alert & prompt in JavaScript
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 001_003_operators-and-expressions · Topic 6</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Basic Input & Output: console, alert & prompt in JavaScript"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Master browser synchronous modal dialogs (alert, prompt, confirm), console methods (table, time, trace, group), and non-blocking alternatives.
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                During a lab session, student <strong>Debangshu</strong> encountered a subtle logical glitch. Debangshu used prompt() inside a loop for a web test. The synchronous prompt froze all background WebSocket pings and CSS animations! Sukanta Hui explained how modal dialogs freeze the browser UI thread and introduced modern HTML dialogs.
              </p>
              <p>
                Mentor <strong>Sukanta Hui</strong> demonstrated the exact step-by-step operand evaluation on the whiteboard, explaining why mastering <em>Input & Output APIs</em> eliminates ambiguity and prevents production regressions.
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            <h2 className="text-lg sm:text-xl font-bold text-sky-400 mb-4 flex items-center gap-2">
              <span>📊</span> Browser Synchronous Modal Freeze vs Non-Blocking Console Pipeline
            </h2>
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 840 280" className="w-full h-auto" role="img" aria-label="Basic Input & Output: console, alert & prompt in JavaScript Architecture">
                ...
                <text x="420" y="28" fill="#f8fafc" fontSize="15" fontWeight="bold" textAnchor="middle">
                  Browser Synchronous Modal Freeze vs Non-Blocking Console Pipeline
                </text>
                
                <g transform="translate(60, 50)">
                  <rect width="330" height="180" rx="10" fill="#1e293b" stroke="#f43f5e" strokeWidth="1.5" />
                  <text x="165" y="26" fill="#f43f5e" fontSize="12" fontWeight="bold" textAnchor="middle">⚠️ Modal Dialogs: alert(), prompt(), confirm()</text>
                  <rect x="15" y="42" width="300" height="120" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="25" y="65" fill="#fda4af" fontSize="10">• Synchronous &amp; UI-blocking</text>
                  <text x="25" y="85" fill="#fda4af" fontSize="10">• Freezes animations, timers &amp; fetch requests</text>
                  <text x="25" y="105" fill="#fda4af" fontSize="10">• Returns string from prompt() or null on cancel</text>
                  <text x="25" y="125" fill="#fda4af" fontSize="10">• Disallowed in cross-origin iframes</text>
                </g>

                <g transform="translate(440, 50)">
                  <rect width="340" height="180" rx="10" fill="#1e293b" stroke="#10b981" strokeWidth="1.5" />
                  <text x="170" y="26" fill="#34d399" fontSize="12" fontWeight="bold" textAnchor="middle">✓ Developer Console APIs</text>
                  <rect x="15" y="42" width="310" height="120" rx="6" fill="#0f172a" stroke="#334155" />
                  <text x="25" y="65" fill="#a7f3d0" fontSize="10" fontFamily="monospace">console.table(arrayOfObjects)</text>
                  <text x="25" y="85" fill="#a7f3d0" fontSize="10" fontFamily="monospace">console.time("label") / timeEnd("label")</text>
                  <text x="25" y="105" fill="#a7f3d0" fontSize="10" fontFamily="monospace">console.trace("CallStack")</text>
                  <text x="25" y="125" fill="#a7f3d0" fontSize="10" fontFamily="monospace">console.groupCollapsed("Group")</text>
                </g>
    
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure 6.1: Runtime execution lifecycle, memory flow, and operator evaluation rules for Input &amp; Output APIs.
            </p>
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  Performance Profiling with console.time() and console.timeEnd()
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              Easily benchmark loop durations and asynchronous network calls with high-precision millisecond output using console.time() and console.timeEnd().
            </p>
...
console.timeEnd('LoopBenchmark'); // LoopBenchmark: 1.42ms`}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Input & Output APIs"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Input & Output APIs"}
            downloadFileName="001_003_operators-and-expressions-topic6-note.txt"
          />
        </section>
