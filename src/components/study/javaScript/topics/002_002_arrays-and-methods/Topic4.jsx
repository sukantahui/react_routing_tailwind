...
import demoCode from "./topic4_files/NonMutatingArrayCombinationsConcatJoinSpreadDemoDemo.js?raw";
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 002_002_arrays-and-methods · Topic 4</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Non-Mutating Array Combinations — concat(), join(), Spread Operator"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            {"Combine, concatenate, and serialize arrays without mutation: concat vs spread operator performance, string joining, and custom delimiters."}
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <p className="text-slate-300 leading-relaxed mb-4">
              {"Combine, concatenate, and serialize arrays without mutation: concat vs spread operator performance, string joining, and custom delimiters."} In high-scale frontend and backend applications, understanding array memory mechanics and transformation algorithms is essential for building efficient data pipelines.
            </p>
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                {"Abhronila used `concat()` with nested arrays expecting a deep merge, but only top-level references were merged."}
              </p>
              <p>
                {"Sukanta Hui clarified that both `concat` and spread `[...]` perform shallow copies of object references."}
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            ...
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 800 240" className="w-full h-auto" role="img" aria-label={"Non-Mutating Array Combinations — concat(), join(), Spread Operator"}>
                <defs>
                ...
                <text x="400" y="30" fill="#f8fafc" fontSize="14" fontWeight="bold" textAnchor="middle">{"Non-Mutating Array Combinations — concat(), join(), Spread Operator"} · Pipeline Architecture</text>
                ...
                <text x="140" y="135" fill="#fef3c7" fontSize="11" textAnchor="middle">{"Source Array A & Source Array B"}</text>
                ...
                <text x="400" y="135" fill="#e0f2fe" fontSize="11" textAnchor="middle">{"Spread [...] / concat() Shallow Allocation"}</text>
                ...
                <text x="660" y="135" fill="#d1fae5" fontSize="11" textAnchor="middle">{"New Combined Array Instance"}</text>
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure: Step-by-step architectural execution for {"Non-Mutating Array Combinations — concat(), join(), Spread Operator"}.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">
              The ECMAScript specification defines strict abstract operations for {"Non-Mutating Array Combinations — concat(), join(), Spread Operator"}. The table below compares spec behavior, engine mechanics, and senior best practices.
            </p>
              ...
              <table className="w-full text-left text-sm text-slate-300 border-collapse border border-slate-800">
                ...
                <tbody className="divide-y divide-slate-800 font-mono text-xs">
                  ...
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"Spread Operator [...a, ...b]"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Iterable unpacking into array literal"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Modern syntax optimized directly in V8"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Standard for modern array merging"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"concat(...items)"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Combines arrays/values into a new array"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Allocates new array; handles non-array values gracefully"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Method chaining pipelines"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"join(separator)"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Serializes all elements to string with separator"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Coerces each element via ToString abstract operation"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"CSV generation, URL paths, UI text formatting"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="mt-6 p-5 rounded-xl bg-slate-900/80 border border-rose-900/40 space-y-3">
              ...
              <ul className="list-disc list-inside space-y-2 text-sm text-slate-300 leading-relaxed">
                <li><strong className="text-rose-300">{"Null & Undefined in join():"}</strong> {"`join()` coerces `null` and `undefined` to empty strings `\"\"` rather than `\"null\"`."}</li>
                <li><strong className="text-rose-300">{"Symbol.isConcatSpreadable Override:"}</strong> {"Setting `obj[Symbol.isConcatSpreadable] = true` forces `concat()` to flatten custom objects as if they were arrays."}</li>
                <li><strong className="text-rose-300">{"Spread on Non-Iterables:"}</strong> {"Attempting to spread a non-iterable primitive (`...null` in array literal) throws TypeError."}</li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="NonMutatingArrayCombinationsConcatJoinSpreadDemoDemo.js"
            />
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-rose-950/20 border border-rose-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-rose-400 font-bold">
                <span>❌</span>
                <span>Anti-Pattern: {"Spreading Massive Arrays in Tight Loops"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                {"Repeatedly spreading arrays `arr = [...arr, item]` creates O(N^2) garbage collection churn."}
              </p>
              <pre className="p-4 rounded-xl bg-slate-950 border border-rose-900/50 text-xs font-mono text-rose-300 overflow-x-auto">
{"// ❌ AVOID: O(N^2) memory reallocation\nlet list = [];\nfor (let i = 0; i < 50000; i++) list = [...list, i];"}
              </pre>
            </div>
            ...
            <div className="bg-emerald-950/20 border border-emerald-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-emerald-400 font-bold">
                <span>✓</span>
                <span>Senior Pro Practice: {"Use push() during construction, freeze or return copy at end"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                {"Build efficiently, then freeze for immutability."}
              </p>
...
{"// ✓ RECOMMENDED: Linear O(N)\nconst list = [];\nfor (let i = 0; i < 50000; i++) list.push(i);"}
              </pre>
            </div>
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  {"Symbol.isConcatSpreadable for Custom Class Interoperability"}
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              {"Control whether custom class instances are flattened or treated as discrete objects when passed to `Array.prototype.concat()`."}
            </p>

            <div className="rounded-xl border border-amber-900/50 bg-slate-950 p-4 font-mono text-xs text-amber-200 overflow-x-auto">
              <pre>{"class CustomCollection {\n  constructor(...items) {\n    this.items = items;\n    this[Symbol.isConcatSpreadable] = true;\n  }\n  get length() { return this.items.length; }\n  get 0() { return this.items[0]; }\n  get 1() { return this.items[1]; }\n}\nconst col = new CustomCollection(\"A\", \"B\");\nconsole.log([1, 2].concat(col)); // [1, 2, \"A\", \"B\"]"}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-indigo-950/20 border border-indigo-800/40 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed mb-4">
              {"What is the key functional difference between `[...arr]` and `arr.slice()` when `arr` has a custom Symbol.iterator?"}
            </p>
            <div className="p-4 rounded-xl bg-slate-900 border border-indigo-900/50 text-xs sm:text-sm text-indigo-300 font-mono">
              💡 Hint: {"Spread operator `[...]` strictly consumes `Symbol.iterator`, while `slice()` accesses internal array elements by index."}
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Non-Mutating Array Combinations — concat(), join(), Spread Operator"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Non-Mutating Array Combinations — concat(), join(), Spread Operator"}
            downloadFileName="002-002-topic4-note.txt"
          />
        </section>
