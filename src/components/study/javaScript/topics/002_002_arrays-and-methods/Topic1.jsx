...
import demoCode from "./topic1_files/DenseVsSparseArraysV8MemoryAllocationDemoDemo.js?raw";
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 002_002_arrays-and-methods · Topic 1</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Dense vs Sparse Arrays & Memory Allocation in the V8 Engine"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            {"Deconstruct V8 array storage internal architectures: Fast Packed Elements vs Slow Dictionary Mode, empty slots vs undefined, and memory profiling."}
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                {"Debangshu benchmarked a large loop over an array with deleted indices and found performance dropped by 80x compared to a continuous array."}
              </p>
              <p>
                {"Sukanta Hui analyzed V8 bytecode, showing how deleting indices transitions an array from `PACKED_ELEMENTS` (C++ vector) to `DICTIONARY_ELEMENTS` (hash table lookup)."}
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            ...
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 800 240" className="w-full h-auto" role="img" aria-label={"Dense vs Sparse Arrays & Memory Allocation in the V8 Engine"}>
                <defs>
                ...
                <text x="400" y="30" fill="#f8fafc" fontSize="14" fontWeight="bold" textAnchor="middle">&#123;"Dense vs Sparse Arrays & Memory Allocation in the V8 Engine"&#125; · Pipeline Architecture</text>
                ...
                <text x="140" y="135" fill="#fef3c7" fontSize="11" textAnchor="middle">&#123;"PACKED_SMI / PACKED_ELEMENTS (Contiguous)"&#125;</text>
                ...
                <text x="400" y="135" fill="#e0f2fe" fontSize="11" textAnchor="middle">&#123;"Hole Creation (delete / skipped index)"&#125;</text>
                ...
                <text x="660" y="135" fill="#d1fae5" fontSize="11" textAnchor="middle">&#123;"DICTIONARY_ELEMENTS (Slow Hash Table)"&#125;</text>
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure: Step-by-step architectural execution for {"Dense vs Sparse Arrays & Memory Allocation in the V8 Engine"}.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">
              The ECMAScript specification defines strict abstract operations for {"Dense vs Sparse Arrays & Memory Allocation in the V8 Engine"}. The table below compares spec behavior, engine mechanics, and senior best practices.
            </p>
              ...
              <table className="w-full text-left text-sm text-slate-300 border-collapse border border-slate-800">
                ...
                <tbody className="divide-y divide-slate-800 font-mono text-xs">
                  ...
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"PACKED_SMI_ELEMENTS"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Array containing only small integers"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Direct flat C++ array with unboxed 31-bit integers"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Ultra-fast numeric computations"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"PACKED_DOUBLE_ELEMENTS"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Array containing floating-point numbers"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Flat C++ double array"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Scientific math & graphics coordinates"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"HOLEY_ELEMENTS"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Array containing unassigned gaps/holes"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Requires prototype chain lookups for every index access"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Avoid in performance-critical code"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"DICTIONARY_ELEMENTS"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Sparse array with large gaps"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Converted to hash map with slow string key hashing"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Use JavaScript Map instead for sparse keys"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="mt-6 p-5 rounded-xl bg-slate-900/80 border border-rose-900/40 space-y-3">
              ...
              <ul className="list-disc list-inside space-y-2 text-sm text-slate-300 leading-relaxed">
                <li><strong className="text-rose-300">{"De-Optimization Irreversibility:"}</strong> {"Once an array transitions to HOLEY or DICTIONARY mode in V8, it can never transition back to PACKED mode."}</li>
                <li><strong className="text-rose-300">{"delete arr[i] Penalty:"}</strong> {"Using `delete` on an array element creates an empty hole and causes immediate de-optimization."}</li>
                <li><strong className="text-rose-300">{"in Operator on Holes vs Undefined:"}</strong> {"`i in sparseArr` returns `false` for holes, but `true` if `arr[i] = undefined`."}</li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="DenseVsSparseArraysV8MemoryAllocationDemoDemo.js"
            />
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-rose-950/20 border border-rose-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-rose-400 font-bold">
                <span>❌</span>
                <span>Anti-Pattern: {"Using delete Operator on Array Elements"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                {"Deleting array indices leaves empty holes and destroys V8 inline caches."}
              </p>
              <pre className="p-4 rounded-xl bg-slate-950 border border-rose-900/50 text-xs font-mono text-rose-300 overflow-x-auto">
{"// ❌ AVOID: Creates hole & de-optimizes\nconst items = [1, 2, 3, 4];\ndelete items[1]; // [1, <empty>, 3, 4]"}
              </pre>
            </div>
            ...
            <div className="bg-emerald-950/20 border border-emerald-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-emerald-400 font-bold">
                <span>✓</span>
                <span>Senior Pro Practice: {"Use splice() or filter() for Element Removal"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                {"Always preserve contiguous memory structures."}
              </p>
...
{"// ✓ RECOMMENDED: Contiguous packed array\nconst items = [1, 2, 3, 4];\nitems.splice(1, 1); // [1, 3, 4] (Fast PACKED array)"}
              </pre>
            </div>
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  {"Detecting Holes vs Explicit Undefined via Object.hasOwn()"}
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              {"Use `Object.hasOwn(arr, index)` to definitively test if an index has an allocated property or an empty V8 hole."}
            </p>

            <div className="rounded-xl border border-amber-900/50 bg-slate-950 p-4 font-mono text-xs text-amber-200 overflow-x-auto">
              <pre>{"const holey = [1, , 3];\nconst explicit = [1, undefined, 3];\nconsole.log(Object.hasOwn(holey, 1)); // false (Hole)\nconsole.log(Object.hasOwn(explicit, 1)); // true (Defined property)"}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-indigo-950/20 border border-indigo-800/40 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed mb-4">
              {"Why is iterating over a holey array with a `for` loop significantly slower than iterating over a packed array?"}
            </p>
            <div className="p-4 rounded-xl bg-slate-900 border border-indigo-900/50 text-xs sm:text-sm text-indigo-300 font-mono">
              💡 Hint: {"When the engine hits a hole, it must pause and perform a prototype lookup on `Array.prototype` and `Object.prototype` to check for inherited index properties."}
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Dense vs Sparse Arrays & Memory Allocation in the V8 Engine"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Dense vs Sparse Arrays & Memory Allocation in the V8 Engine"}
            downloadFileName="002-002-topic1-note.txt"
          />
        </section>
