...
import demoCode from "./topic12_files/ArrayBufferTypedArraysDataViewDemoDemo.js?raw";
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 002_002_arrays-and-methods · Topic 12</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"ArrayBuffer, TypedArrays (Uint8Array, Float64Array) & DataView Architecture"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            {"Work with raw binary data in JavaScript: ArrayBuffer memory allocation, TypedArray views (Uint8Array, Float64Array), DataView endianness control, and WebGL/binary protocol handling."}
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <p className="text-slate-300 leading-relaxed mb-4">
              {"Work with raw binary data in JavaScript: ArrayBuffer memory allocation, TypedArray views (Uint8Array, Float64Array), DataView endianness control, and WebGL/binary protocol handling."} In high-scale frontend and backend applications, understanding array memory mechanics and transformation algorithms is essential for building efficient data pipelines.
            </p>
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                {"Debangshu was parsing a binary WebSocket protocol and experienced data corruption because different CPU architectures read multi-byte integers in little-endian vs big-endian."}
              </p>
              <p>
                {"Sukanta Hui demonstrated using `DataView` with explicit `littleEndian` flags to decode network byte packets deterministically across all client machines."}
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            ...
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 800 240" className="w-full h-auto" role="img" aria-label={"ArrayBuffer, TypedArrays (Uint8Array, Float64Array) & DataView Architecture"}>
                <defs>
                ...
                <text x="400" y="30" fill="#f8fafc" fontSize="14" fontWeight="bold" textAnchor="middle">&#123;"ArrayBuffer, TypedArrays (Uint8Array, Float64Array) & DataView Architecture"&#125; · Pipeline Architecture</text>
                ...
                <text x="140" y="135" fill="#fef3c7" fontSize="11" textAnchor="middle">&#123;"Raw Memory: ArrayBuffer (Bytes)"&#125;</text>
                ...
                <text x="400" y="135" fill="#e0f2fe" fontSize="11" textAnchor="middle">&#123;"TypedArray View (Uint8Array, Int32Array, Float64Array)"&#125;</text>
                ...
                <text x="660" y="135" fill="#d1fae5" fontSize="11" textAnchor="middle">&#123;"DataView (Explicit Endianness & Mixed Types)"&#125;</text>
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure: Step-by-step architectural execution for {"ArrayBuffer, TypedArrays (Uint8Array, Float64Array) & DataView Architecture"}.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">
              The ECMAScript specification defines strict abstract operations for {"ArrayBuffer, TypedArrays (Uint8Array, Float64Array) & DataView Architecture"}. The table below compares spec behavior, engine mechanics, and senior best practices.
            </p>
              ...
              <table className="w-full text-left text-sm text-slate-300 border-collapse border border-slate-800">
                ...
                <tbody className="divide-y divide-slate-800 font-mono text-xs">
                  ...
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"ArrayBuffer(byteLength)"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Fixed-length raw binary data buffer in memory"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Allocates contiguous heap memory outside V8 pointer space"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Binary file storage, network streams, audio"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"TypedArray Views"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Uint8Array, Int32Array, Float64Array views over buffer"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Fixed-type, zero-overhead memory access"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"WebGL graphics, WebAssembly memory, audio DSP"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"DataView"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Low-level heterogeneous binary reader/writer"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Supports mixed types & explicit Little/Big Endian control"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Binary file format parsers (PNG, MP3, TCP packets)"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="mt-6 p-5 rounded-xl bg-slate-900/80 border border-rose-900/40 space-y-3">
              ...
              <ul className="list-disc list-inside space-y-2 text-sm text-slate-300 leading-relaxed">
                <li><strong className="text-rose-300">{"RangeError: Invalid typed array length:"}</strong> {"Allocating buffer sizes exceeding available system memory."}</li>
                <li><strong className="text-rose-300">{"Byte Alignment Offset Errors:"}</strong> {"Creating a `Uint32Array` on an unaligned byte offset (must be a multiple of 4 bytes)."}</li>
                <li><strong className="text-rose-300">{"Non-Resizable Fixed Buffers:"}</strong> {"Standard ArrayBuffers cannot be resized; use `SharedArrayBuffer` or copy to a larger buffer."}</li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="ArrayBufferTypedArraysDataViewDemoDemo.js"
            />
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-rose-950/20 border border-rose-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-rose-400 font-bold">
                <span>❌</span>
                <span>Anti-Pattern: {"Assuming Host Machine Endianness Matches Network Byte Order"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                {"Network protocols use Big-Endian while x86/ARM CPUs use Little-Endian."}
              </p>
              <pre className="p-4 rounded-xl bg-slate-950 border border-rose-900/50 text-xs font-mono text-rose-300 overflow-x-auto">
{"// ❌ AVOID: Direct typed array assumes host endianness\nconst id = new Uint32Array(buffer)[0];"}
              </pre>
            </div>
            ...
            <div className="bg-emerald-950/20 border border-emerald-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-emerald-400 font-bold">
                <span>✓</span>
                <span>Senior Pro Practice: {"Use DataView with Explicit Endianness Flag"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                {"Always pass explicit boolean flag for endianness."}
              </p>
...
{"// ✓ RECOMMENDED: Guaranteed cross-platform decoding\nconst view = new DataView(buffer);\nconst id = view.getUint32(0, false); // false = Big-Endian (Network Order)"}
              </pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-indigo-950/20 border border-indigo-800/40 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed mb-4">
              {"What is the memory size difference between storing 1,000,000 numbers in a standard JS array vs a `Float64Array`?"}
            </p>
            <div className="p-4 rounded-xl bg-slate-900 border border-indigo-900/50 text-xs sm:text-sm text-indigo-300 font-mono">
              💡 Hint: {"Standard JS arrays store boxed pointers and object headers (~8-16 bytes per number + hole bitmasks), while `Float64Array` uses exactly 8MB of flat contiguous memory."}
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "ArrayBuffer, TypedArrays (Uint8Array, Float64Array) & DataView Architecture"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "ArrayBuffer, TypedArrays (Uint8Array, Float64Array) & DataView Architecture"}
            downloadFileName="002-002-topic12-note.txt"
          />
        </section>
