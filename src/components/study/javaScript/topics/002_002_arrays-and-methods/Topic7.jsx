...
import demoCode from "./topic7_files/TransformationPipelinesMapFlatMapFlatDemoDemo.js?raw";
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 002_002_arrays-and-methods · Topic 7</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Transformation Pipelines — map(), flatMap(), flat() with Depth Control"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            {"Transform data structures using map, flat, and flatMap: recursive array flattening with depth controls, 1-to-N transformations, and building immutable pipelines."}
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <p className="text-slate-300 leading-relaxed mb-4">
              {"Transform data structures using map, flat, and flatMap: recursive array flattening with depth controls, 1-to-N transformations, and building immutable pipelines."} In high-scale frontend and backend applications, understanding array memory mechanics and transformation algorithms is essential for building efficient data pipelines.
            </p>
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                {"Debangshu mapped an array of orders to an array of item lists and ended up with a nested `[[Item], [Item]]` array, then struggled to filter individual items."}
              </p>
              <p>
                {"Sukanta Hui introduced `flatMap()`, which maps each element and flattens the resulting array by depth 1 in a single optimized pass."}
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            ...
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 800 240" className="w-full h-auto" role="img" aria-label={"Transformation Pipelines — map(), flatMap(), flat() with Depth Control"}>
                <defs>
            ...
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure: Step-by-step architectural execution for {"Transformation Pipelines — map(), flatMap(), flat() with Depth Control"}.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">
              The ECMAScript specification defines strict abstract operations for {"Transformation Pipelines — map(), flatMap(), flat() with Depth Control"}. The table below compares spec behavior, engine mechanics, and senior best practices.
            </p>
              ...
              <table className="w-full text-left text-sm text-slate-300 border-collapse border border-slate-800">
                ...
                <tbody className="divide-y divide-slate-800 font-mono text-xs">
                  ...
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"map(fn)"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Transforms each element 1-to-1 into new array"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Allocates array of exact same length as source"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Data reshaping & projection"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"flat(depth = 1)"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Flattens sub-arrays recursively up to specified depth"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"Concatenates nested elements into single array"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Normalizing multi-level nested data"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"flatMap(fn)"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Maps each element then flattens result by 1 level"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"More efficient than .map().flat() in memory"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"1-to-many data expansions & filtering"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="mt-6 p-5 rounded-xl bg-slate-900/80 border border-rose-900/40 space-y-3">
              ...
              <ul className="list-disc list-inside space-y-2 text-sm text-slate-300 leading-relaxed">
                <li><strong className="text-rose-300">{"flat() Cleans Sparse Holes:"}</strong> {"`flat()` automatically removes empty holes from sparse arrays."}</li>
                <li><strong className="text-rose-300">{"flatMap() Depth Limit:"}</strong> {"`flatMap()` only flattens by exactly 1 level; deeper nesting requires `.flat(Infinity)`."}</li>
                <li><strong className="text-rose-300">{"map() Mutating In-Place:"}</strong> {"Mutating object properties inside `map` callbacks breaks immutability."}</li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="TransformationPipelinesMapFlatMapFlatDemoDemo.js"
            />
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-rose-950/20 border border-rose-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-rose-400 font-bold">
                <span>❌</span>
                <span>Anti-Pattern: {"Chaining .map().flat() Instead of flatMap()"}</span>
              </div>
...
{"// ❌ INEFFECTIVE: Allocates two arrays\nconst tags = posts.map(p => p.tags).flat();"}
            ...
            <div className="bg-emerald-950/20 border border-emerald-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-emerald-400 font-bold">
                <span>✓</span>
                <span>Senior Pro Practice: {"Use Single-Pass flatMap()"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
...
{"// ✓ RECOMMENDED: Single-pass allocation\nconst tags = posts.flatMap(p => p.tags);"}
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  {"Simultaneous Map and Filter via flatMap()"}
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              {"By returning an empty array `[]` to drop items or `[transformed]` to keep items, `flatMap()` can map and filter in a single pass."}
            </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-indigo-950/20 border border-indigo-800/40 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed mb-4">
              {"What is the time complexity difference between `arr.flat(Infinity)` on a deeply nested tree vs iterative BFS queue flattening?"}
            </p>
            <div className="p-4 rounded-xl bg-slate-900 border border-indigo-900/50 text-xs sm:text-sm text-indigo-300 font-mono">
              💡 Hint: {"Both are O(N) where N is total nodes, but iterative BFS avoids recursion stack memory limits."}
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Transformation Pipelines — map(), flatMap(), flat() with Depth Control"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Transformation Pipelines — map(), flatMap(), flat() with Depth Control"}
            downloadFileName="002-002-topic7-note.txt"
          />
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <Teacher
            note={"In my 27+ years of mentoring software engineers at Coder & AccoTax in Barrackpore, I have consistently seen that mastering " + "Transformation Pipelines — map(), flatMap(), flat() with Depth Control" + " separates code monkeys from genuine software engineers. " + "`flatMap` is one of JavaScript's most powerful secret weapons: it allows you to filter, map, and expand elements simultaneously in one clean pass."}
          />
        </section>
