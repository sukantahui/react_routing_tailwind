...
import demoCode from "./topic2_files/MutatingStackQueuePushPopShiftUnshiftDemoDemo.js?raw";
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-amber-500/30 selection:text-amber-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-950/70 border border-amber-700/60 text-amber-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>Module 002_002_arrays-and-methods · Topic 2</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-sky-300 tracking-tight leading-tight">
            {"Mutating Stack & Queue Methods — push(), pop(), shift(), unshift()"}
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            {"Master in-place array mutation: Stack (LIFO) and Queue (FIFO) mechanics, time complexity analysis (O(1) vs O(N)), and high-throughput buffer architectures."}
          </p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg hover:border-slate-700 transition-all">
            ...
            <p className="text-slate-300 leading-relaxed mb-4">
              {"Master in-place array mutation: Stack (LIFO) and Queue (FIFO) mechanics, time complexity analysis (O(1) vs O(N)), and high-throughput buffer architectures."} In high-scale frontend and backend applications, understanding array memory mechanics and transformation algorithms is essential for building efficient data pipelines.
            </p>
            ...
            <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-900/40 text-sm text-slate-300 leading-relaxed space-y-2">
              ...
              <p>
                {"Tuhina built a queue processor for 500,000 incoming logs using `shift()` and wondered why processing each batch took several seconds."}
              </p>
              <p>
                {"Sukanta Hui explained that `shift()` and `unshift()` have O(N) linear complexity because every subsequent element must be re-indexed in memory, and showed how ring buffers achieve O(1) performance."}
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl">
            ...
            <div className="w-full overflow-x-auto">
              <svg viewBox="0 0 800 240" className="w-full h-auto" role="img" aria-label={"Mutating Stack & Queue Methods — push(), pop(), shift(), unshift()"}>
                <defs>
                ...
                <text x="400" y="30" fill="#f8fafc" fontSize="14" fontWeight="bold" textAnchor="middle">{"Mutating Stack & Queue Methods — push(), pop(), shift(), unshift()"} · Pipeline Architecture</text>
                ...
                <text x="140" y="135" fill="#fef3c7" fontSize="11" textAnchor="middle">{"Push / Pop (Tail: O(1) Amortized)"}</text>
                ...
                <text x="400" y="135" fill="#e0f2fe" fontSize="11" textAnchor="middle">{"Shift / Unshift (Head: O(N) Re-indexing)"}</text>
                ...
                <text x="660" y="135" fill="#d1fae5" fontSize="11" textAnchor="middle">{"Double-Ended Circular Buffer Pattern"}</text>
              </svg>
            </div>
            <p className="text-xs text-slate-400 mt-3 text-center">
              Figure: Step-by-step architectural execution for {"Mutating Stack & Queue Methods — push(), pop(), shift(), unshift()"}.
            </p>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-6">
          <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">
              The ECMAScript specification defines strict abstract operations for {"Mutating Stack & Queue Methods — push(), pop(), shift(), unshift()"}. The table below compares spec behavior, engine mechanics, and senior best practices.
            </p>
              ...
              <table className="w-full text-left text-sm text-slate-300 border-collapse border border-slate-800">
                ...
                <tbody className="divide-y divide-slate-800 font-mono text-xs">
                  ...
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"push(...items)"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Appends items to end; returns new length"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"O(1) amortized memory allocation at tail"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Stack push, appending data streams"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"pop()"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Removes & returns last item; returns undefined if empty"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"O(1) decrement length & return pointer"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Stack pop, undo operations"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"unshift(...items)"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Prepends items to start; returns new length"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"O(N) shifts all existing elements right"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Small arrays only (< 1000 items)"}</td>
                  </tr>
                  <tr className="hover:bg-slate-800/30">
                    <td className="p-3 text-sky-400">{"shift()"}</td>
                    <td className="p-3 text-slate-300 font-sans">{"Removes & returns first item; returns undefined if empty"}</td>
                    <td className="p-3 text-amber-300 font-sans">{"O(N) shifts all remaining elements left"}</td>
                    <td className="p-3 text-emerald-400 font-sans">{"Small arrays only; use pointer queue for large queues"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="mt-6 p-5 rounded-xl bg-slate-900/80 border border-rose-900/40 space-y-3">
              ...
              <ul className="list-disc list-inside space-y-2 text-sm text-slate-300 leading-relaxed">
                <li><strong className="text-rose-300">{"Maximum Call Stack / Argument Limit with Spread:"}</strong> {"`arr.push(...largeArray)` throws `RangeError: Maximum call stack size exceeded` if `largeArray` has > 65,536 elements."}</li>
                <li><strong className="text-rose-300">{"Accidental Mutation of State:"}</strong> {"Calling `push` or `pop` directly on component state breaks immutability."}</li>
                <li><strong className="text-rose-300">{"Sparse Hole Re-Indexing:"}</strong> {"`shift()` shifts empty holes, preserving holey de-optimizations."}</li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 overflow-hidden shadow-2xl bg-slate-900">
            <JavaScriptEditableCodeBlock
              ...
              title="MutatingStackQueuePushPopShiftUnshiftDemoDemo.js"
            />
          </div>
        </section>
...
{"// ❌ AVOID: O(N^2) overall complexity\nwhile (queue.length) {\n  const task = queue.shift(); // O(N) on every loop iteration!\n}"}
            ...
            <div className="bg-emerald-950/20 border border-emerald-800/40 rounded-2xl p-6 shadow-lg space-y-3">
              <div className="flex items-center gap-2 text-emerald-400 font-bold">
                <span>✓</span>
                <span>Senior Pro Practice: {"Use Pointer Head Index or Ring Buffer"}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                {"Maintain a head pointer index to consume items in O(1) time."}
              </p>
...
{"// ✓ RECOMMENDED: O(1) Pointer-Based Queue\nlet head = 0;\nwhile (head < queue.length) {\n  const task = queue[head++];\n  processTask(task);\n}"}
              </pre>
            </div>
          </div>
        </section>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">
                  {"Batch Pushing Without Stack Overflow via Chunking"}
                </h3>
              </div>
            </div>

            <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-6">
              {"Safely append massive arrays to an existing array without triggering argument count limits."}
            </p>

            <div className="rounded-xl border border-amber-900/50 bg-slate-950 p-4 font-mono text-xs text-amber-200 overflow-x-auto">
              <pre>{"function batchPush(target, source, chunkSize = 10000) {\n  for (let i = 0; i < source.length; i += chunkSize) {\n    target.push(...source.slice(i, i + chunkSize));\n  }\n  return target.length;\n}\nconst bigArr = [];\nbatchPush(bigArr, Array.from({ length: 150000 }, (_, i) => i));\nconsole.log(\"Big Array Length:\", bigArr.length); // 150000"}</pre>
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <div className="bg-indigo-950/20 border border-indigo-800/40 rounded-2xl p-6 md:p-8 shadow-lg">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed mb-4">
              {"Why does `push()` return the new array length while `pop()` returns the removed element?"}
            </p>
            <div className="p-4 rounded-xl bg-slate-900 border border-indigo-900/50 text-xs sm:text-sm text-indigo-300 font-mono">
              💡 Hint: {"Consistency with traditional stack ADT operations and providing immediate access to the mutated array's bounds."}
            </div>
          </div>
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <FAQTemplate
            title={"Frequently Asked Questions · " + "Mutating Stack & Queue Methods — push(), pop(), shift(), unshift()"}
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint
            content={noteText}
            title={"JavaScript Master Note · " + "Mutating Stack & Queue Methods — push(), pop(), shift(), unshift()"}
            downloadFileName="002-002-topic2-note.txt"
          />
        </section>
