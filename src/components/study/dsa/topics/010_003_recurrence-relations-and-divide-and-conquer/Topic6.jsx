...
import demoCode from "./topic6_files/ExampleDiscussionNaiveRecursivDemo.c?raw";
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-cyan-500/30 selection:text-cyan-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-cyan-950/70 border border-cyan-700/60 text-cyan-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>DSA Segment 10 · Topic 6</span>
          </div>
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-sky-300 to-indigo-300 tracking-tight leading-tight">
            Example &amp; Discussion: Naive Recursive Fibonacci Explosion (T(n) = T(n-1) + T(n-2) = O(2^n)) vs Strassen's Matrix Multiplication (O(n^2.81))
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Recurrence Relations &amp; Divide-and-Conquer Complexity: Master Theorem &amp; Recursion Trees: Deep technical architectural breakdown, memory layout mechanics, pointer invariants, and industrial C implementation at Coder &amp; AccoTax Barrackpore Lab.
          </p>

          <div className="flex flex-wrap items-center justify-center gap-4 text-xs font-mono text-slate-400 pt-2">
            <span className="px-2.5 py-1 rounded-md bg-slate-900 border border-slate-800 text-cyan-400">Course Code: DSA-C-1006</span>
            ...
            <div className="space-y-6 text-slate-300 leading-relaxed text-sm sm:text-base">
              ...
              <div className="bg-slate-950/70 border border-slate-800 rounded-xl p-5 space-y-3">
                <h3 className="text-cyan-400 font-bold flex items-center gap-2 text-base">
                  <span>💡</span> The Physical Intuition Behind Example &amp; Discussion: Naive Recursive Fibonacci Explosion (T(n) = T(n-1) + T(n-2) = O(2^n)) vs Strassen's Matrix Multiplication (O(n^2.81))
                </h3>
                <p>
              ...
              <div className="bg-slate-950/70 border border-slate-800 rounded-xl p-5 space-y-3">
                ...
                <div className="space-y-3 text-xs sm:text-sm font-sans border-l-2 border-cyan-500/40 pl-4 py-1">
                  <p>
                    <strong className="text-emerald-400">Swadeep:</strong> <em>"Sir, what is the most critical invariant we must protect when implementing Example &amp; Discussion: Naive Recursive Fibonacci Explosion (T(n) = T(n-1) + T(n-2) = O(2^n)) vs Strassen's Matrix Multiplication (O(n^2.81)) in C?"</em>
                  </p>
                  <p>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <EditableCCodeBlock code={demoCode} initialCode={demoCode} title="ExampleDiscussionNaiveRecursivDemo.c" />
        </section>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint content={noteText} title="DSA Topic Note: Example &amp; Discussion: Naive Recursive Fibonacci Explosion (T(n) = T(n-1) + T(n-2) = O(2^n)) vs Strassen's Matrix Multiplication (O(n^2.81))" />
        </section>
