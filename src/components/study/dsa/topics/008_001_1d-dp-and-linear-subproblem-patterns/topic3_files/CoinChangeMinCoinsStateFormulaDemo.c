 ...
 * Mentor: Sukanta Hui
 * Topic: Coin Change (Min Coins): State formulation `dp[amount] = min(dp[amount - coin] + 1)` and handling unreachable amounts
...
// Core demonstration for: Coin Change (Min Coins): State formulation `dp[amount] = min(dp[amount - coin] + 1)` and handling unreachable amounts
void executeTopicDemo(void) {
    printf("Executing: Coin Change (Min Coins): State formulation `dp[amount] = min\n");
