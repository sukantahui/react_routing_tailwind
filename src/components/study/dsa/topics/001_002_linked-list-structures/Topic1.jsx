...
import demoCode from "./topic1_files/doubly_circular_demo.c?raw";

export default function DoublyLinkedListTopic() {
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-cyan-500/30 selection:text-cyan-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-cyan-950/70 border border-cyan-700/60 text-cyan-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>DSA Module 02 · Topic 1</span>
          </div>
          ...
          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Explore bidirectional navigation with <code className="text-cyan-300 font-mono">prev</code> pointers, constant O(1) node removal, and circular head-to-tail wrapping.
          </p>

          <div className="flex flex-wrap items-center justify-center gap-4 text-xs font-mono text-slate-400 pt-2">
            <span className="px-2.5 py-1 rounded-md bg-slate-900 border border-slate-800 text-cyan-400">Course Code: DSA-C-102B</span>
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-sky-300">
                  Teacher's Desk: The Two-Way Train Track
                </h2>
            ...
            <div className="space-y-6 text-slate-300 leading-relaxed text-sm sm:text-base">
              <div className="bg-slate-950/70 border border-slate-800 rounded-xl p-5 space-y-3">
                <h3 className="text-sky-400 font-bold flex items-center gap-2 text-base">
                  <span>🚆</span> Two-Way Railway Analogy
                </h3>
                <p>
                  A Singly Linked List is like a one-way street — you can only move forward!
                </p>
                <p>
                  A <strong>Doubly Linked List</strong> provides tracks in both directions: `next` moves right, `prev` moves left. You can walk backward to inspect previous states instantly!
                </p>
              </div>
            </div>
          </div>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          ...
          <EditableCCodeBlock code={demoCode} initialCode={demoCode} title="doubly_linked_list.c" />
        </section>
        ...
        {/* PRINTABLE NOTE */}
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint content={noteText} title="DSA Topic Note: Doubly Linked Lists & Bidirectional Pointers" />
        </section>
