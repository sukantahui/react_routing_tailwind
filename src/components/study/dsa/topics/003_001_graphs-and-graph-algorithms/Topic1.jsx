...
import demoCode from "./topic1_files/dijkstra_kruskal_demo.c?raw";

export default function DijkstraTopic() {
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-cyan-500/30 selection:text-cyan-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-cyan-950/70 border border-cyan-700/60 text-cyan-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>DSA Module 07 · Topic 1</span>
          </div>
          ...
          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Master single-source shortest path relaxation, greedy minimum distance extraction, edge weight non-negativity rules, and navigation map calculations.
          </p>

          <div className="flex flex-wrap items-center justify-center gap-4 text-xs font-mono text-slate-400 pt-2">
            <span className="px-2.5 py-1 rounded-md bg-slate-900 border border-slate-800 text-cyan-400">Course Code: DSA-C-301B</span>
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-indigo-300">
                  Teacher's Desk: Google Maps GPS Router
                </h2>
            ...
            <div className="space-y-6 text-slate-300 leading-relaxed text-sm sm:text-base">
              <div className="bg-slate-950/70 border border-slate-800 rounded-xl p-5 space-y-3">
                <h3 className="text-indigo-400 font-bold flex items-center gap-2 text-base">
                  <span>🗺️</span> Barrackpore GPS Navigation
                </h3>
                <p>
                  Suppose you want to travel from Barrackpore Railway Station to Shyambazar Kolkata.
                </p>
                <p>
                  <strong>Dijkstra's Algorithm</strong> relaxes distances for every road junction (vertex) and greedily picks the unvisited road junction with the smallest accumulated travel time!
                </p>
              </div>
            </div>
          </div>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          <h2 className="text-2xl font-bold text-cyan-400 flex items-center gap-2">
            <span>🛠️</span> Runnable C Dijkstra Code
          </h2>
          <EditableCCodeBlock code={demoCode} initialCode={demoCode} title="dijkstra.c" />
        </section>
        ...
        {/* PRINTABLE NOTE */}
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint content={noteText} title="DSA Topic Note: Dijkstra Shortest Path Algorithm" />
        </section>
