...
export default function ArrayTopic2() {
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12 font-sans selection:bg-cyan-500/30 selection:text-cyan-200">
        ...
        <header ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-cyan-950/70 border border-cyan-700/60 text-cyan-300 text-xs font-semibold uppercase tracking-wider shadow-lg">
            <span>⚡</span>
            <span>DSA Module 01 · Topic 2</span>
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-sky-300 to-indigo-300 tracking-tight leading-tight">
            Dynamic Array Resizing with realloc(), malloc() &amp; free() in C
          </h1>

          <p className="text-sm sm:text-base md:text-lg text-slate-300 max-w-3xl mx-auto leading-relaxed">
            Master heap memory expansion with <code className="text-cyan-300 font-mono">realloc()</code>, amortized O(1) capacity doubling, temporary pointer safety checks, and memory leak prevention.
          </p>

          <div className="flex flex-wrap items-center justify-center gap-4 text-xs font-mono text-slate-400 pt-2">
            <span className="px-2.5 py-1 rounded-md bg-slate-900 border border-slate-800 text-cyan-400">Course Code: DSA-C-101C</span>
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-cyan-300">
                  Teacher's Desk: The Expanding Suitcase Metaphor
                </h2>
            ...
            <div className="space-y-6 text-slate-300 leading-relaxed text-sm sm:text-base">
              <div className="bg-slate-950/70 border border-slate-800 rounded-xl p-5 space-y-3">
                <h3 className="text-cyan-400 font-bold flex items-center gap-2 text-base">
                  <span>🧳</span> Expanding Suitcase Analogy
                </h3>
                <p>
                  Imagine packing a small suitcase for 2 days. Suddenly your trip is extended to 8 days! Instead of buying a new suitcase every single day, you buy a suitcase twice as big!
                </p>
                <p>
                  That is <strong>Capacity Doubling in Dynamic Vectors</strong>! It guarantees an amortized <strong>O(1)</strong> push operation time!
                </p>
              </div>
            </div>
          </div>
        ...
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12 space-y-4">
          <h2 className="text-2xl font-bold text-cyan-400 flex items-center gap-2">
            <span>🛠️</span> Runnable C Dynamic Array Code
          </h2>
          <EditableCCodeBlock code={demoCode} initialCode={demoCode} title="dynamic_realloc.c" />
        </section>
        ...
        {/* PRINTABLE NOTE */}
        <section ref={addRef} className="reveal-section max-w-5xl mx-auto mb-12">
          <PlainTextPrint content={noteText} title="DSA Topic Note: Dynamic Array Resizing & Memory Management" />
        </section>
