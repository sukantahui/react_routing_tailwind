 ...
 * Mentor: Sukanta Hui
 * Topic: Tree Construction Algorithm: Recursive post-order building `tree[idx] = tree[left] + tree[right]` in O(n)
...
// Core demonstration for: Tree Construction Algorithm: Recursive post-order building `tree[idx] = tree[left] + tree[right]` in O(n)
void executeTopicDemo(void) {
    printf("Executing: Tree Construction Algorithm: Recursive post-order building `\n");
