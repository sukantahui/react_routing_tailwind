...
const PythonQuestionPaperTemplate = ({ data, isLoggedIn = false, organizationDetails = {} }) => {
  ...
  // Format question text – replaces ```python ... ``` with PythonCodeBlock
  const formatQuestion = (text) => {
    ...
    while ((match = pythonBlockRegex.exec(text)) !== null) {
      ...
      if (match.index > lastIndex) {
        parts.push(<span key={`text-${lastIndex}`} className="whitespace-pre-wrap">
          {text.slice(lastIndex, match.index)}
        </span>);
      }
      // The code block itself
      const code = match[1].trim();
      parts.push(<PythonCodeBlock key={`code-${match.index}`} code={code} title="Python Code" />);
    ...
    return parts.length ? parts : <span className="whitespace-pre-wrap">{text}</span>;
  };

  // Format answer – same as question but also wraps plain text in paragraphs
  const formatAnswer = (answer) => {
    ...
    while ((match = pythonBlockRegex.exec(answer)) !== null) {
      if (match.index > lastIndex) {
        const textSegment = answer.slice(lastIndex, match.index);
        if (textSegment.trim()) {
          parts.push(<p key={`text-${lastIndex}`} className="whitespace-pre-wrap text-gray-300 print:text-black print:leading-tight">
            {textSegment}
          </p>);
        }
      }
    ...
    if (lastIndex < answer.length) {
      const textSegment = answer.slice(lastIndex);
      if (textSegment.trim()) {
        parts.push(<p key={`text-${lastIndex}`} className="whitespace-pre-wrap text-gray-300 print:text-black print:leading-tight">
          {textSegment}
        </p>);
    ...
    return parts.length ? parts : <p className="whitespace-pre-wrap text-gray-300 print:text-black print:leading-tight">{answer}</p>;
  };

  // Helper to detect if an answer contains Python code blocks
  const hasPythonCode = (answer) => {
    return answer && (answer.includes('```python') || /class\s+\w+:|def\s+\w+\(|import\s+\w+/.test(answer));
  };
    ...
    <div className="max-w-4xl mx-auto p-6 bg-gray-900 shadow-lg rounded-xl text-gray-100 print:bg-white print:text-black print:p-4">
      ...
      <div id="print-content" className="print:leading-tight">
          ...
          <div key={sectionIdx} className="mb-8 print:mb-4">
            ...
            <ol className="list-decimal pl-5 space-y-6 print:space-y-2">
              {section.questions.map((q, qIdx) => {
                ...
                const hasCode = q.answer && hasPythonCode(q.answer);

                return (
                  <li key={qIdx} className="space-y-2 print:space-y-1">
                    <div className="flex justify-between items-start gap-4">
                      {/* Question text with Python code blocks */}
                    ...
                    {/* Answer (with Python code blocks) */}
                      ...
                      <div className={`answer-content mt-3 p-4 rounded-lg border ${hasCode ? 'bg-gray-850' : 'bg-gray-800'} print:bg-white border-gray-700 print:border-black print:p-2 print:mt-1`}>
                        <div className="text-sm print:text-xs">
                          <div className="text-emerald-400 print:text-black text-xs font-semibold mb-2 flex items-center gap-2 print:mb-1">
                            <span>📝 Answer:</span>
                            {hasCode && <span className="text-blue-400 print:text-gray-600">(Includes Python Code)</span>}
                          </div>
        ...
        <div className="mt-8 pt-4 border-t border-gray-700 print:border-gray-300 text-center text-xs text-gray-500 print:text-gray-600 print:mt-4 print:pt-2">
          <p>© {new Date().getFullYear()} Python Question Paper - All Rights Reserved</p>
...
export default PythonQuestionPaperTemplate;